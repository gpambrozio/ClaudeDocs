...
[Claude Code on the web](claude-code-on-the-web.md) has its own admin surface: on the Cloud environments page in admin settings, owners and admins create [organization-shared environments](claude-code-on-the-web.md) that set the [network access level](claude-code-on-the-web.md), environment variables, and setup script for members’ cloud sessions, and choose the organization’s default environment.
