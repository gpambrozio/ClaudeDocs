...
| `CLAUDE_CODE_NO_FLICKER` | Set to `1` to enable [fullscreen rendering](fullscreen.md), a research preview that reduces flicker and keeps memory flat in long conversations |
...
| `ENABLE_PROMPT_CACHING_1H_BEDROCK` | Set to `1` when using [Bedrock](amazon-bedrock.md) to request a 1-hour prompt cache TTL instead of the default 5 minutes. Bedrock only |
...
| `OTEL_LOG_TOOL_DETAILS` | Set to `1` to include MCP server names and tool details in telemetry. Disabled by default to protect PII. See [Monitoring](monitoring-usage.md) |
