...
In [auto mode](permission-modes.md#eliminate-prompts-with-auto-mode), a classifier runs safety checks on actions such
...
[Server-side classifier review](permission-modes.md#server-side-classifier-review) lists which sessions ask the server
...
[only Claude Sonnet 5, Opus 4.7 or later, and the Fable models](permission-modes.md#enable-auto-mode-on-bedrock-agent-platform-or-foundry)
...
[non-interactive mode](headless.md) with `-p` it prints the text to stderr, and in `stream-json` output it emits a
`system` warning message, which Agent SDK applications can read from the message stream. In the
[VS Code extension](vs-code.md) the message appears as a notice in the conversation, with nothing to acknowledge.
...
notice; see [Server-side classifier review](permission-modes.md#server-side-classifier-review).
...
[gateway compatibility guide](llm-gateway-protocol.md#feature-pass-through) describes. A gateway that passes traffic
...
[`env` settings key](settings-reference.md#env):
...
[`CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS=1`](llm-gateway-protocol.md#disable-pre-release-capabilities) while
