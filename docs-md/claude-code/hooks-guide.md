...
`bypassPermissions` only applies if you started the session with bypass mode already available: `--dangerously-skip-permissions`, `--permission-mode bypassPermissions`, `--allow-dangerously-skip-permissions`, or `permissions.defaultMode: "bypassPermissions"` in [user, `--settings`, or managed settings](settings-reference.md). It doesn’t apply if bypass mode is disabled by [`permissions.disableBypassPermissionsMode`](permissions.md), or if you started the session in [restricted mode](cli-reference.md).Claude Code never saves it as `defaultMode`.
