...
* **Install and manage steps, and where `/plugin` runs**: see [Install and manage plugins](install.md)
* **What a command changes on disk and which scope takes precedence**: see [Plugin loading reference](loading.md)
* **What an error message means**: see [Troubleshoot plugins](troubleshooting.md)
...
For the create, test, and edit workflow that begins with this command, see [Create a plugin](create.md).
...
The command has no flag for another location. To scaffold inside a project instead, see [Create a plugin](create.md).
...
Most plugins install without a prompt. For a plugin whose marketplace entry [runs a command to install it](host-marketplace.md) or [sets a `headersHelper` for its download](host-marketplace.md#how-users-accept-a-headershelper-command), Claude Code first prints the command and asks `Run this command now? [y/N]`.

| Flag                        | Description                                                                                                                                                                                                                                                                                         |
| :-------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `-s, --scope <scope>`       | Installation scope: `user`, `project`, or `local`. Defaults to `user`                                                                                                                                                                                                                               |
| `--config <key=value>`      | Set a [`userConfig`](manifest-reference.md) option the plugin's manifest declares. Repeat the flag for each option. Requires Claude Code v2.1.147 or later                                                                                                                                 |
...
| `--prune`             | Also remove auto-installed [dependencies](dependencies.md) that no remaining plugin needs                                                                                                             |
...
Enable a disabled plugin. For a [plugin synced from claude.ai](loading.md#synced-plugins), pass `<name>@synced` as the plugin.
...
* **A scope that [takes precedence](loading.md) over the declaring one**: Claude Code writes an override at the scope you passed. For example, `claude plugin disable formatter --scope local` turns off a project-enabled plugin for you alone
...
When the plugin declares [dependencies](dependencies.md), Claude Code enables them too. The command fails in these cases:
...
Disable a plugin without uninstalling it. For a [plugin synced from claude.ai](loading.md#synced-plugins), pass `<name>@synced` as the plugin.
...
* **Another enabled plugin [depends on](dependencies.md) it**: the command fails and names the dependents to disable first
...
| `-y, --yes`                 | Accept a changed install command from a [command-source](host-marketplace.md) plugin, without the prompt. Required when stdin or stdout isn't a TTY, unless you pass `--accept-command`. Requires Claude Code v2.1.229 or later |
...
`managed` is the one scope you can update but not install to. For admin-installed plugins, see [Manage plugins for your organization](org.md).
...
* **`Synced from claude.ai`**: [plugins synced from your claude.ai account](loading.md#synced-plugins)
...
| `version`      | string           | For a marketplace install, the [version Claude Code computed](loading.md#versions-and-updates) at install. For a session-only, skills-directory, or synced plugin, the manifest's `version`, or `unknown` when it declares none |
...
| `source`          | string or object | The marketplace entry's [source](marketplace-reference.md): a string for a relative path, an object otherwise |
...
For what the two cost figures mean, see [Measure plugin cost and usage](measure.md).
...
Remove auto-installed [dependencies](dependencies.md) that no installed plugin needs anymore. The command never removes a plugin you installed yourself. `autoremove` is an alias for `prune`.
...
Run a plugin's [eval cases](../plugin-evals.md) and report scored results. Requires Claude Code v2.1.269 or later.
...
See [Test plugins with evals](../plugin-evals.md) for the case format, graders, results, and CI usage.
...
| :------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------- |
| `--runs <n>`               | Runs per case in each [arm](../plugin-evals.md#compare-against-a-no-plugin-baseline)                                                                              | Each case's `runs`, else 3                                                     |
...
The exit code reports how the run ended. To act on it in a pipeline, see [Run evals in CI](../plugin-evals.md#run-evals-in-ci).
...
Create an eval suite for the plugin in the current directory. Requires Claude Code v2.1.269 or later. See [Create your first eval suite](../plugin-evals.md#create-your-first-eval-suite).
...
For when to tag a release, see [Publish a plugin](publish.md).
...
Validate a plugin manifest, a marketplace manifest, or the skills, agents, and commands in a directory, and exit with a code a CI job can act on. For the create, test, and edit workflow, see [Create a plugin](create.md). For what the validator checks in each manifest, see the [plugin manifest reference](manifest-reference.md) and the [marketplace reference](marketplace-reference.md).
...
For what a marketplace is and how Claude Code caches it, see [Plugin loading reference](loading.md).
...
After you add it, Claude Code installs any [dependencies](dependencies.md) that your installed plugins were missing.
...
| `--claudeai`          | Read the argument as the name of a [marketplace hosted on claude.ai](install.md#add-from-claude-ai) instead of a source. Requires Claude Code v2.1.273 or later |

`<source>` takes any of the forms in the table below, and its form decides the source type and how Claude Code fetches the marketplace. For the resulting source object, see the [marketplace reference](marketplace-reference.md).
...
For a host whose clone URLs don't carry the `.git` suffix, such as AWS CodeCommit, add the marketplace as a git entry in [`extraKnownMarketplaces`](../settings-reference.md#extraknownmarketplaces) instead. Claude Code clones a git entry whether or not its URL ends in `.git`.
...
Add a [marketplace hosted on claude.ai](install.md#add-from-claude-ai) by the name printed in the `From claude.ai:` section of `claude plugin marketplace list`:
...
An added [claude.ai marketplace](install.md#add-from-claude-ai) has no local clone, so its entry carries its claude.ai identifiers, `marketplaceId` and `organizationUuid`, in place of `installLocation`. It also carries `scope` when one is recorded, and `status`.

If your terminal sessions [sync plugins from your claude.ai account](loading.md#synced-plugins), the text listing ends with a `From claude.ai:` section. That section names the marketplaces claude.ai lists for your account that you haven't added, both git-based and hosted. It requires Claude Code v2.1.273 or later.

To add a marketplace from that section, see [Add a marketplace from claude.ai](install.md#add-from-claude-ai).
...
For which surfaces have `/plugin`, how to install without it, and what each panel tab shows, see [Install and manage plugins](install.md).
...
| `/plugin install <plugin> --marketplace <source>`   | `i`                                            | Adds the marketplace at `<source>` when you haven't added it yet, asking you to confirm first, then opens the plugin's details. See [Add a marketplace and install in one command](install.md#add-a-marketplace-and-install-in-one-command). Requires Claude Code v2.1.275 or later |
| `/plugin manage`                                    |                                                | Opens the **Installed** tab                                                                                                                                                                                                                                                                  |
| `/plugin stats`                                     |                                                | Opens the **Stats** tab, in sessions where [`/skill-doctor`](../skills.md#find-unused-skills) is available. Anywhere else it opens the panel on the **Discover** tab                                                                                                                           |
...
| `/plugin configure <plugin>`                        | `config`                                       | Opens the plugin's [`userConfig`](manifest-reference.md) dialog, or reports that the plugin declares none. Requires Claude Code v2.1.147 or later                                                                                                                                   |
...
When a reloaded plugin's [dependencies](dependencies.md) are missing, Claude Code installs them, reloads again, and appends `(+ N dependencies: <names>) resolved` to the summary.

### Reloads that change MCP tools

When the reload would add or remove a plugin MCP server or the `LSP` tool, and that change would invalidate the [prompt cache](../prompt-caching.md#enabling-or-disabling-a-plugin), Claude Code doesn't apply the reload. It prints a line such as `This reload changes MCP tools (<server>) — your next message will re-read the whole conversation instead of using the cache. Run /reload-plugins --force to apply.` Pass `--force` to apply it anyway.

### Sessions without an interactive terminal

`/reload-plugins` also runs in sessions without an interactive terminal, such as the desktop app, the Agent SDK, and [non-interactive mode](../headless.md) with `-p`. Requires Claude Code v2.1.260 or later.

In those sessions, the command runs only when you type it into the session yourself, such as in the `-p` prompt or the desktop app's prompt box. When it arrives another way, such as through [Remote Control](../remote-control.md) or a message relayed from Slack, the command replies `/reload-plugins isn't available over a remote connection in this session.` and reloads nothing.
...
Plugin authors use them to test a plugin before publishing. For the load-edit-reload workflow, see [Develop without a marketplace](create.md#develop-without-a-marketplace).
...
When a session-only plugin shares a name with an installed plugin, Claude Code loads the session-only copy for that session and skips the installed one. The installed copy loads instead if you disabled the session-only copy with `claude plugin disable <name>@inline`, or if managed settings lock that plugin name. For the precedence, see [Plugin loading reference](loading.md).

An administrator can reject both flags, and folders named in the [`CLAUDE_CODE_PLUGIN_DIRS`](../env-vars.md#variables) variable, with the managed [`disableSideloadFlags`](../settings-reference.md#disablesideloadflags) setting. Claude Code then prints that the flag is disabled by your organization's managed settings and exits `1` without starting.

From the Agent SDK, the [`plugins`](../agent-sdk/plugins.md) option is the equivalent of `--plugin-dir`.

## Next steps

* [Install and manage plugins](install.md): the same operations as steps, with what you see at each one
* [Plugin loading reference](loading.md): what each command changes on disk and which scope takes effect
* [Troubleshoot plugins](troubleshooting.md): install, marketplace, load, and validation error messages with their fixes
* [Plugin manifest reference](manifest-reference.md): the fields `claude plugin validate` checks
