...
* **Claude Code's own security model**: see [Security](../security.md)
* **Restricting or requiring plugins for an organization**: see [Manage plugins for your organization](org.md)
* **The `security-guidance` or `claude-security` plugins**: this page isn't about them. See [`security-guidance`](../security-guidance.md) and [`claude-security`](../claude-security.md)
...
* **Hooks**: a plugin's [hooks](../hooks.md) run as shell commands at points in Claude Code's lifecycle, such as before or after a tool call.
* **MCP and LSP servers**: Claude Code connects to the [MCP servers](../mcp.md) an enabled plugin declares and gives Claude their tools. A stdio MCP server runs as a process that Claude Code starts on your machine. Claude Code also starts the language servers the plugin declares.
...
* **Updates**: when auto-update is on for the marketplace you installed a plugin from, Claude Code updates that plugin in the background, so the files you reviewed can change on disk. [When auto-update runs](loading.md#when-auto-update-runs) has the timing. To turn auto-update on or off per marketplace, see [Keep plugins updated](install.md#keep-plugins-updated).

Claude Code's [permission rules](../permissions.md) and [sandbox](../sandboxing.md) cover the tool calls Claude makes, not the code a plugin runs by itself:
...
Installing a plugin also enables it, unless its manifest or marketplace entry sets [`defaultEnabled: false`](install.md#choose-an-install-scope) and you haven't enabled it yourself.
...
For how the official, community, and demo marketplaces differ and where to browse what each one lists, see [Anthropic's marketplaces](anthropic-marketplaces.md).
...
In your shell, run [`claude plugin uninstall <plugin>`](cli-reference.md#plugin-uninstall) with the `--scope` you installed it at. Then check what the uninstall removed and what it left:
...
* **Cached files**: the plugin's files stay on disk under `~/.claude/plugins/cache/` for 14 days before a [background sweep removes them](loading.md#cleanup-of-previous-versions). After you uninstall your last plugin, orphaned directories stay until you install another. To delete the files now, remove the plugin's directory under `~/.claude/plugins/cache/<marketplace>/<plugin>/` yourself.
* **The marketplace**: if you don't trust the marketplace's owner either, [remove the marketplace](install.md#manage-marketplaces) too, which uninstalls every plugin you installed from it.
...
If your organization sets `pluginTrustMessage` in [managed settings](org.md), Claude Code appends that text to the warning.
...
* **Untrusted marketplace source**: when a marketplace uses an official or community name but its source is outside `github.com/anthropics/`, Claude Code stops loading the marketplace and the plugins you installed from it. The error is [Marketplace is registered from an untrusted source](../errors.md#marketplace-is-registered-from-an-untrusted-source).
* **Archive integrity**: when a marketplace entry pins an [`archive` source](marketplace-reference.md#archive-plugin-source) to a `sha256` digest and the downloaded file's digest doesn't match it, Claude Code refuses the install. The error is [Plugin archive integrity check failed](../errors.md#plugin-archive-integrity-check-failed).
...
With [managed settings](org.md), an administrator can enforce these plugin controls:
...
* Stop plugins from members' claude.ai accounts from loading in Claude Code, with [`syncClaudeAiPlugins`](org.md#control-matrix)

The [control matrix](org.md#control-matrix) says what each key does and doesn't cover.

## Find plugins in telemetry

If your organization exports Claude Code's [OpenTelemetry events](../monitoring-usage.md) to its own backend, the [marketplace tiers](#marketplace-tiers) decide which plugin names appear there:

* **[Plugin loaded event](../monitoring-usage.md#plugin-loaded-event)**: the event reports official-tier plugin and marketplace names as they are. For the community and third-party tiers, `plugin.name` and `marketplace.name` are the literal string `third-party` unless you set `OTEL_LOG_TOOL_DETAILS=1`.
* **Plugin scope**: the loaded event's `plugin.scope` still reports where the plugin came from, such as `org` for a plugin your managed settings enable or `user-local` for any other third-party plugin. The [plugin loaded event](../monitoring-usage.md#plugin-loaded-event) lists every value.
* **[Plugin installed event](../monitoring-usage.md#plugin-installed-event)**: unless you set `OTEL_LOG_TOOL_DETAILS=1`, the event omits the name fields for non-official plugins instead of reporting `third-party`.
* **[Claude Code Analytics API](../../api/api/beta/organization/analytics/plugins/list.md)**: Claude Code reports plugins from the official and community tiers by name and reports every other plugin as `third-party`.

## Next steps

* [Manage plugins for your organization](org.md): restrict which marketplaces users can install from and require the ones you trust
* [Install and manage plugins](install.md): review a plugin's details pane before you choose a scope
* [Anthropic's marketplaces](anthropic-marketplaces.md): which marketplace names are Anthropic's
* [Security](../security.md): Claude Code's own security model
