...
Code intelligence plugins work in terminal sessions. In [cloud sessions](../claude-code-on-the-web.md), Claude Code doesn't start plugin language servers, so Claude gets no diagnostics or code navigation there. To write your own language server plugin, or to connect a language server that has no plugin, see [LSP servers in plugin components](components.md#lsp-servers).

To get started, find your language in the table under [Install a code intelligence plugin](#install-a-code-intelligence-plugin). The plugins in that table come from Anthropic's [official plugin marketplace](anthropic-marketplaces.md).
...
A confirmation message says whether the plugin is active now or needs `/reload-plugins`. If the install fails with `Marketplace "claude-plugins-official" not found`, see the [troubleshooting entry for that error](troubleshooting.md#marketplace-claude-plugins-official-not-found). To control where the plugin is installed, or to run the install from your shell instead of inside Claude Code, see [Install plugins](install.md).
...
* **Code navigation**: Claude gets an `LSP` tool that looks up symbols through the server instead of searching text for them. The tool is read-only. For what Claude can look up with the tool and how permissions apply to it, see [LSP tool behavior](../tools-reference.md#lsp-tool-behavior).
...
* **Not for cloud sessions**: the dialog never appears when your terminal is attached to a cloud session, such as one you started with [`claude --cloud`](../claude-code-on-the-web.md#from-terminal-to-cloud).
...
The plugins troubleshooting page covers the symptoms specific to code intelligence plugins under [Language server doesn't start, uses too much memory, or reports wrong diagnostics](troubleshooting.md#language-server-doesnt-start):
...
2. Then load the plugin with [`--plugin-dir`](cli-reference.md#flags-that-load-a-plugin-for-one-session) or publish it to a marketplace.

For the file's fields and a worked example, see [LSP servers in plugin components](components.md#lsp-servers).

## Next steps

* [LSP servers in plugin components](components.md#lsp-servers): write the `.lsp.json` for a language server that has no official plugin
* [Install and manage plugins](install.md): scopes, updates, and uninstalling
* [Troubleshoot plugins](troubleshooting.md): load errors beyond the language-server ones on this page
* [Find plugins in the official marketplace](anthropic-marketplaces.md#find-plugins-in-the-official-marketplace): where to browse the rest of the official marketplace
