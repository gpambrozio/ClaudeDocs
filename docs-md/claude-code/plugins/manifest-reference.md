...
* **Learning to build a plugin**: start with [Create a plugin](create.md)
* **What each component does at runtime**: see [Plugin components](components.md)
...
* A message from `claude plugin validate`: the [troubleshooting page](troubleshooting.md) lists each message with its fix and links to the relevant sections on this page
...
| `workflows`                          | Path, or array of paths          | [Workflow](../workflows.md#distribute-a-workflow-in-a-plugin) `.js` files or directories. Replaces the default `workflows/` scan                                                                                                                                                                                   |
...
For a marketplace-installed plugin, a `displayName` on the [marketplace entry](marketplace-reference.md#plugin-entries) takes precedence over this value.

### `version`

A version string, not checked against semver. Setting it pins the plugin to that version until you change it; see [Versions and updates](loading.md#versions-and-updates). A plugin with a [`command` source](marketplace-reference.md), a plugin from a [marketplace hosted on claude.ai](install.md#add-from-claude-ai), and a plugin [loaded in place](loading.md#find-plugins-on-disk) from a marketplace added as a local directory aren't pinned by this field.
...
Whether the plugin starts enabled when the user hasn't set it in [`enabledPlugins`](../settings-reference.md#enabledplugins). Defaults to `true`. A plugin that an enabled plugin depends on starts enabled regardless. The same field in the marketplace entry overrides this one.
...
Plugins that must be enabled for this one to work. Each entry is `"name"`, `"name@marketplace"`, or `{ "name": "...", "marketplace": "...", "version": "..." }`. Bare names resolve against this plugin's own marketplace. See [dependency constraints](dependencies.md).

### `settings`

Settings Claude Code applies while the plugin is enabled. Only `agent` and `subagentStatusLine` take effect; other keys are dropped at load. A `settings.json` at the plugin root takes precedence over this key. See [Default settings](components.md#default-settings).

## Component path forms

Every component key accepts a path relative to the plugin root. `hooks`, `mcpServers`, `lspServers`, and `experimental.monitors` also accept inline configuration, `commands` also accepts an object map, and `mcpServers` also accepts MCP bundle paths and URLs. The examples that follow show each accepted shape once. For what each component does at runtime, see [Plugin components](components.md).
...
`hooks` takes a `.json` file path, an inline hooks object in the same shape as [`hooks` in `settings.json`](../hooks.md#configuration), or an array mixing both. For hook events and handler fields, see the [hooks reference](../hooks.md#hook-events).
...
`mcpServers` takes a `.json` file path, an MCP bundle path or URL, an inline map, or an array mixing them. For server config fields, see [plugin-provided MCP servers](../mcp.md#plugin-provided-mcp-servers).
...
For the language servers Anthropic publishes as plugins and how the servers behave at runtime, see [Code intelligence](code-intelligence.md).
...
Non-sensitive values are saved under [`pluginConfigs`](../settings-reference.md#pluginconfigs) in the user's `settings.json`. Sensitive values go to the platform's secure credential store instead. The [settings page](../settings-reference.md#pluginconfigs) lists which settings files `pluginConfigs` is read from.

### Reference a saved value

Reference a saved value where the plugin needs it, in one of two forms:

* **`${user_config.KEY}`**: substituted in MCP server config, LSP server config, [exec-form](../hooks.md#exec-form-and-shell-form) hook `args`, and skill and agent content. In skill and agent content, only non-sensitive values are substituted, and a sensitive value there becomes a placeholder
...
Shell-form hook commands, monitor commands, and MCP [`headersHelper`](../mcp.md#use-dynamic-headers-for-custom-authentication) reject `${user_config.*}`. A component that references it in one of these fields fails with an [error](../errors.md#plugin-command-references-user-config) instead of running, because the field's value is passed to a shell that would re-parse the substituted value.
...
`channels` declares the message channels a plugin provides, such as a bridge to a chat app. When you declare one, Claude Code can prompt for the channel's configuration when the plugin is enabled. For how the server injects messages, see the [channels reference](../channels-reference.md#package-as-a-plugin).
...
`${CLAUDE_PLUGIN_ROOT}` changes when the plugin updates, so don't write state there. For where the root moves and when the old directory is cleaned up, see the [loading page](loading.md).

When you uninstall the plugin from the last place it's installed, the `${CLAUDE_PLUGIN_DATA}` directory is deleted unless you pass [`--keep-data`](cli-reference.md).
...
* **Hook commands**: use [exec form](../hooks.md#exec-form-and-shell-form) with `args` so each path is one argument with no quoting
...
| Agents        | `agents/`                    | Agent Markdown files. Subfolders are part of the [agent name](components.md#agents)                                                                                                                                                                                                                                   |
...
| Executables   | `bin/`                       | Files here are on the Bash tool's `PATH` while the plugin is enabled, so Claude runs them as bare commands. claude.ai and Cowork don't install a plugin that has this directory, including one you [distribute through claude.ai organization settings](host-marketplace.md#distribute-through-organization-settings) |
...
To click through this layout and read what each file does, open the [plugin explorer](components.md#explore-the-plugin-directory).
...
A [marketplace entry](marketplace-reference.md) accepts every field on this page alongside [its own fields](marketplace-reference.md#plugin-entries), including `strict`.
...
For the full precedence table, see [Strict mode](marketplace-reference.md).

## Next steps

* [Add components to a plugin](components.md): what each component does at runtime, with an example that validates
* [Marketplace reference](marketplace-reference.md): the entry fields a marketplace can set for your plugin
* [Plugin commands reference](cli-reference.md#plugin-validate): `claude plugin validate` flags and output
* [Troubleshoot plugins](troubleshooting.md#claude-plugin-validate-reports-errors): each validation message with its fix
