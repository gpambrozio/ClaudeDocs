...
* **You want to install plugins**: see [Install and manage plugins](install.md)
...
The spinner tip and the session-start notification both stop appearing when the user or project sets [`spinnerTipsEnabled`](../settings-reference.md#spinnertipsenabled) to `false`, or when a [`spinnerTipsOverride`](../settings-reference.md#spinnertipsoverride) with `excludeDefault` replaces the built-in tips. The Discover-tab pin isn't affected by either setting.
...
Users see no suggestions from a marketplace until an administrator allowlists it in [managed settings](org.md), even when its `marketplace.json` declares `relevance`.

To allowlist a marketplace, edit your managed settings as follows:

* Add the marketplace name to `pluginSuggestionMarketplaces`.
* For any marketplace other than the official Anthropic marketplace, also declare the marketplace source, either as that name's entry in [`extraKnownMarketplaces`](org.md#require-a-marketplace-and-its-plugins) or as an entry in [`strictKnownMarketplaces`](org.md#allowlist-with-strictknownmarketplaces).
...
* [Host a marketplace](host-marketplace.md): run the marketplace that hosts your plugins
* [Marketplace reference](marketplace-reference.md#plugin-entries): every field a plugin entry accepts
* [Recommend your plugin from your CLI](cli-hints.md): prompt users from your own CLI instead of from Claude Code's session signals
* [Manage plugins for your organization](org.md): `extraKnownMarketplaces`, `strictKnownMarketplaces`, and the rest of the plugin policy keys
