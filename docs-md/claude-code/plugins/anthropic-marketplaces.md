...
* **How to install a plugin**: see [Install plugins](install.md)
* **A failed install**: see [Troubleshoot plugins](troubleshooting.md)
...
| How you get it   | Claude Code adds it the first time you start an interactive terminal session, unless a [managed policy](org.md#allow-the-official-marketplace-and-your-own) or `CLAUDE_CODE_DISABLE_OFFICIAL_MARKETPLACE_AUTOINSTALL` blocks it. See [Marketplace `claude-plugins-official` not found](troubleshooting.md#marketplace-claude-plugins-official-not-found) if it's missing | You add it in a Claude Code session with `/plugin marketplace add anthropics/claude-plugins-community` | You add it in a Claude Code session with `/plugin marketplace add anthropics/claude-code` |

If you wrote a plugin and want other people to install it, see [Publish a plugin](publish.md), which covers your own marketplace and submitting to Anthropic's directory.
...
* **In Claude Code, by name**: run `/plugin install <name>` in a session, which looks the name up in the marketplaces you've added. If the plugin is in one of them, its details open in the `/plugin` panel, and nothing installs until you choose an [installation scope](install.md#install-a-plugin) and confirm there. If it isn't, you see `Plugin "<name>" not found in any marketplace`.
...
Anthropic's directory is separate from these marketplaces. The directory is the catalog on claude.ai, and `/plugin` doesn't list it. A plugin you add from the directory on claude.ai reaches Claude Code through [account sync](loading.md#synced-plugins). To list your own plugin there, see [Submit to Anthropic's directory](publish.md#submit-to-anthropics-directory).

To install from the desktop app or from a script, or to see what a cloud session loads, see [Install plugins](install.md).
...
For `not found` errors and marketplaces that won't add, see [Troubleshoot plugins](troubleshooting.md#install-a-plugin).
...
Anthropic doesn't review third-party marketplaces, so read [Plugin security and trust](security.md) before you add one.
...
For other ways to add a marketplace, see [Add a marketplace](install.md#add-a-marketplace).

## Next steps

* [Install and manage plugins](install.md): install a plugin from one of these marketplaces and choose a scope
* [Plugin security and trust](security.md): what a plugin can do on your machine and how to review one before you install it
* [Code intelligence plugins](code-intelligence.md): install one of the official marketplace's language-server plugins
* [Create a marketplace](create-marketplace.md): run your own marketplace alongside Anthropic's
