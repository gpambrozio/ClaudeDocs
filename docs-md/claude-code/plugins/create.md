...
* **Not sure you need a plugin**: see [Decide whether you need a plugin](overview.md#decide-whether-you-need-a-plugin) on the overview
...
In this walkthrough, you create a plugin whose only component is one skill, a greeting, and run it with `--plugin-dir`, which loads a plugin for one session without installing it. A plugin can hold any mix of [components](components.md), such as skills, agents, hooks, and MCP servers, and none is required; one skill is the smallest example that shows the layout.

You need Claude Code [installed and signed in](../quickstart.md#step-1-install-claude-code).
...
The [manifest](manifest-reference.md) is a JSON file named `plugin.json` that tells Claude Code the plugin's name and describes it. Save this one as `my-first-plugin/.claude-plugin/plugin.json`:
...
* **`version`**: optional. Setting it keeps users on that version until you change it; [Release a new version](host-marketplace.md#release-a-new-version) says when to set or omit it.
* **`author`**: who to credit. `name` is required inside it; `email` and `url` are optional.

Every other field is on the [manifest reference](manifest-reference.md#fields).
...
The command prints the manifest path it checked and `✔ Validation passed`. If it prints `✘ Validation failed` instead, each line above that result line names the field to fix. Look up each message under [`claude plugin validate` reports errors](troubleshooting.md#claude-plugin-validate-reports-errors).
...
To have Claude scaffold and check a larger plugin with you, [install](install.md#install-a-plugin) Anthropic's `plugin-dev` plugin from the `claude-plugins-official` marketplace, which adds skills and agents for writing components such as skills, hooks, and MCP servers and for validating the finished plugin. Once it's installed, run `/plugin-dev:create-plugin` followed by a description of the plugin you want, and Claude walks you through designing, creating, and validating it.
...
* **Send it to a few people directly**: give them the plugin's directory or a `.zip` of it, and nothing needs to be published. See [Share a plugin without a marketplace](publish.md#share-a-plugin-without-a-marketplace).
* **List it in your own marketplace**: teammates add your marketplace once and install the plugin by name, and they receive your updates. See [Publish through your own marketplace](publish.md#publish-through-your-own-marketplace).
* **Submit it to Anthropic's directory**: after it passes review, people can add it on claude.ai and in Cowork, and it reaches Claude Code through their account. See [Submit to Anthropic's directory](publish.md#submit-to-anthropics-directory).

### Plugin layout

Each kind of [component](components.md), such as skills, agents, hooks, and MCP servers, goes in a fixed directory under the plugin root, which is the directory you pass to `--plugin-dir`. Add only the directories you use. To click through a complete plugin directory and read what each file does, open the [plugin explorer](components.md#explore-the-plugin-directory).

The table lists the directories most plugins start with, and the [full layout](manifest-reference.md#standard-layout) lists the rest.
...
You don't need a [marketplace](overview.md#get-plugins-from-a-marketplace) to run a plugin you're writing. Load it directly from disk or a URL instead:
...
If two plugins loaded in different ways share a name, see [Name conflicts](loading.md#name-conflicts) for which one Claude Code keeps.
...
A message appears in the session for each of these changes. If loading or unloading a plugin mid-conversation would [invalidate the prompt cache](../prompt-caching.md#enabling-or-disabling-a-plugin), the change is held instead, and the message tells you to run `/reload-plugins` to apply it.
...
To load plugins in a session where you can't add the `--plugin-dir` flag, list their absolute paths in the [`CLAUDE_CODE_PLUGIN_DIRS`](../env-vars.md#variables) environment variable instead. Claude Code loads each path as it loads a `--plugin-dir` path. These plugins load in addition to any you pass with `--plugin-dir`. [Project and local settings can't set this variable](../settings-reference.md#variables-claude-code-ignores-in-env). `CLAUDE_CODE_PLUGIN_DIRS` requires Claude Code v2.1.280 or later.

Managed settings can turn off `--plugin-dir` and `CLAUDE_CODE_PLUGIN_DIRS`. See [Flags that load a plugin for one session](cli-reference.md#flags-that-load-a-plugin-for-one-session). To test a plugin together with a plugin it depends on, see [Test a plugin and its dependency locally](dependencies.md#test-a-plugin-and-its-dependency-locally).
...
Pass `--with skills` to have `claude plugin init` scaffold a skill under `skills/` for you. The other `--with` values are on the [plugin commands reference](cli-reference.md#plugin-init).
...
1. In your shell, run `claude plugin validate <path>`. It checks the manifest and the frontmatter of every skill, agent, and command file, and exits `0` on `Validation passed`. Add `--strict` to fail on warnings too. Exit codes and directory handling are on the [plugin commands reference](cli-reference.md#plugin-validate).
...
To check an MCP server, run `/mcp` in the session to see the server's status. When the server is healthy, `/mcp` lists it as connected. If it isn't, see [MCP servers that don't start](troubleshooting.md#invalid-mcp-server-config-for-and-mcp-servers-that-dont-start).

To check a hook, trigger the event it matches. For example, ask Claude to edit a file to trigger a `PostToolUse` hook. Then read the [debug log](../hooks.md#debug-hooks), which shows which hooks matched, their exit codes, and their output.

The next sections cover the failures you're most likely to hit while developing, and the [troubleshooting page](troubleshooting.md#build-a-plugin) has the full entry for each.
...
* [Plugin components](components.md): add agents, hooks, MCP servers, LSP servers, and user configuration to your plugin
* [Test plugins with evals](../plugin-evals.md): write eval cases and run them with `claude plugin eval` to check how reliably the plugin guides Claude's behavior
* [Publish a plugin](publish.md): version it, put it in a marketplace, and submit it for review
...
* [Plugin manifest reference](manifest-reference.md): every `plugin.json` field, path rule, and directory
* [Skills](../skills.md): write the skills your plugin provides
