...
This reference is for marketplace maintainers who need an exact field name or value, and for administrators who need to know which `source` values are valid in [`extraKnownMarketplaces`](../settings-reference.md#extraknownmarketplaces), [`strictKnownMarketplaces`](../settings-reference.md#strictknownmarketplaces), and [`blockedMarketplaces`](org.md#restrict-what-users-can-install).

These cases are covered on other pages:

* **Building or hosting a marketplace**: see [Create a marketplace](create-marketplace.md) and [Host and maintain a marketplace](host-marketplace.md)
* **Allowlist and blocklist recipes**: see [Manage plugins for your organization](org.md)
...
| `forceRemoveDeletedPlugins`                | boolean          | When `true`, a plugin you remove from `plugins` is uninstalled on users' machines. See [Host and maintain a marketplace](host-marketplace.md)                                                                                                                 |
| `allowCrossMarketplaceDependenciesOn`      | array of strings | Marketplace names whose plugins may be installed as dependencies of this marketplace's plugins. When you install a plugin, only the list in that plugin's own marketplace applies, for its whole dependency chain. See [Plugin dependencies](dependencies.md) |
| `renames`                                  | object           | Map from a former plugin `name` to its current name, or to `null` for a plugin you removed. Requires Claude Code v2.1.193 or later. See [Host and maintain a marketplace](host-marketplace.md)                                                                |
...
An entry also accepts every [`plugin.json` field](manifest-reference.md), such as `description`, `version`, `author`, `commands`, and `hooks`. For when those fields apply, see [How an entry combines with plugin.json](#entry-and-plugin-json).
...
| `description`    | string           | Shown in [`/plugin`](install.md) listings and details                                                                                                                                                                                                                                                    |
| `version`        | string           | Version string for the plugin. When `plugin.json` also sets `version`, `plugin.json` takes precedence and `claude plugin validate` warns. See [Plugin loading reference](loading.md)                                                                                                                     |
...
| `relevance`      | object           | Signals that tell Claude Code when to suggest the plugin. See [Recommend plugins for your org](relevance.md)                                                                                                                                                                                             |
| `dependencies`   | array            | Plugins that must be enabled for this one to work. Each item is `"name"`, `"name@marketplace"`, or an object. See [Plugin dependencies](dependencies.md)                                                                                                                                                 |
| `defaultEnabled` | boolean          | Default `true`. Whether the plugin starts enabled when the user hasn't set it in [`enabledPlugins`](../settings-reference.md#enabledplugins). The entry value takes precedence over `plugin.json`                                                                                                                   |
...
* **No `plugin.json`**: the entry is the manifest regardless of `strict`. Every manifest field in the entry applies, including [`mcpServers`, `lspServers`, `userConfig`, and `channels`](manifest-reference.md).
...
| `true`, the default | present       | any                    | `plugin.json` is the authority. Claude Code appends the entry's component fields to it, except `hooks`, whose matchers [replace the manifest's per event](manifest-reference.md#how-entry-fields-combine-with-plugin-json) |
...
For how each type is fetched, cached, and versioned, see [Plugin loading reference](loading.md).
...
Claude Code fetches the package with your npm client. The package's install scripts, such as `preinstall` or `postinstall`, never run, and its dependencies aren't installed during the fetch. If the package has a supported lockfile beside its `package.json`, Claude Code installs those [Node.js package dependencies](loading.md#node-js-package-dependencies) in a separate step, also with scripts disabled.
...
For how users accept the command, see [Install from your shell](install.md#install-from-your-shell). For what users see after you change it, see [Change the command of a command source](host-marketplace.md#change-the-command-of-a-command-source). Administrators turn command sources off with [`disableCommandPluginSources`](../settings-reference.md#disablecommandpluginsources).
...
* **`copy`**: Claude Code copies the directory into the plugin cache and derives the [plugin version](loading.md#how-claude-code-computes-the-version) from a hash of the copied files. Your tool can delete or rewrite the directory after the command exits. A re-run that produces identical files counts as up to date.
...
* **Include `node_modules`**: Claude Code skips the [Node.js package dependency install](loading.md#node-js-package-dependencies) for a link-mode plugin, so print a directory that already contains the packages the plugin needs.
...
* **[`claude plugin marketplace add`](cli-reference.md)**: Claude Code builds the source from the string you pass.
* **[`extraKnownMarketplaces`](../settings-reference.md#extraknownmarketplaces)**: you write the source yourself as the `source` object.
* **[`strictKnownMarketplaces`](../settings-reference.md#strictknownmarketplaces) and [`blockedMarketplaces`](org.md#restrict-what-users-can-install)**: administrators write sources in these two policy lists. `strictKnownMarketplaces` is the allowlist and `blockedMarketplaces` is the blocklist.
...
| `skills-dir`  | none                                 | Not produced                                                                                                                                           | Fails to load: `Unsupported marketplace source type`         | Keeps [skills-directory plugins](org.md#keep-skills-directory-plugins-loading) loading while an allowlist is set. See [Source values valid only in policy lists](#source-values-valid-only-in-policy-lists) | Stops skills-directory plugins from loading                            |
...
| `headersHelper` | `url`           | Command that prints headers whose values are too short-lived to list in `headers`. Requires Claude Code v2.1.238 or later. See [Authenticate archive downloads](host-marketplace.md#authenticate-archive-downloads)                              |
...
| `skipLfs`       | `github`, `git` | Accepted and has no effect. See [Keep plugin files out of Git LFS](host-marketplace.md#keep-plugin-files-out-of-git-lfs)                                                                                                                         |
...
* **`skills-dir`**: not a source. If you set `strictKnownMarketplaces` at all, [skills-directory plugins](org.md#keep-skills-directory-plugins-loading) stop loading until you add `{"source": "skills-dir"}` to that list.
...
For match order, exact-`ref` semantics, and recipes, see [Manage plugins for your organization](org.md).
...
`claude plugin validate <path>` takes the marketplace root or the marketplace file itself. It prints errors and warnings. For exit codes and `--strict`, see [plugin validate](cli-reference.md#plugin-validate).

A message names a plugin entry by its index, written as `plugins.1.source` or `plugins[1].source`.

A message prefixed with an entry index and `plugin.json →`, such as `plugins[2] plugin.json →`, is about that plugin's own files. [`claude plugin validate` reports errors](troubleshooting.md#claude-plugin-validate-reports-errors) lists those messages with their fixes.
...
[`claude plugin list`](cli-reference.md) shows a plugin that failed to load with its error, and [Troubleshoot plugins](troubleshooting.md) covers the load-time strings.

## Next steps

* [Create a marketplace](create-marketplace.md): build a marketplace from these fields and install from it locally
* [Host and maintain a marketplace](host-marketplace.md): where to put the file and how users receive changes
* [Plugin manifest reference](manifest-reference.md): the `plugin.json` fields an entry can override
* [Manage plugins for your organization](org.md): allowlist and blocklist recipes that use these source values
