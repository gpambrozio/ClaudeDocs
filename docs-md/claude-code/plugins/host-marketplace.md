...
* **You haven't written the catalog file yet**: start with [Create a marketplace](create-marketplace.md)
* **You're an admin requiring, restricting, or pre-installing marketplaces across your organization's machines**: read [Manage plugins for your organization](org.md)
...
To pin a branch or tag of a GitHub or git-URL marketplace, tell users to append `#<ref>`, as in `your-org/your-marketplace#stable`. The [plugin commands reference](cli-reference.md#plugin-marketplace-add) lists every form the command accepts.
...
For plugins installed from a local path, or from a [`command` source](marketplace-reference.md#command-plugin-source) whose `mode` is the default `copy`, Claude Code preserves only symlinks that resolve within the plugin's own directory and skips all others.
...
* **Plugin sources**: each plugin source must be of type `github`, `url`, or `git-subdir`, or a [relative path](marketplace-reference.md#relative-path-plugin-source) that starts with `./`
...
For a GitHub Enterprise Server host, users need git access to that host from their machine. See [Plugin marketplaces on GHES](../github-enterprise-server.md#plugin-marketplaces-on-ghes) for what each Claude Code surface needs to reach a GHES-hosted marketplace.
...
* **The check finds new commits, or fails because it can't reach or authenticate to the remote**: Claude Code clones the marketplace again and replaces the existing checkout with the new clone. If that clone fails, the existing checkout stays in place. The re-clone can [time out on large repositories](troubleshooting.md#git-clone-timed-out-after-120s).
...
| An administrator           | Registers the marketplace and turns its plugins on for everyone with `extraKnownMarketplaces` and `enabledPlugins` in managed settings, and sets `autoUpdate` there   | [Require a marketplace and its plugins](org.md#require-a-marketplace-and-its-plugins) and [Set update policy](org.md#set-update-policy) |
| Each person                | Needs read access to a private git repository, with credentials already stored on their machine. Without an administrator, they also run the add and install commands | [Add a private marketplace](install.md#add-a-private-marketplace)                                                                                |
...
* **A pre-populated plugins directory**: [Seed containers and CI](org.md#seed-containers-and-ci), which also serves users who have no git-host account
...
* **Ask an admin to set it**: if an admin sets `"autoUpdate": true` on your marketplace's `extraKnownMarketplaces` entry in managed settings, it's on for everyone who receives those settings. See [Set update policy](org.md#set-update-policy).
...
For what users see when an update reaches them, see [When auto-update runs](loading.md#when-auto-update-runs).

### Release a new version

To release a new version to users, change the plugin's `version`. Users get a new copy only when the plugin's computed version differs from the one they have. That version comes from `plugin.json` first, then from the marketplace entry, per [Versions and updates](loading.md#versions-and-updates).

A plugin that users [load in place](loading.md#find-plugins-on-disk) from a marketplace they added as a local directory isn't controlled by `version`. It loads your current files at every session start, whatever its version string says.
...
* **`ref` and `sha` on the plugin entry**: `ref` names a branch or tag and `sha` names a commit for a `github`, `url`, or `git-subdir` source. See [Plugin sources](marketplace-reference.md#plugin-sources).
...
* **`<plugin>--v<version>` tags**: a dependency's version range resolves against these tags. See [Release a plugin that others depend on](dependencies.md#tag-plugin-releases-for-version-resolution).
...
If you change the `command` of a [`command` source](marketplace-reference.md#command-plugin-source), or switch its `mode`, each user has to accept the new command before Claude Code runs it. Claude Code runs only the exact command a user accepted when they installed or last updated the plugin.

After a user's copy of your marketplace picks up the change, that user sees the following:

* **No more background runs**: the [once-per-session run](loading.md#when-a-command-source-re-runs) of the command stops for that user, so the tool's new output doesn't reach them.
...
To assign the channels to user groups instead of letting users choose, an admin gives each group the matching `extraKnownMarketplaces` entry, as described under [Set update policy](org.md#set-update-policy).
...
When you must change a `name`, add a top-level `renames` map to `marketplace.json` so Claude Code migrates existing users instead of reporting [`Plugin "<name>" not found in marketplace`](troubleshooting.md#plugin-not-found-in-marketplace). Do the same when you remove an entry from `plugins`. Automatic migration requires Claude Code v2.1.193 or later.
...
For a marketplace users added from a git repository or URL, a renamed plugin reports [`Plugin "<name>" not cached at <path>`](troubleshooting.md#plugin-not-cached-at) until the user runs `/plugin install code-formatter@your-marketplace` once in a session.
...
To authenticate an [`archive`](marketplace-reference.md#archive-plugin-source) download, such as a download from a private registry, set the HTTP headers Claude Code sends with it. You can set `headers` in either of these places:

* **The marketplace's `url` source**: the `url` source you registered the marketplace from, such as an [`extraKnownMarketplaces`](../settings-reference.md#extraknownmarketplaces) entry.
...
The [marketplace reference](marketplace-reference.md#plugin-entries) lists the `headers` and `headersHelper` entry fields.
...
This entry sets `headersHelper` beside `source`. It also sets [`"strict": false`](marketplace-reference.md#strict-mode), which Claude Code requires of a `marketplace.json` entry that sets `headersHelper`:
...
* **Entry sets a routing or identity header**: Claude Code drops request-routing and client-identity names such as `Host`, `Cookie`, and `X-Forwarded-*` from an entry's `headers` and command output, and keeps authentication names such as `Authorization`. Every `marketplace.json` entry is filtered this way. For an inline plugin entry in settings, see [`extraKnownMarketplaces`](../settings-reference.md#extraknownmarketplaces).
* **Command set in an `--add-dir` directory's settings**: the command is ignored, on a `url` source and on an [inline plugin entry](../settings-reference.md#extraknownmarketplaces) alike, and only that file's `headers` are sent.
...
In a non-interactive shell, pass [`--yes`](cli-reference.md#plugin-install) to accept the command. To accept only the command that a previous `--json` run displayed, pass [`--accept-command`](cli-reference.md#plugin-install) with the `sha256` the run reported.
...
You declare a marketplace `url` source's `headersHelper` in a settings file, such as an [`extraKnownMarketplaces`](../settings-reference.md#extraknownmarketplaces) entry, rather than in the catalog the marketplace publishes. Claude Code therefore doesn't ask the user to accept it on each install or update. Instead, the settings file that declares it decides when Claude Code runs it:
...
| Server-managed settings                                                       | In an interactive session, only after the user approves the delivered settings in the [security approval dialog](../server-managed-settings.md#security-approval-dialogs)                                                                      |
...
For version ranges, the `<plugin>--v<version>` git-tag convention they resolve against, and cross-marketplace trust, see [Plugin dependencies](dependencies.md).

To have Claude Code suggest a plugin when a project matches it, add a `relevance` block to the entry with the signals that identify the project. Users see suggestions from your marketplace only when an admin lists it in `pluginSuggestionMarketplaces`. For the signals and the enablement step, see [Plugin relevance](relevance.md).
...
* **Restrict what else users install**: the marketplace allowlist is a managed setting, `strictKnownMarketplaces`. See [Restrict what users can install](org.md#restrict-what-users-can-install).
* **Install or enable a plugin without the user asking**: no entry field installs a plugin. Managed `enabledPlugins` does that for a fleet; see [Pre-install and require plugins](org.md#pre-install-and-require-plugins).
...
* [Marketplace reference](marketplace-reference.md): `marketplace.json` fields, source types, and validation messages
* [Manage plugins for your organization](org.md): require, restrict, or seed your marketplace across your organization's machines
* [Plugin dependencies](dependencies.md): tag releases so plugins that depend on yours can resolve versions
* [Troubleshoot plugins](troubleshooting.md): the errors your users see when adding or updating from your marketplace
