...
* **Claude Code printed an error**: find it in [Troubleshoot plugins](troubleshooting.md)
...
As an example, this section installs [`commit-commands`](https://github.com/anthropics/claude-plugins-official/tree/main/plugins/commit-commands) from [Anthropic's official marketplace](anthropic-marketplaces.md), which adds commands for committing, pushing, and opening pull requests.
...
A plugin can run hooks and MCP servers, so read the pane before you install. See [Plugin security and trust](security.md).
...
* **Load failed**: `The plugin couldn't be loaded`. Open the **Errors** tab in `/plugin` for the reason, then see [After install: plugin not working](troubleshooting.md#plugin-installed-but-not-working).
...
If `/commit-commands:commit` doesn't appear, see [After install: plugin not working](troubleshooting.md#plugin-installed-but-not-working).
...
To enable, disable, or uninstall later, use **+ > Plugins > Manage plugins**. The plugin browser isn't available in the desktop app's cloud sessions. See [Install plugins in the desktop app](../desktop.md#install-plugins).
...
Your changes apply to open sessions without a restart. See [Manage plugins in VS Code](../vs-code.md#manage-plugins).

**Cloud session**

A [cloud session](../cloud-environments.md), including [the browser at claude.ai/code](../claude-code-on-the-web.md), has no plugin browser and doesn't load the plugins you installed on your own machine or the ones your repository's `.claude/settings.json` turns on. For plugins your organization distributes through managed settings, see [Manage plugins for your organization](org.md).

See [which parts of your setup are also available in a cloud session](../cloud-environments.md#what-carries-over-from-your-setup) for the rest of your setup.
...
* **Project scope**: the plugin is enabled for everyone who works in this repository. The entry goes in `.claude/settings.json`, which you commit. Committing that entry turns the plugin on for your collaborators but doesn't download it to their machines, so each collaborator also runs `claude plugin install <name>@<marketplace> --scope project` once; see [Enabled in project settings but not installed](loading.md#enabled-in-project-settings-but-not-installed).
* **Local scope**: the plugin is enabled for you in this repository only. The entry goes in `.claude/settings.local.json`.

Some plugins are set by their author to start turned off, through the [`defaultEnabled`](manifest-reference.md#defaultenabled) field. Such a plugin is installed but stays off until you turn it on with `claude plugin enable <name>` in your shell, or from the **Installed** tab of `/plugin` in a session.

When the same plugin is set at several scopes, the local setting overrides the project setting, and the project setting overrides the user setting. See [Find where a plugin is enabled](loading.md#find-where-a-plugin-is-enabled) for the full rule.
...
* **Agent SDK**: load plugins through the SDK's plugin option. See [Load plugins in the Agent SDK](../agent-sdk/plugins.md).

If Claude Code reports that a plugin enabled in the repository's `.claude/settings.json` isn't installed, see [Enabled in project settings but not installed](loading.md#enabled-in-project-settings-but-not-installed).

If you're a plugin author testing a copy of your plugin on disk, start Claude Code from your shell with `--plugin-dir` to load it for one session instead of installing it. See [Flags that load a plugin for one session](cli-reference.md#flags-that-load-a-plugin-for-one-session).
...
For sync timing, sign-in requirements, and turning sync off, see [Plugins synced from claude.ai](loading.md#synced-plugins).
...
When the command succeeds, it prints `Successfully added marketplace: <name>`, and the marketplace's plugins appear on the **Discover** tab the next time you open `/plugin`, with no reload needed. If it fails, match the error message in [Troubleshoot plugins](troubleshooting.md#add-a-marketplace).
...
* **GitHub `owner/repo` shorthand**: Claude Code checks whether your SSH key authenticates to `github.com`, then clones over SSH if it does and over HTTPS if it doesn't. Set [`CLAUDE_CODE_PLUGIN_PREFER_HTTPS=1`](../env-vars.md#variables) to skip that check and always clone over HTTPS.

The same credentials apply when you run `/plugin install`, `/plugin marketplace update`, and `claude plugin update`.

On a GitHub Enterprise Server host, see [Plugin marketplaces on GHES](../github-enterprise-server.md#plugin-marketplaces-on-ghes) for the credentials each operation needs.

If your organization registers the marketplace for you through managed settings, you don't add it yourself. See [Pre-install and require plugins](org.md#pre-install-and-require-plugins).

<h3 id="add-from-claude-ai">
  Add a marketplace from claude.ai
</h3>

In terminal sessions where [plugins sync from your claude.ai account](loading.md#synced-plugins), claude.ai can also list plugin marketplaces for you, such as your organization's plugin library and your own claude.ai uploads. You add one of these by its name rather than by a source. Adding a marketplace from claude.ai requires Claude Code v2.1.273 or later.
...
The tab can also show plugins at **Managed** scope. Your organization installed those through [managed settings](../settings.md#settings-files), and you can't enable, disable, or uninstall them here.
...
When you close the `/plugin` panel with pending changes you made in it, Claude Code runs `/reload-plugins` for you to apply them. If the reload would [invalidate the prompt cache](../prompt-caching.md#enabling-or-disabling-a-plugin), it warns and leaves the changes pending instead. Run `/reload-plugins --force` to apply them anyway.

### Manage plugins synced from claude.ai

The **Installed** tab in `/plugin` also lists the [plugins synced from your claude.ai account](loading.md#synced-plugins), with `synced` as their source. Synced plugins appear in terminal sessions on Claude Code v2.1.273 or later.
...
In your shell, run `claude plugin details <name>` for an installed plugin. The `Always-on` line is the number of tokens the plugin adds to every session where it's enabled, and the per-component rows show which skill or agent contributes most. For the full output and what each figure means, see [Measure what a plugin costs](measure.md#measure-what-a-plugin-costs).
...
* **Uninstall**: auto-installed dependencies stay until you run `claude plugin prune` in your shell; see [plugin prune](cli-reference.md#plugin-prune).

If you loaded the plugin with `--plugin-dir` instead, see [Test a plugin and its dependency locally](dependencies.md#test-a-plugin-and-its-dependency-locally).
...
* **On by default**: `claude-plugins-official` and the other [official marketplace names](security.md#official-marketplace-names) except `knowledge-work-plugins` and `first-party-plugins`, plus [marketplaces added from claude.ai](#add-from-claude-ai).
...
For when auto-update runs, which plugins it skips, and the environment variables that turn it off, see [When auto-update runs](loading.md#when-auto-update-runs).
...
For a private marketplace, see [What background auto-update does with credentials](host-marketplace.md#what-background-auto-update-does-with-credentials) for how background auto-updates authenticate over SSH and HTTPS, and [Troubleshoot plugins](troubleshooting.md#add-a-marketplace) for the messages you see when they fail.
...
* [Anthropic's marketplaces](anthropic-marketplaces.md): how the official, community, and demo marketplaces differ and where to browse each one
* [Plugin loading reference](loading.md): why a plugin loaded, didn't load, or didn't change after an update
* [Plugin security and trust](security.md): what to review before you install a plugin from a marketplace you don't know
* [Troubleshoot plugins](troubleshooting.md): install and marketplace error messages with their fixes
* [Create a plugin](create.md): build your own
