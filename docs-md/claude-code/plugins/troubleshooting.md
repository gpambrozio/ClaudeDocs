...
You get this reply in a session that has no terminal to draw the `/plugin` panel in: [non-interactive mode](../headless.md) with `claude -p`, the Agent SDK, the Claude desktop app's Code tab, the VS Code extension panel, and the browser at claude.ai/code.
...
* **Claude desktop app, local or SSH session**: click the **+** button next to the prompt, then **Plugins**, then **Add plugin** to open the [plugin browser](../desktop.md#install-plugins)
* **VS Code extension**: use the **VS Code** tab under [Install a plugin](install.md#install-a-plugin)
* **Claude Code on the web, or a desktop cloud session**: a cloud session has no plugin browser. See the **Cloud session** tab under [Install a plugin](install.md#install-a-plugin) for what a cloud session loads
...
The cause isn't the plugin command. Either Claude Code isn't installed, or its install directory isn't on your `PATH` in this shell. Follow [`command not found: claude` after installation](../troubleshoot-install.md#command-not-found-claude-after-installation), then retry the plugin command.
...
If the name starts with `claudeai-`, the marketplace is hosted on claude.ai, and you add it by name from your shell with `claude plugin marketplace add --claudeai <name>`. See [Add a marketplace from claude.ai](install.md#add-from-claude-ai).

For any other name, an install line names a marketplace but doesn't say where the marketplace is hosted, and Claude Code has no index to look a marketplace name up in. Ask whoever sent the line for the marketplace's source, which is a GitHub `owner/repo`, a git URL, or a path. Then [add the marketplace](install.md#add-a-marketplace) and run the install line again.

A marketplace someone sends you is third-party, so [review the plugin before you install it](security.md#review-a-plugin-before-you-install).
...
The default location is `.claude-plugin/marketplace.json` at the repository root, and the [marketplace reference](marketplace-reference.md) lists the accepted locations.
...
For what background auto-updates can and can't do with your credentials, see [What background auto-update does with credentials](host-marketplace.md#what-background-auto-update-does-with-credentials).
...
Each session, for a marketplace with [auto-update on](loading.md#which-marketplaces-and-plugins-auto-update), Claude Code checks the marketplace's git host for new commits in the background. When that check can't reach the host, it tries to clone the marketplace again, and offline that clone fails too.
...
For a fully offline deployment, pre-populate the plugins directory at image build time with `CLAUDE_CODE_PLUGIN_SEED_DIR` instead, following [Seed containers and CI](org.md#seed-containers-and-ci).
...
* [A policy error](../github-enterprise-server.md#marketplace-add-fails-with-a-policy-error) means your organization restricted marketplace sources and an admin needs to add a `hostPattern` for the host
* [A GitHub access error on claude.ai](../github-enterprise-server.md#marketplace-add-on-claude-ai-fails-with-a-github-access-error) means your own GitHub Enterprise account isn't connected yet
...
The plugin is already available in every project, so there's nothing to add. To change its [scope](install.md), enable or disable it, or configure it, open `/plugin` and go to **Installed**.
...
Update Claude Code, then retry the install. Source types are on the [marketplace reference](marketplace-reference.md).

<h3 id="plugin-archive-integrity-check-failed">
  `Plugin archive integrity check failed`
</h3>

You installed a plugin that's distributed as a zip archive, and Claude Code refused it with this line and `The archive was not installed.` The plugin's marketplace entry uses an [`archive` source](marketplace-reference.md) with a `sha256` pin, and the downloaded file's digest doesn't match the pin.
...
The marketplace is registered under a name that is [reserved for official Anthropic marketplaces](marketplace-reference.md), but its registered source isn't an `anthropics` GitHub repository. Reserved names are re-checked every time a marketplace loads or refreshes, so the marketplace and the plugins installed from it stop loading.
...
Claude Code prints `Successfully added marketplace: claude-plugins-official`, and **Discover** lists its plugins. The [Anthropic marketplaces](anthropic-marketplaces.md) page lists the other marketplaces you can add.
...
You ran `marketplace add`, and the catalog at that source has the same name as a marketplace that a settings file already declares under [`extraKnownMarketplaces`](../settings-reference.md#extraknownmarketplaces) with a different source. Claude Code refuses the add and registers nothing.
...
The table lists each message and its fix. To declare dependencies as an author, see [Plugin dependencies](dependencies.md).
...
Claude Code didn't activate the plugin during the install, either because activating it would [invalidate the prompt cache](../prompt-caching.md#enabling-or-disabling-a-plugin) or because the activation attempt failed.
...
To opt out of a project-enabled plugin on your machine, set the id to `false` in `.claude/settings.local.json`, which has higher precedence than the project file. For the other sources the message can name, see [Disabled in user settings but still loads](loading.md#disabled-in-user-settings-but-still-loads).

If `claude plugin list` instead marks the plugin `required by your org`, no settings file is involved: your organization marks that synced plugin as required on claude.ai, and it loads even if you disabled it earlier. See [Plugins synced from claude.ai](loading.md#synced-plugins).
...
For any other error, run the hook's command yourself from the plugin directory to see the full output, or capture the full stderr with [debug logging](../hooks.md#debug-hooks).
...
Open the [debug log](../hooks.md#debug-hooks), which records which hooks matched. A hook that ran shows up there with its exit code.
...
You installed a [code intelligence plugin](code-intelligence.md) and Claude isn't seeing diagnostics, or the language server is using too much memory or reporting errors that aren't real.
...
You're developing a plugin and loading it with `--plugin-dir` or installing it from a local marketplace. These entries cover the failures you hit while developing a plugin. For the checks to run after each change, see [Test and debug](create.md#test-and-debug).
...
If your script needs backslashes, switch the hook to one of the forms that keep native paths, described under [exec form and shell form](../hooks.md#exec-form-and-shell-form):
...
To measure how often the skill triggers across realistic prompts rather than checking one at a time, write an eval case with a [`tool_used: Skill` grader](../plugin-evals.md#create-your-first-eval-suite) and run it with `claude plugin eval` after each description change.
...
Change to the plugin's root, the directory that holds `.claude-plugin/plugin.json` or the skill's `SKILL.md`, and run the command again. To scaffold the suite somewhere else on purpose, pass `--eval-dir`. See [Test plugins with evals](../plugin-evals.md).
...
`plugin.json` fields are on the [manifest reference](manifest-reference.md), and marketplace-level messages are under [Marketplace validation errors](#marketplace-validation-errors).
...
The plugin has its own `plugin.json`, and its marketplace entry sets `strict: false` while also declaring any of `commands`, `agents`, `skills`, `hooks`, `outputStyles`, or `themes`. Remove those fields from the entry, or set `strict: true` in the entry so Claude Code appends them to `plugin.json`. See [Strict mode](marketplace-reference.md#strict-mode).
...
Alternatively, host the marketplace in a git repository and tell users to add it with the repository URL. For a git source, Claude Code clones the whole repository, so relative paths resolve. Source types are on the [marketplace reference](marketplace-reference.md).
...
Your organization deploys managed settings that restrict plugins, and a command was refused with a policy message. These entries name the setting behind each refusal so you know what to ask your administrator for. For the admin side, see [Manage plugins for your organization](org.md).
...
To change the marketplace's content, ask the person who maintains the seed image to update it. For the procedure, see [Seed containers and CI](org.md#seed-containers-and-ci).

## Next steps

* [Plugin loading reference](loading.md): why scopes, the cache, and precedence behave the way they do
* [Plugin commands reference](cli-reference.md): flags, defaults, output, and exit codes for the `claude plugin` commands
* [Install and manage plugins](install.md): the install steps from the start
* [Manage plugins for your organization](org.md#troubleshoot-policy): policy-side troubleshooting for administrators
