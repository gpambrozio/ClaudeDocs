...
Pass an [effort level](model-config.md) to trade coverage for confidence. At `low` and `medium`, the review reports only the findings it’s most confident in, so you see fewer false positives; `high` through `max` cast a wider net and may include findings the review is less sure about. Without an effort argument, the review uses the session’s current effort.
...
You can’t schedule the review: `/code-review` is marked [`disable-model-invocation`](skills.md), so if you set it as a [scheduled task](scheduled-tasks.md)’s prompt, Claude reads it as plain text instead of running the review.
