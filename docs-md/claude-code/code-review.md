...
Lower [effort levels](model-config.md) return fewer, higher-confidence findings, while `high` through `max` give broader coverage and may include uncertain findings. Without an effort argument, the review uses the session’s current effort. Pass a path or PR reference to review a specific target instead of the current diff.
