...
For managed MCP servers, which you deploy alongside any of these through `managed-mcp.json`, see [Managed MCP configuration](managed-mcp.md).
...
* **Running sessions**: a session picks up most changes on the schedule in the table without a restart. Claude Code reads [`forceRemoteSettingsRefresh`](settings-reference.md#forceremotesettingsrefresh) and [`requiredMinimumVersion`](settings-reference.md#requiredminimumversion) only at session start, arms a new or changed [`policyHelper`](settings-reference.md#policyhelper) entry at the next launch, and reads [some user-editable keys once at session start](settings.md#when-edits-take-effect).
...
* **[`extraKnownMarketplaces`](settings-reference.md#extraknownmarketplaces)**: a later entry with the same name replaces the earlier one whole
...
This table shows how Claude Code combines each kind of key under `"merge"`; the [`managedSourcesBehavior` entry](settings-reference.md#managedsourcesbehavior) names every key in the restriction allowlist and highest-source-only rows.
...
* **The emitted `managedSettings` object is the only managed settings for the session**, including for the [keys it otherwise reads from every admin source](#keys-read-from-every-admin-source), apart from `forceRemoteSettingsRefresh`, which Claude Code checks in every admin source at startup before the helper runs. For which helper runs fail, and what Claude Code does when one does, see [Helper failures](settings-reference.md#helper-failures)

Claude Code selects the source at startup, and that selection decides whether a helper runs. The [`policyHelper`](settings-reference.md#policyhelper) entry says which sources can configure a helper.
