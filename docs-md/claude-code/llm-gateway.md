...
If your gateway uses model names that do not match the discovery filter, use the environment variables documented in [Model configuration](model-config.md) to add them manually.
...
Route to a gateway that forwards to the [Claude Platform on AWS](claude-platform-on-aws.md) endpoint:
