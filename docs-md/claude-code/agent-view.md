...
An effort the session took from the [`effortLevel` setting](settings.md) rather than from `--effort` or `/effort` isn’t fixed at dispatch: each process started for the session reads the setting again, so editing `effortLevel` in `settings.json` reaches sessions you background with `←` or `/bg` and their later restarts.
A name you set with [`/rename`](commands.md) or `Ctrl+R` also persists across that restart, so [`claude --resume <name>`](sessions.md) still resolves the session.
