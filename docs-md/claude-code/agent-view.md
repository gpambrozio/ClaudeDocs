...
| Needs input | Yellow | Claude is waiting on something only you can provide: an answer to a question, a permission decision, a [sandbox](sandboxing.md) prompt to allow a network host, an MCP server’s [request for input](mcp.md), a managed-settings prompt, or an MCP authentication or settings request, or an `/install-github-app` request, held by a session with no terminal attached |
...
- When the current session is itself running inside a linked [worktree](worktrees.md) that has a main working tree to return to, the copy runs in that main working tree, so the two sessions don’t edit the same checkout. The confirmation ends with `runs in the origin tree`.
