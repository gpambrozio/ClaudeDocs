...
Each refresh is one short Haiku-class request through your normal provider, billed and handled under the same [data usage terms](data-usage.md) as the session itself. On third-party providers such as Amazon Bedrock, Google Cloud’s Agent Platform, Microsoft Foundry, and custom gateways, the request falls back to the session’s main model when no Haiku model is configured. Set [`ANTHROPIC_DEFAULT_HAIKU_MODEL`](model-config.md) to choose the model for these summaries on those providers.
