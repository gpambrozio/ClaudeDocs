...
* **Installing a plugin that has dependencies**: see [Manage installed plugins](plugins/install.md#manage-installed-plugins)
* **Reading a dependency error**: see [Dependency errors](plugins/troubleshooting.md#dependency-errors)
* **Declaring the npm and Bun packages that your plugin's own code needs**: see [Node.js package dependencies](plugins/loading.md#node-js-package-dependencies)
...
To add a plugin to the standard set later, publish a new `backend-standard` version with the extra dependency. When the marketplace doesn't [auto-update by default](plugins/loading.md#which-marketplaces-and-plugins-auto-update), engineers either turn on auto-update for the marketplace or update manually:
...
For the engineer-side steps, see [Keep plugins updated](plugins/install.md#keep-plugins-updated).

To deploy a bundle to everyone in an organization, an administrator adds it to `enabledPlugins` in managed settings. See [Pre-install and require plugins](plugins/org.md#pre-install-and-require-plugins).
...
If you're developing a plugin and the plugin it depends on at the same time, start Claude Code from your shell and load both with [`--plugin-dir`](plugins/cli-reference.md#flags-that-load-a-plugin-for-one-session):
...
Create the tag from the plugin directory, with an `origin` remote configured to receive the pushed tag, using [`claude plugin tag`](plugins/cli-reference.md#plugin-tag):
...
The [`claude plugin tag` reference](plugins/cli-reference.md#plugin-tag) lists the remaining flags.
...
Tag-based resolution applies only to git-backed sources. For a dependency with an `npm`, `archive`, or `command` [plugin source](plugins/marketplace-reference.md#plugin-sources), the constraint doesn't control which version is fetched. It's still checked when the plugin loads, and the dependent plugin is disabled if the installed version doesn't satisfy it.
...
Claude Code never installs a dependency with a `command` source itself, so users [install it first](plugins/marketplace-reference.md#command-plugin-source). It also never runs a dependency's [`headersHelper`](plugins/host-marketplace.md#authenticate-archive-downloads), so users also install a dependency whose marketplace entry sets one before they install your plugin.
...
If you force-move a tag to a different commit, the next install fetches that commit's content instead of reusing a stale cached copy. See [Versions and updates](plugins/loading.md#versions-and-updates) for how a plugin's version becomes its cache key.
...
* [`claude plugin prune`](plugins/cli-reference.md#plugin-prune): remove auto-installed dependencies no plugin needs anymore
* [Host a marketplace](plugins/host-marketplace.md): release channels and recommending other plugins
