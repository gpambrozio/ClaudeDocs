...
| `chat:sendNow`        | Ctrl+Enter, Ctrl+X Ctrl+S         | Interrupt the running turn so your [queued messages](interactive-mode.md#queue-messages-while-claude-works), and your draft with them, go out right away. When nothing is running, it submits the draft, and in [shell mode](interactive-mode.md#shell-mode-with-prefix) it queues the command without interrupting. Terminals that don't report extended keys deliver `Ctrl+Enter` as plain `Enter`, so `Ctrl+X Ctrl+S` is the binding that works in any terminal. Requires v2.1.275 or later |
