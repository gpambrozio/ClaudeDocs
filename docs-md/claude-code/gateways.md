...
Claude apps gateway is Anthropic’s self-hosted gateway, included in the `claude` binary. It routes to Amazon Bedrock, Google Cloud, Microsoft Foundry, or the Anthropic API as the upstream. Developers sign in with your corporate identity provider through `/login`, the gateway enforces model access and [managed settings](permissions.md) by IdP group, and it emits [OpenTelemetry Protocol (OTLP)](monitoring-usage.md) usage metrics to your own observability stack.
...
- **Other network traffic**: Claude Code itself sends version checks and downloads directly to Anthropic, separate from the gateway path. Whether the optional client telemetry stream is also on depends on your provider; the [telemetry defaults table](data-usage.md) covers each case. On a signed-in Claude apps gateway session, the gateway credential disables the Anthropic-bound analytics and, when [telemetry forwarding](claude-apps-gateway-config.md) is configured, pins OTLP export to the gateway. Your network still needs egress to the [required domains](network-config.md), or set [`CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC`](env-vars.md) to turn off the optional streams.
- **Corporate HTTP proxies**: an `HTTPS_PROXY` sits between Claude Code and every server it talks to, including the gateway. If your network requires one, [configure the proxy](network-config.md) in addition to the gateway. For Claude apps gateway specifically, [sign-in checks that the proxy host is also on a private network](claude-apps-gateway.md); if it isn’t, add the gateway host to `NO_PROXY` so the CLI connects to it directly.
