...
- **Client telemetry**: on a Claude apps gateway session, Claude Code sends no client analytics to Anthropic. It disables the Anthropic-bound analytics when the session signs in. For other gateways, whether the optional client telemetry stream is on depends on your provider, and the [telemetry defaults table](data-usage.md) covers each case. Where a gateway session’s telemetry goes depends on how the session signed in, and [What’s enforced on developers](claude-apps-gateway.md) says where each kind of session’s exports go.
