...
By the end you’ll have an environment on the [**Cloud environments** admin page](https://claude.ai/admin-settings/cloud-environments), a runner polling for work, and a session running on your host. Before you connect real repositories or internal systems, work through [Deploy to production](self-hosted-environments-deploy.md), which covers the security posture, egress control, git credentials, and orchestration.

## [​](#prerequisites) Prerequisites

### [​](#organization-and-roles) Organization and roles

The claude.ai side needs:

- **Allow self-hosted environments** turned on by an [Owner or admin](cloud-environments.md) on the [**Cloud environments** admin page](https://claude.ai/admin-settings/cloud-environments); the **New** button doesn’t appear until it is. If you don’t hold the role, someone who does can create the environment and hand you its secret; the runner and terminal steps on this page need no claude.ai role, and where a step checks status in the admin UI, the runner’s own log lines give you the same signal.
...
### [​](#host-and-network) Host and network
...
### [​](#software-on-the-runner-host) Software on the runner host
...
## [​](#set-up-an-environment-and-runner) Set up an environment and runner
...
Choose a base directory, replacing `<writable-dir>` in the runner command below with an absolute path that the runner can write to or create. The runner creates the directory at startup, then checks repositories out and creates per-session directories under it. Without `--base-dir` it uses `/workspace`, which only works if that directory already exists and is writable or you start the runner as root.If the runner can’t create or write to the path, it exits at startup with an error naming the directory instead of registering. See [Troubleshooting](self-hosted-environments-deploy.md).Then start the runner with `--environment-secret-file` and `--base-dir`. The runner registers with your environment and begins polling for work. If the runner exits, restart it by hand. Production deployments run the runner under an orchestrator that restarts exited runners, normally with a fresh filesystem per restart; [Reuse a pre-warmed checkout](self-hosted-environments-deploy.md) covers the supported persistent-disk setup.
...
## [​](#send-a-follow-up-message-to-a-running-session) Send a follow-up message to a running session
...
- [Deploy to production](self-hosted-environments-deploy.md): harden the deployment, control egress, configure git credentials, and run the fleet under Kubernetes or Compose
...
- [Test end to end](self-hosted-environments-testing.md): a CI smoke test that dispatches a session and reads Claude’s replies
