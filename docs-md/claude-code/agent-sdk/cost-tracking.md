...
Use these fields for development insight and approximate budgeting. For authoritative billing, use the [Usage and Cost API](build-with-claude/usage-cost-api.md) or the Usage page in the [Claude Console](https://platform.claude.com/usage). Do not bill end users or trigger financial decisions from these fields.
...
When Claude responds, it sends one or more assistant messages. In TypeScript, each assistant message contains a nested `BetaMessage` (accessed via `message.message`) with an `id` and a [`usage`](api/messages.md) object with token counts (`input_tokens`, `output_tokens`). In Python, the `AssistantMessage` dataclass exposes the same data directly via `message.usage` and `message.message_id`. When Claude uses multiple tools in one turn, all messages in that turn share the same ID, so deduplicate by ID to avoid double-counting.

2

The result message provides the cumulative estimate

When the `query()` call completes, the SDK emits a result message with `total_cost_usd` and cumulative `usage`, typed as [`SDKResultMessage`](agent-sdk/typescript.md) in TypeScript and [`ResultMessage`](agent-sdk/python.md) in Python. If you make multiple `query()` calls, for example in a multi-turn session, each result reflects only the cost of that individual call. If you only need the estimated total, you can ignore the per-step usage and read this single value.In streaming input mode, each turn emits its own result message. See [Track costs in streaming input mode](#track-costs-in-streaming-input-mode) for how to read call totals in that mode.

## [​](#track-costs-in-streaming-input-mode) Track costs in streaming input mode

In [streaming input mode](agent-sdk/streaming-vs-single-mode.md), one `query()` call carries multiple user turns and each turn emits its own result message. The result fields differ in scope:
...
In TypeScript, the SDK also emits an [`SDKConversationResetMessage`](agent-sdk/typescript.md) at each reset, so you can detect resets from the stream. The Python SDK drops that message before it reaches your app, so in Python, count the resets yourself from the `/clear` turns your app sends.
...
The result message, typed as [`SDKResultMessage`](agent-sdk/typescript.md) in TypeScript and [`ResultMessage`](agent-sdk/python.md) in Python, marks the end of the agent loop for a `query()` call. It includes `total_cost_usd`, the cumulative estimated cost across all steps in that call. In Python the field is typed as optional, so check that it isn’t `None` before you read it. Success and error results both carry it, though the final result of a [session crash](#recover-totals-after-a-session-crash) may carry it zeroed.
...
## [​](#handle-errors-caching-and-output-token-counts) Handle errors, caching, and output token counts
...
### [​](#read-output-tokens-from-the-result-message) Read output tokens from the result message
...
To watch a response’s output count grow while it streams, set `includePartialMessages`, or `include_partial_messages` in Python, and read `usage` from each `message_delta` stream event, typed as [`SDKPartialAssistantMessage`](agent-sdk/typescript.md) in TypeScript and [`StreamEvent`](agent-sdk/python.md) in Python.
...
### [​](#recover-totals-after-a-session-crash) Recover totals after a session crash
...
The Agent SDK automatically uses [prompt caching](build-with-claude/prompt-caching.md) to reduce costs on repeated content. You do not need to configure caching yourself. The usage object includes two additional fields for cache tracking:
...
Track these separately from `input_tokens` to understand caching savings. In TypeScript, these fields are typed on the [`Usage`](agent-sdk/typescript.md) object. In Python, they appear as keys in the [`ResultMessage.usage`](agent-sdk/python.md) dict (for example, `message.usage.get("cache_read_input_tokens", 0)`).
