...
- Set `alwaysLoad: true` on a server’s config to make its tools available at their full schemas on the first turn, [exempt from tool search deferral](mcp.md). Claude Code blocks startup on that server’s connection, capped at the same deadline, while other servers keep connecting in the background. The `alwaysLoad` field requires Claude Code v2.1.121 or later.
