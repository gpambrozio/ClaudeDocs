...
- Set `alwaysLoad: true` on a server’s config to make its tools available at their full schemas on the first turn, [exempt from tool search deferral](mcp.md). Claude Code waits at startup for that server’s tools, capped at the same deadline, while other servers keep connecting in the background; a remote server with a cached tool list supplies them without connecting, per the table above. The `alwaysLoad` field requires Claude Code v2.1.121 or later.
...
### [​](#auto-approve-with-allowedtools) Auto-approve with allowedTools
...
An SDK MCP server registered by an [`initialize` control request](agent-sdk/typescript.md) begins connecting as soon as Claude Code processes the request.
...
Tool search is enabled by default. See [Tool search](agent-sdk/tool-search.md) for configuration options, best practices, and using tool search with custom SDK tools.
...
MCP server connections time out after 30 seconds by default. If your server takes longer to start, the connection fails. Raise the limit with the [`MCP_TIMEOUT`](env-vars.md) environment variable, in milliseconds. For servers that need more startup time, also consider:
