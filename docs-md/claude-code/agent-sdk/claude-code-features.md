...
Each source loads settings from a specific location, where `<cwd>` is the working directory you pass via the `cwd` option, or the process's current directory if unset. For the full type definition, see [`SettingSource`](typescript.md#settingsource) (TypeScript) or [`SettingSource`](python.md#settingsource) (Python).

| Source      | What it loads                                                                                                          | Location                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| :---------- | :--------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `"project"` | Project `settings.json` and hooks; project CLAUDE.md and `.claude/rules/*.md`; project skills, commands, and subagents | `<cwd>/.claude/` for `settings.json` and hooks; `<cwd>` and every parent directory for CLAUDE.md and rules; `<cwd>` and every parent directory up to the repository root for skills, commands, and subagents, plus the `.claude/skills/`, `.claude/commands/`, and `.claude/agents/` folders of each directory you pass through the `additionalDirectories` or `add_dirs` option, which the SDK passes to Claude Code as [`--add-dir`](../permissions.md#additional-directories-grant-file-access-not-configuration) |
...
| :------------------------------------------------------------------------------------------------------------------------------ | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Managed policy settings                                                                                                         | Endpoint-managed policy, such as an MDM plist, registry policy, or managed settings file, loads from the host. [Server-managed settings](../server-managed-settings.md) are fetched on an [eligible configuration](../server-managed-settings.md#platform-availability) when the session authenticates with a qualifying credential, such as an organization OAuth login, a directly configured API key, or a `user_oauth` [Anthropic profile](../authentication.md#anthropic-profiles-and-federation-credentials) | Endpoint policy: remove the managed settings file, plist, or registry policy from the host. Server-managed settings: an [Owner](../server-managed-settings.md#access-control) in your Claude organization controls them; you can't disable them from the SDK |
...
| [claude.ai MCP connectors](../mcp.md#use-mcp-servers-from-claude-ai)                                                              | Loaded when the session authenticates with your claude.ai login. Not loaded when `CLAUDE_CODE_OAUTH_TOKEN` holds a token from [`claude setup-token`](../authentication.md#generate-a-long-lived-token), which can only make model requests. Passing `mcpServers: {}` does not suppress the connectors                                                                                                                                                                                                          | Set `strictMcpConfig: true`, [`disableClaudeAiConnectors: true`](../mcp.md#disable-claude-ai-connectors) in settings, or `ENABLE_CLAUDEAI_MCP_SERVERS=false` in `env`                                                                                        |
| [`sandbox.credentials`](../sandboxing.md#protect-credentials) `deny` entries and file `mask` entries in `~/.claude/settings.json` | When the [command sandbox](../sandboxing.md) runs, Claude Code applies the `deny` entries and keeps the `credentials.files` `mask` entries as restrictions even when `settingSources` excludes user settings. Claude Code uses these entries only to narrow what sandboxed commands can access                                                                                                                                                                                                                 | Remove the entries from `~/.claude/settings.json`                                                                                                                                                                                                          |

Do not rely on default `query()` options for multi-tenant isolation. Because the inputs above are read regardless of `settingSources`, an SDK process can pick up host-level configuration and per-directory memory. For multi-tenant deployments, run each tenant in its own filesystem and set `settingSources: []` plus `CLAUDE_CODE_DISABLE_AUTO_MEMORY=1` in `env`. [Server-managed settings](../server-managed-settings.md) are fetched when the process authenticates with an organization credential; filesystem isolation does not remove them. See [Secure deployment](secure-deployment.md).
...
You can also inject context directly via `systemPrompt` without using CLAUDE.md files. See [Modify system prompts](modifying-system-prompts.md). Use CLAUDE.md when you want the same context shared between interactive Claude Code sessions and your SDK agents.

For how to structure and organize CLAUDE.md content, see [Manage Claude's memory](../memory.md).
...
Skills must be created as filesystem artifacts (`.claude/skills/<name>/SKILL.md`). The SDK does not have a programmatic API for registering skills. See [Agent Skills in the SDK](skills.md) for full details.

## Hooks

The SDK supports two ways to define hooks, and they run side by side:

* **Filesystem hooks:** shell commands defined in `settings.json`, loaded when `settingSources` includes the relevant source. These are the same hooks you'd configure for [interactive Claude Code sessions](../hooks-guide.md).
* **Programmatic hooks:** callback functions passed directly to `query()`. These run in your application process and can return structured decisions. See [Control execution with hooks](hooks.md).
...
The TypeScript SDK supports additional hook events beyond Python, including `SessionStart`, `SessionEnd`, `TeammateIdle`, and `TaskCompleted`. See the [hooks guide](hooks.md) for the full event compatibility table.

For full details on programmatic hooks, see [Control execution with hooks](hooks.md). For filesystem hook syntax, see [Hooks](../hooks.md).
...
| :------------------------------------------------------------------------------------------------ | :-------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Set project conventions your agent always follows                                                 | [CLAUDE.md](../memory.md)                       | `settingSources: ["project"]` loads it automatically                                                                                                           |
| Give the agent reference material it loads when relevant                                          | [Skills](skills.md)                | `settingSources` + `skills` option                                                                                                                             |
| Run a reusable workflow (deploy, review, release)                                                 | [User-invocable skills](skills.md) | `settingSources` + `skills` option                                                                                                                             |
| Delegate an isolated subtask to a fresh context (research, review)                                | [Subagents](subagents.md)          | `agents` parameter + `allowedTools: ["Agent"]`                                                                                                                 |
...
| Run deterministic logic on tool calls (audit, block, transform)                                   | [Hooks](hooks.md)                  | `hooks` parameter with callbacks, or shell scripts loaded via `settingSources`                                                                                 |
| Give Claude structured tool access to an external service                                         | [MCP](mcp.md)                      | `mcpServers` parameter                                                                                                                                         |

Every feature you enable adds to your agent's context window. For per-feature costs and how these features layer together, see [Extend Claude Code](../features-overview.md#understand-context-costs).

## Related resources

* [Extend Claude Code](../features-overview.md): Conceptual overview of all extension features, with comparison tables and context cost analysis
* [Skills in the SDK](skills.md): Full guide to using skills programmatically
* [Subagents](subagents.md): Define and invoke subagents for isolated subtasks
* [Hooks](hooks.md): Intercept and control agent behavior at key execution points
* [Permissions](permissions.md): Control tool access with modes, rules, and callbacks
* [System prompts](modifying-system-prompts.md): Inject context without CLAUDE.md files
