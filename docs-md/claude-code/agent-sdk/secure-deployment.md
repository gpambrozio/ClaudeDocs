...
Agents can take unintended actions due to prompt injection (instructions embedded in content they process) or model error. Claude models are designed to resist this; see the [model overview](../../api/models/overview.md) and the system card for the model you deploy for evaluation details.
