...
| `can_use_tool` | [`CanUseTool`](#canusetool)  `| None` | `None` | Tool permission callback, invoked only when the [permission flow](agent-sdk/permissions.md) falls through to a prompt. Not invoked for calls auto-approved by `allowed_tools`, allow rules, or `permission_mode`. See [`CanUseTool`](#canusetool) for details |
