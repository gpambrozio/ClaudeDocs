...
| `"dontAsk"` | Never prompts. Tools pre-approved by [permission rules](settings.md) run; everything else is denied. `AskUserQuestion`, connector tools [your organization set to `ask`](mcp.md), and MCP tools marked [`requiresUserInteraction`](mcp.md) are denied even if you’ve allowed them |
| `"auto"` | Uses a model classifier to approve or deny permission prompts. See [Auto mode](permission-modes.md) for availability and behavior |
| `"bypassPermissions"` | Runs all allowed tools without asking, except tools matched by an explicit [`ask` rule](settings.md), connector tools [your organization set to `ask`](mcp.md), and tools that require user interaction; see [How permissions are evaluated](agent-sdk/permissions.md) for the precedence order. Cannot be used when running as root on Unix. Use only in isolated environments where the agent’s actions cannot affect systems you care about |
