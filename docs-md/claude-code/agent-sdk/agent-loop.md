...
3. **Execute tools.** The SDK runs each requested tool and collects the results. Each set of tool results feeds back to Claude for the next decision. You can use [hooks](agent-sdk/hooks.md) to intercept, modify, or block tool calls before they run.
...
* **`SystemMessage`:** session lifecycle events. The `subtype` field distinguishes them:
  ...
  * `"worker_shutting_down"`: the loop will end after the current turn because the host is exiting or Remote Control disconnected
  ...
  In TypeScript, each subtype other than `"init"` is its own type in the [`SDKMessage` union](agent-sdk/typescript.md) rather than a subtype of `SDKSystemMessage`.
...
* **Live streaming:** enable partial messages (`include_partial_messages` in Python, `includePartialMessages` in TypeScript) to get `StreamEvent` messages in real time. See [Stream responses in real-time](agent-sdk/streaming-output.md).
...
* **Define custom tools** with [custom tool handlers](agent-sdk/custom-tools.md)
* **Load project skills** via [setting sources](agent-sdk/claude-code-features.md) for reusable workflows
...
You can limit how many turns the loop takes, how much it costs, how deeply Claude reasons, and whether tools require approval before running. All of these are fields on [`ClaudeAgentOptions`](agent-sdk/python.md) (Python) / [`Options`](agent-sdk/typescript.md) (TypeScript).
...
When either limit is hit, the SDK returns a `ResultMessage` with a corresponding error subtype (`error_max_turns` or `error_max_budget_usd`). See [Handle the result](#handle-the-result) for how to check these subtypes and [`ClaudeAgentOptions`](agent-sdk/python.md) / [`Options`](agent-sdk/typescript.md) for syntax.

The budget cap covers [subagents](agent-sdk/subagents.md): their spend counts toward the total. Once spend reaches the cap, spawning another subagent fails with `Budget limit reached`, and Claude Code stops any background subagents still running. The cap-enforcement behaviors require Claude Code v2.1.217 or later.

With [streaming input](agent-sdk/streaming-vs-single-mode.md), a message that is still queued when a turn ends at the max-turns limit stays queued. Claude Code doesn't add it to that turn's last model call. It starts a new turn for the message, and the max-turns count starts over for that turn.

### Effort level

The `effort` option controls how much reasoning Claude applies. Lower effort levels use fewer tokens per turn and reduce cost. Not all models support the effort parameter. See [Effort](build-with-claude/effort.md) for which models support it.
...
| `"xhigh"`  | Extended reasoning depth          | Coding and agentic tasks on the [models that support it](model-config.md) |
...
`effort` trades latency and token cost for reasoning depth within each response. [Extended thinking](build-with-claude/extended-thinking.md) is a separate feature that produces `thinking` blocks in the output, and the `display` field on `ThinkingConfig` for [Python](agent-sdk/python.md) or [TypeScript](agent-sdk/typescript.md) controls whether you receive their text. They are independent: you can set `effort: "low"` with extended thinking enabled, or `effort: "max"` without it.
...
| `"dontAsk"`           | Never prompts. Tools pre-approved by [permission rules](settings-reference.md) run; everything else is denied. `AskUserQuestion`, connector tools [your organization set to `ask`](mcp.md), and MCP tools marked [`requiresUserInteraction`](mcp.md) are denied even if you've allowed them                                                                                                                                                                                                                                                                                                                                                                        | You want a fixed, explicit tool surface for a headless agent and prefer a hard deny over silent reliance on `canUseTool` being absent         |
| `"auto"`              | Uses a model classifier to approve or deny permission prompts. See [Auto mode](permission-modes.md) for availability and behavior                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Autonomous agents that still want safety guardrails on tool use                                                                               |
| `"bypassPermissions"` | Runs all allowed tools without asking, except tools matched by an explicit [`ask` rule](settings-reference.md), connector tools [your organization set to `ask`](mcp.md), and tools that require user interaction. The [cross-session messaging safeguards](permission-modes.md) still apply. See [How permissions are evaluated](agent-sdk/permissions.md) for the precedence order. In the TypeScript SDK, also requires `allowDangerouslySkipPermissions: true` in `options`. Can't be used when running as root on Unix. Use only in isolated environments where the agent's actions can't affect systems you care about | CI, containers, or other isolated environments                                                                                                |
...
Compaction replaces older messages with a summary, so specific instructions from early in the conversation may not be preserved. Persistent rules belong in CLAUDE.md (loaded via [`settingSources`](agent-sdk/claude-code-features.md)) rather than in the initial prompt, because CLAUDE.md content is re-injected on every request.
...
* **Manual compaction:** Send `/compact` as a prompt string to trigger compaction on demand. Commands sent this way are ordinary SDK inputs. See [dispatch commands by name](agent-sdk/skills.md).
...
* After a session crash, the final result is an `error_during_execution` whose cost fields may be zeroed and whose `stop_reason` is `null`, and the process exits after emitting it. See [Recover totals after a session crash](agent-sdk/cost-tracking.md).
* In Python, `total_cost_usd`, `usage`, and `model_usage` are typed as optional, so check that they aren't `None` before you read them.

The `usage` field covers only the main agent loop. Use `modelUsage`, or `model_usage` in Python, for whole-tree token and cost accounting. See [Tracking costs and usage](agent-sdk/cost-tracking.md) for details on interpreting the `usage` fields.
...
To detect refusals, check `stop_reason === "refusal"` (TypeScript) or `stop_reason == "refusal"` (Python). See [`SDKResultMessage`](agent-sdk/typescript.md) (TypeScript) or [`ResultMessage`](agent-sdk/python.md) (Python) for the full type.

## Hooks

[Hooks](agent-sdk/hooks.md) are callbacks that fire at specific points in the loop: before a tool runs, after it returns, when the agent finishes, and so on. Some commonly used hooks are:
...
Both SDKs support all the events above. The TypeScript SDK includes additional events that Python does not yet support. See [Control execution with hooks](agent-sdk/hooks.md) for the complete event list, per-SDK availability, and the full callback API.
...
* **Haven't run an agent yet?** Start with the [quickstart](agent-sdk/quickstart.md) to get the SDK installed and see a full example running end to end.
* **Ready to hook into your project?** [Load CLAUDE.md, skills, and filesystem hooks](agent-sdk/claude-code-features.md) so the agent follows your project conventions automatically.
