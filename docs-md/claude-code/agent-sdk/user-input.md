...
In Python, `can_use_tool` requires [streaming mode](agent-sdk/streaming-vs-single-mode.md) and a `PreToolUse` hook that returns `{"continue_": True}` to keep the stream open. Without this hook, the stream closes before the permission callback can be invoked.
