...
- **`claude_code.hook`:** wraps each [hook](agent-sdk/hooks.md) execution.

Every span carries a `session.id` attribute. When you make several `query()` calls against the same [session](agent-sdk/sessions.md), filter on `session.id` in your backend to see them as one timeline.
