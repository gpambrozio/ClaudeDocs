...
Continue, resume, and fork are option fields you set on `query()` ([`ClaudeAgentOptions`](agent-sdk/python.md) in Python, [`Options`](agent-sdk/typescript.md) in TypeScript).
...
See the [Python SDK reference](agent-sdk/python.md) for details on when to use `ClaudeSDKClient` vs the standalone `query()` function.
...
* **Recover from a limit.** The first run ended with `error_max_turns` or `error_max_budget_usd` (see [Handle the result](agent-sdk/agent-loop.md)); resume with a higher limit. In a single-shot `query()` call the SDK raises after yielding that error result, so catch the error before resuming.
...
To find your session's directory, replace every non-alphanumeric character in the absolute working directory with `-`: `/Users/me/proj` becomes `-Users-me-proj`. For a working directory whose converted name exceeds 200 characters, Claude Code [truncates the name and appends a hash](sessions.md), so match the first 200 characters of the converted name when you list `projects/`.

If you set [`CLAUDE_CODE_PROJECT_DIR_NAME`](sessions.md) beside `CLAUDE_CONFIG_DIR`, look under that name in `projects/` instead. Requires TypeScript Agent SDK v0.3.234 or later, or Python Agent SDK v0.2.140 or later.
...
Both SDKs also expose functions for looking up and mutating individual sessions: [`get_session_info()`](agent-sdk/python.md), [`rename_session()`](agent-sdk/python.md), and [`tag_session()`](agent-sdk/python.md) in Python, and [`getSessionInfo()`](agent-sdk/typescript.md), [`renameSession()`](agent-sdk/typescript.md), and [`tagSession()`](agent-sdk/typescript.md) in TypeScript. Use them to organize sessions by tag or give them human-readable titles.
