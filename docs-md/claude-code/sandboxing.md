...
Each entry carries `"mode": "deny"`, which is the only supported value. The explicit `mode` field keeps the schema forward-compatible with future modes. File paths follow the same [prefix rules](settings.md) as `sandbox.filesystem.*` settings, and entries from every [settings scope](settings.md) are merged. Because the only mode is `deny`, any scope can add restrictions but none can remove them.
...
- **Environment variables**: sandboxed Bash commands inherit the parent process environment by default, including any credentials set there. Use [`sandbox.credentials`](#protect-credentials) to unset specific variables for sandboxed commands, or set [`CLAUDE_CODE_SUBPROCESS_ENV_SCRUB`](env-vars.md) to strip Anthropic and cloud provider credentials from all subprocesses.
