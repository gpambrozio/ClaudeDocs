...
Developers can log in from several paths: the terminal `/login` flow, the [VS Code extension](vs-code.md), the Agent SDK, `claude setup-token`, `/install-github-app`, and [gateway](claude-apps-gateway.md) sign-in for organizations that route through a cloud gateway. On Claude Code v2.1.212 or later, every path applies `forceLoginMethod`; before v2.1.212, only terminal logins applied either key. In the interactive `/login` flow, Claude Code pre-selects a `claudeai` or `console` method without enforcing it, so even with `forceLoginMethod` set to `"claudeai"`, a developer can still complete a Console login there. The paths differ on `forceLoginOrgUUID`:
...
The keys also decide whether a session that doesn’t use a login credential can start. See [`forceLoginOrgUUID`](settings.md) in the settings reference for the full behavior.
...
- **Custom credential scripts**: configure the [`apiKeyHelper`](settings.md) setting to run a shell script that returns an API key.
...
4. [`apiKeyHelper`](settings.md) script output. Use this for dynamic or rotating credentials, such as short-lived tokens fetched from a vault.
