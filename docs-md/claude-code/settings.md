...
| `remote.defaultEnvironmentId` | Default [cloud environment](cloud-environments.md) for cloud sessions you create from the CLI, such as with `claude --cloud` or [ultraplan](ultraplan.md). Written to user settings when you pick an environment with [`/remote-env`](cloud-environments.md). Follows the standard settings precedence, so a value in a repo’s project settings overrides the user-level pick | `"env_0123abcd"` |
