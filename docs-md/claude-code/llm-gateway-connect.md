...
An [LLM gateway](llm-gateway.md) is a proxy your organization runs between Claude Code and the model provider. When your organization uses one, Claude Code authenticates to the gateway with a credential your organization issues instead of your personal claude.ai login.
...
Administrators can distribute the gateway address and credential through [managed settings](managed-settings.md), device management, or an [`apiKeyHelper`](#rotate-credentials-with-apikeyhelper), so Claude Code picks them up at startup with nothing for you to set. To check whether your organization already did this:
...
If you export the gateway only in your shell, it doesn't reliably reach background agents hosted by the [supervisor](agent-view.md#how-background-sessions-are-hosted); see [how each background session sources its gateway](agent-view.md#llm-gateway). Use a settings file for any gateway that background agents must always route through.

#### Set in a settings file

To make the configuration apply everywhere Claude Code runs, including [background agents](agent-view.md#how-background-sessions-are-hosted), set the variables in the `env` block of a [settings file](settings.md) instead of relying on your shell. Settings files have different scopes:
...
* **Distributed by an administrator**: if your organization has [deployed the configuration](llm-gateway-rollout.md#distribute-through-managed-settings), the desktop app routes through the gateway with no setup on your part
...
With the gateway configuration active, the desktop app runs sessions on your local machine only: the environment picker doesn't offer SSH sessions or Anthropic-hosted cloud environments, and [Remote Control](remote-control.md) is unavailable. To use Claude Code on a remote host through the gateway, run the CLI on that host with [`ANTHROPIC_BASE_URL` and the gateway credential](#set-the-base-url-and-credential) set there.
...
[Claude Code GitHub Actions](github-actions.md) reads `ANTHROPIC_BASE_URL` and `ANTHROPIC_CUSTOM_HEADERS` from the workflow's `env` block. Pass the credential as the action's `anthropic_api_key` input; the action sets it as `ANTHROPIC_API_KEY`, so it reaches the gateway in the `x-api-key` header.
...
For the action's other authentication options, including `CLAUDE_CODE_OAUTH_TOKEN` and workload identity federation, see [Claude Code GitHub Actions](github-actions.md) and the action's [README](https://github.com/anthropics/claude-code-action#readme).

### Agent SDK

The [Agent SDK](agent-sdk/overview.md) has no gateway-specific options; it passes environment variables to the Claude Code process it spawns. Each SDK accepts an `env` option that sets the spawned process's environment, and the TypeScript and Python SDKs treat it differently:
...
Some gateways route or tag requests using a custom header in addition to the credential, for example a tenant identifier or a routing key. To send one, set [`ANTHROPIC_CUSTOM_HEADERS`](env-vars.md) with one `Name: Value` pair per line. The example below adds a routing header named `X-Org-Route`:
...
Routing and tenant header names like these count as [headers that need approval](server-managed-settings.md#environment-variables-and-the-approval-dialog). When the headers come from a project settings file, Claude Code applies them under the [rules for when it applies `env` values](settings-reference.md#when-claude-code-applies-env-values).

### Add gateway models to the model picker

With model discovery enabled, Claude Code queries the gateway for its model list at startup and adds those names to the `/model` picker alongside the built-in entries. If you or your administrator set `replaceBuiltInOptions` in a [`modelPicker`](settings-reference.md#modelpicker) lineup, Claude Code hides the discovered names too. It keeps a row for the model the session is already using.
...
The helper is any shell command that prints the current credential to stdout. Claude Code runs it through your system shell, so on Windows it can be an executable or a PowerShell invocation. Make the command print nothing but the credential. On Claude Code v2.1.227 or later, a banner or a log line printed alongside the key makes the [helper fail](errors.md#your-apikeyhelper-script-is-failing). Write the script, make it executable, and reference it from `apiKeyHelper` in your [settings file](settings.md):
...
See [`apiKeyHelper`](settings-reference.md#apikeyhelper) for the other cases in which Claude Code re-runs the helper.
...
* It suppresses the [fast mode](fast-mode.md) availability check. Unless a previous check already enabled fast mode on the machine, `/fast` reports that fast mode is unavailable.
...
* The WebFetch tool's [domain safety check](data-usage.md#webfetch-domain-safety-check) isn't affected and still calls `api.anthropic.com`. Turn it off separately with `skipWebFetchPreflight: true` in [settings](settings.md) if your network blocks that host.
* For each telemetry stream and the variable that controls it, see [telemetry services](data-usage.md#telemetry-services).
...
Leave `AWS_BEARER_TOKEN_BEDROCK` unset when the gateway issues its own credential. If you set it, Claude Code sends that [Amazon Bedrock API key](amazon-bedrock.md#2-configure-aws-credentials) as the `Authorization` header instead of your gateway token, even with `CLAUDE_CODE_SKIP_BEDROCK_AUTH` set.
...
The block covers routing and authentication. The region overrides and model pins from the [Agent Platform setup](google-vertex-ai.md#4-configure-claude-code) apply through a gateway as well:

* **Per-model regions**: if your gateway serves some models from a region other than `CLOUD_ML_REGION`, set the matching `VERTEX_REGION_CLAUDE_*` variable for each, for example `VERTEX_REGION_CLAUDE_4_6_SONNET=europe-west1`. The [environment variables reference](env-vars.md) lists the exact names.
* **Model versions**: pin `ANTHROPIC_DEFAULT_OPUS_MODEL`, `ANTHROPIC_DEFAULT_SONNET_MODEL`, and `ANTHROPIC_DEFAULT_HAIKU_MODEL` as in [Pin model versions](google-vertex-ai.md#5-pin-model-versions). Setting `ANTHROPIC_DEFAULT_HAIKU_MODEL` also moves background tasks such as session titles to that model, and that section explains which model runs them otherwise.
* **Model capabilities**: if you pin a model ID that your Claude Code version doesn't recognize, features such as effort levels or extended thinking can stay disabled on it. Declare what the model supports with [`ANTHROPIC_DEFAULT_OPUS_MODEL_SUPPORTED_CAPABILITIES`](model-config.md#customize-pinned-model-display-and-capabilities) and its Sonnet and Haiku counterparts.

#### Microsoft Foundry

Put the gateway's credential in `ANTHROPIC_FOUNDRY_API_KEY`; it is sent to the gateway as the `x-api-key` header. A gateway that expects a bearer token can take [`ANTHROPIC_FOUNDRY_AUTH_TOKEN`](env-vars.md) instead. Claude Code sends that value as the `Authorization: Bearer` header, and it takes precedence over `ANTHROPIC_FOUNDRY_API_KEY` when both are set. Requires Claude Code v2.1.203 or later.
...
See [Claude Platform on AWS](claude-platform-on-aws.md) for the workspace ID.
...
| Certificate or TLS errors such as `SSL certificate verification failed` or `Self-signed certificate detected`, when the [curl test](#verify-the-connection) succeeds                                                                                                                                                                                                                                                                                                                    | Claude Code's runtime isn't trusting the same certificate authority that `curl` uses. Common behind corporate TLS-inspection proxies                                                                                                                                                                                                                                                                                              | Set `NODE_EXTRA_CA_CERTS` to the CA bundle path; see [CA certificate store](network-config.md#ca-certificate-store)                                                                                                                                                                                                                                                                             |

If Claude Code prompts you to log in repeatedly after removing gateway configuration, the cause is usually credential storage rather than the gateway; see [authentication errors](errors.md#authentication-errors).
