...
| Models missing from the `/model` picker | Gateway model names aren’t in Claude Code’s built-in list | Enable [gateway model discovery](#add-gateway-models-to-the-model-picker) or add names with the [model configuration](model-config.md) variables |
