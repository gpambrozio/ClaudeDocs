...
## [​](#configure-claude-code-yourself) Configure Claude Code yourself
...
### [​](#set-the-credential-variable) Set the credential variable
...
### [​](#set-the-base-url-and-credential) Set the base URL and credential
...
#### [​](#set-as-shell-environment-variables) Set as shell environment variables
...
If you export the gateway only in your shell, it doesn’t reliably reach background agents hosted by the [supervisor](agent-view.md); see [how each background session sources its gateway](agent-view.md). Use a settings file for any gateway that background agents must always route through.

#### [​](#set-in-a-settings-file) Set in a settings file

To make the configuration apply everywhere Claude Code runs, including [background agents](agent-view.md), set the variables in the `env` block of a [settings file](settings.md) instead of relying on your shell. Settings files have different scopes:
...
#### [​](#confirm-in-claude-code) Confirm in Claude Code
...
### [​](#how-the-credential-variable-maps-to-a-header) How the credential variable maps to a header
...
### [​](#conflicts-with-an-existing-login) Conflicts with an existing login
...
## [​](#configure-each-surface) Configure each surface
...
- **Distributed by an administrator**: if your organization has [deployed the configuration](llm-gateway-rollout.md), the desktop app routes through the gateway with no setup on your part
...
### [​](#slack-web-and-remote-control) Slack, web, and Remote Control
...
### [​](#send-additional-headers) Send additional headers
...
See [`apiKeyHelper`](settings-reference.md) for the other cases in which Claude Code re-runs the helper.
...
### [​](#turn-off-traffic-outside-the-gateway-path) Turn off traffic outside the gateway path
...
- The WebFetch tool’s [domain safety check](data-usage.md) isn’t affected and still calls `api.anthropic.com`. Turn it off separately with `skipWebFetchPreflight: true` in [settings](settings.md) if your network blocks that host.
- For each telemetry stream and the variable that controls it, see [telemetry services](data-usage.md).

### [​](#route-to-a-cloud-provider-through-a-gateway) Route to a cloud provider through a gateway
...
#### [​](#confirm-the-provider-route) Confirm the provider route
...
## [​](#troubleshoot-gateway-errors) Troubleshoot gateway errors
...
| Certificate or TLS errors such as `SSL certificate verification failed` or `Self-signed certificate detected`, when the [curl test](#verify-the-connection) succeeds | Claude Code’s runtime isn’t trusting the same certificate authority that `curl` uses. Common behind corporate TLS-inspection proxies | Set `NODE_EXTRA_CA_CERTS` to the CA bundle path; see [CA certificate store](network-config.md) |

If Claude Code prompts you to log in repeatedly after removing gateway configuration, the cause is usually credential storage rather than the gateway; see [authentication errors](errors.md).
