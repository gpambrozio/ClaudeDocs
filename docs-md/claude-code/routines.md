...
Create a routine from the web, the Desktop app, or the CLI. All three surfaces write to the same cloud account, so a routine you create in the CLI shows up at claude.ai/code/routines immediately. In the Desktop app, click **New task** and choose **New remote task**; choosing **New local task** instead creates a [local Desktop scheduled task](desktop-scheduled-tasks.md), which runs on your machine and is not a routine.
...
All of your connected [MCP connectors](mcp.md) are included by default. Remove any that the routine doesn’t need. Connectors give Claude access to external services like Slack, Linear, or Google Drive during each run.
...
### [​](#create-from-the-desktop-app) Create from the Desktop app

Open the **Schedule** page in the Desktop app, click **New task**, and choose **New remote task**. The Desktop app shows both local scheduled tasks and routines in the same grid. See [Desktop scheduled tasks](desktop-scheduled-tasks.md) for details on the local option.
...
Each routine runs in a [cloud environment](claude-code-on-the-web.md) that controls network access, environment variables, and setup scripts. Configure environments before creating a routine to give Claude access to APIs, install dependencies, or restrict network scope. See [cloud environment](claude-code-on-the-web.md) for the full setup guide.
