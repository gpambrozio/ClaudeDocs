...
Auto and Bypass permissions appear only after you enable **Allow dangerously skip permissions** in the extension settings. Auto also requires a Team, Enterprise, or API plan and Claude Sonnet 4.6 or Opus 4.6, so the option may remain unavailable even with the toggle on.See the [VS Code guide](vs-code.md) for extension-specific details.
...
Permission modes are set through the UI, CLI flags, or settings files. Telling Claude “stop asking for permission” in the chat does not change the mode. See [Permissions](permissions.md) for how modes interact with allow, ask, and deny rules.
...
### [​](#start-and-use-plan-mode) Start and use plan mode
...
### [​](#how-actions-are-evaluated) How actions are evaluated

Each action goes through a fixed decision order. The first matching step wins:

1. Actions matching your [allow or deny rules](permissions.md) resolve immediately
...
### [​](#how-auto-mode-handles-subagents) How auto mode handles subagents

When Claude spawns a [subagent](sub-agents.md), the classifier evaluates the delegated task before the subagent starts. A task description that looks dangerous on its own, like “delete all remote branches matching this pattern”, is blocked at spawn time.
...
`bypassPermissions` mode offers no protection against prompt injection or unintended actions. For a safer alternative that still maintains background safety checks, use [auto mode](#eliminate-prompts-with-auto-mode). Administrators can block this mode by setting `permissions.disableBypassPermissionsMode` to `"disable"` in [managed settings](permissions.md).

## [​](#compare-permission-approaches) Compare permission approaches
...
## [​](#customize-permissions-further) Customize permissions further
...
**Permission rules** are the first stop. Add `allow`, `ask`, or `deny` entries to your settings file to pre-approve safe commands, force a prompt for risky ones, or block specific tools entirely. Rules apply in every mode except `bypassPermissions`, which skips the permission layer entirely, and are matched by tool name and argument pattern. See [Manage permissions](permissions.md) for syntax and examples.
**Hooks** cover logic that pattern-matching rules can’t express. A [`PreToolUse` hook](hooks.md) runs before every tool call and can allow, deny, or escalate based on command content, file paths, time of day, or a response from an external policy service. A [`PermissionRequest` hook](hooks.md) intercepts the permission dialog itself and answers on your behalf. See [Hooks](hooks.md) for configuration.

## [​](#see-also) See also

- [Permissions](permissions.md): permission rules, syntax, managed policies
- [Hooks](hooks.md): custom permission logic, lifecycle scripting
...
- [Non-interactive mode](headless.md): run Claude Code programmatically with the `-p` flag
