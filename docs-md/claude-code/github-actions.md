...
- [Code Review](code-review.md): automatic review on every pull request, without writing a workflow
- [Claude Code on the web](claude-code-on-the-web.md): Claude Code sessions from your browser or phone
- [Claude Agent SDK](agent-sdk/overview.md): custom automation outside GitHub Actions. The Claude Code GitHub Action is built on the SDK
- [GitHub Enterprise Server](github-enterprise-server.md): Claude Code with self-hosted GitHub
...
- Quick setup works with the Claude API and Claude subscriptions. If you use Amazon Bedrock, Google Cloud’s Agent Platform, or Microsoft Foundry, see [Use Claude Code GitHub Actions with cloud providers](github-actions-cloud-providers.md)
...
- `CLAUDE_CODE_OAUTH_TOKEN`: an OAuth token that authenticates with your Claude subscription, available on Pro, Max, Team, and Enterprise plans. Generate one by running `claude setup-token` locally. See [Generate a long-lived token](authentication.md)
...
### [​](#set-up-for-an-organization) Set up for an organization
...
For data handling and retention questions in a security review, see [data usage](data-usage.md) and [security](security.md).
...
If you configured a [cloud provider](github-actions-cloud-providers.md), also delete the provider secrets, such as `AWS_ROLE_TO_ASSUME`, the `GCP_*` secrets, or the `AZURE_*` secrets, and uninstall the custom GitHub App along with its `APP_ID` and `APP_PRIVATE_KEY` secrets.

### [​](#github-app-permissions) GitHub App permissions

The [Claude GitHub App](https://github.com/apps/claude) is shared by every Claude feature that integrates with GitHub, including the Claude Code GitHub Action, [Code Review](code-review.md), and [auto-fix for pull requests](claude-code-on-the-web.md) on Claude Code on the web. A GitHub App has a single permission set covering all of its features, so the set includes some permissions that the Claude Code GitHub Action doesn’t use.
...
## [​](#interactive-and-automation-modes) Interactive and automation modes
...
### [​](#who-can-trigger-runs) Who can trigger runs
...
### [​](#respond-to-@claude-mentions) Respond to @claude mentions
...
- For a skill packaged in a [plugin](plugins.md), install the plugin with the `plugin_marketplaces` and `plugins` inputs, then pass the namespaced `/plugin-name:skill-name` as the `prompt`. The `plugins` input takes `plugin-name@marketplace-name`, where the marketplace name comes from the marketplace’s own manifest rather than its repository URL.

The following workflow installs the `code-review` plugin and runs its skill on each new or updated pull request. It runs the same plugin as the review workflow from quick setup. Use a workflow like this when you want to control the prompt, model, and triggers yourself. For automatic reviews without maintaining a workflow file, see [Code Review](code-review.md). On public repositories, GitHub withholds secrets from runs triggered by fork pull requests, so the review runs only on pull requests from branches in the same repository.
...
With a `prompt` input, the Claude Code GitHub Action runs in automation mode on any GitHub event, including a cron schedule. For a plain-text prompt, Claude has no shell or GitHub API access until you grant the tools the prompt needs, with `--allowedTools` in `claude_args` or a [`permissions.allow` rule](permissions.md) in the `settings` input. If you invoke a skill instead, Claude can use the tools its [`allowed-tools` frontmatter](skills.md) grants. GitHub runs scheduled workflows only from the default branch and, in public repositories, disables the schedule after 60 days without repository activity.
...
### [​](#define-project-standards-in-claude-md) Define project standards in CLAUDE.md

Create a `CLAUDE.md` file in your repository root to define code style guidelines, review criteria, project-specific rules, and preferred patterns. Claude follows these guidelines when creating PRs and responding to requests. See the [memory documentation](memory.md) for details.
...
For usage tracking across your organization, see the [analytics dashboard](analytics.md) and [monitoring](monitoring-usage.md). For how usage is measured and billed, see [costs](costs.md).

## [​](#use-a-cloud-provider) Use a cloud provider

By default, the Claude Code GitHub Action calls the Claude API directly with your API key or OAuth token. To route inference through your own cloud account instead, set the input for your provider and follow [Use Claude Code GitHub Actions with cloud providers](github-actions-cloud-providers.md):
...
- For Bedrock, Agent Platform, and Foundry, see the cloud provider page’s [troubleshooting section](github-actions-cloud-providers.md)
...
| --- | --- | --- |
| `prompt` | Instructions for Claude, as plain text or a [skill](skills.md) invocation. When omitted, Claude responds to the [trigger phrase](#interactive-and-automation-modes) instead | No |
...
The `claude_args` parameter accepts any [Claude Code CLI argument](cli-reference.md):
...
- `--model`: model to use, for example `claude-sonnet-5`. Without this argument, the Claude Code GitHub Action uses the Claude Code [default model](model-config.md)
- `--mcp-config`: path to [MCP configuration](mcp.md)
- `--allowedTools`: comma-separated list of allowed tools. The `--allowed-tools` alias also works
- `--debug`: enable debug output

## [​](#upgrade-from-beta) Upgrade from beta
...
- [Use Claude Code GitHub Actions with cloud providers](github-actions-cloud-providers.md): route inference through Amazon Bedrock, Google Cloud’s Agent Platform, or Microsoft Foundry
...
- [Code Review](code-review.md): automatic pull request review without maintaining a workflow file
