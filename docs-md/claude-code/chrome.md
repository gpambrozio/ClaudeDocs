...
For VS Code, see [browser automation in VS Code](vs-code.md).
...
If your organization blocks the `claude-in-chrome` MCP server with the [`deniedMcpServers` managed setting](managed-mcp.md), Claude Code doesn’t show the install prompt.
...
In the [VS Code extension](vs-code.md), Chrome is available whenever the Chrome extension is installed. No additional flag is needed.
...
In [plan mode](permission-modes.md), browser tool calls that only read the page or browser state run without a permission prompt, and calls that change state prompt for approval.
...
- **Permissions**: Claude can upload a file only when the session is allowed to read it, so [permission rules](settings-reference.md) that deny `Read` access to a file also block uploading it.
...
| Error | Cause | Fix |
| --- | --- | --- |
| ”Browser extension is not connected” | Native messaging host cannot reach the extension, or your organization’s IP allowlist rejects the connection to `bridge.claudeusercontent.com` | Restart Chrome and Claude Code, then run `/chrome` to reconnect. If your organization uses IP allowlisting and the error persists, see [Organization IP allowlists and proxy egress](network-config.md) |
...
- [Use Claude Code in VS Code](vs-code.md): browser automation in the VS Code extension
