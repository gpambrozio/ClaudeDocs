...
A [self-hosted environment](self-hosted-environments.md) lets [Claude Code on the web](claude-code-on-the-web.md) sessions run on infrastructure you operate instead of on Anthropic’s. Because the session runs inside your network, Claude can call your internal services directly. Those services need a way to confirm that a request came from a Claude Code session in your environment, and to identify the user or service identity that created that session.
