...
Self-hosted environments are in public beta on Team and Enterprise plans; an [Owner or admin](cloud-environments.md) enables them by turning on **Allow self-hosted environments** on the [**Cloud environments** admin page](https://claude.ai/admin-settings/cloud-environments). This page covers session identity verification; see the [quickstart](self-hosted-environments-quickstart.md) for setup and [Deploy to production](self-hosted-environments-deploy.md) for the fleet recipes.

A [self-hosted environment](self-hosted-environments.md) lets [Claude Code on the web](claude-code-on-the-web.md) sessions run on infrastructure you operate instead of on Anthropic’s. Because the session runs inside your network, Claude can call your internal services directly. Those services need a way to confirm that a request really came from a Claude Code session in your environment, and to identify which user created that session.
...
### [​](#verify-the-token-from-your-service) Verify the token from your service
...
### [​](#the-act-chain) The `act` chain

The `act` claim records the full delegation path from the user who created the session down to the [environment](self-hosted-environments.md) whose secret admitted the runner, and the identity that created that secret. The creating user is the outermost actor, so `act.sub` identifies them directly.
...
## [​](#scope-derived-credentials) Scope derived credentials
...
- [Customize sessions](self-hosted-environments-configuration.md): wrapper scripts that consume the token, and the `spawn-runner` hook
- [Reference](self-hosted-environments-reference.md): CLI flags, environment variables, and metrics
