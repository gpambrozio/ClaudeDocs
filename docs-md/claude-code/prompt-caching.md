...
- **Tools loaded into the prefix**: any change to them invalidates the cache. This happens when [tool search is unavailable or disabled](mcp.md), such as on Google Cloud’s Agent Platform or with a custom `ANTHROPIC_BASE_URL` gateway. It also happens for a server or tool marked [`alwaysLoad`](mcp.md), and for definitions kept upfront by [threshold-based loading](mcp.md).
