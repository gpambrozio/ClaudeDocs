...
Plugin changes apply when you run [`/reload-plugins`](discover-plugins.md) or start a new session. For a plugin with a `command` source, Claude Code [can reload the plugin itself](plugin-marketplaces.md). Claude Code can also activate a plugin you [install from the `/plugin` interface](discover-plugins.md) during the install; the install summary tells you whether it did or whether to run `/reload-plugins`. If that reload would trigger the full re-read below, the command warns first and applies when you rerun it with `--force`.
