...
Plugin changes apply when you run [`/reload-plugins`](discover-plugins.md) or start a new session. Claude Code can also activate a plugin you [install from the `/plugin` interface](discover-plugins.md) during the install; the install summary tells you whether it did or whether to run `/reload-plugins`. If that reload would trigger the full re-read below, the command warns first and applies when you rerun it with `--force`.
...
A [fork](sub-agents.md), by contrast, inherits the parent’s system prompt, tools, and conversation history exactly, so its first request reads the parent’s cache. The compaction summarization call described in [Compacting the conversation](#compacting-the-conversation) uses the same prefix-sharing approach.
