...
| Authentication | Claude.ai SSO or email | API key or a [Console sign-in without one](authentication.md) | API key or AWS credentials | API key or AWS credentials | GCP credentials | API key or Microsoft Entra ID |
...
### [​](#leverage-mcp-for-integrations) Use MCP for integrations
