...
When Claude Code stops using a session, it archives the session instead of leaving it in the session list. This happens, for example, when [compaction](context-window.md) rewrites the conversation while Remote Control is disconnected, or when you switch to a different conversation with `/resume`. The session stays available when you [filter for archived sessions](claude-code-on-the-web.md).
...
Context changes stay in sync too. While Claude Code [compacts the conversation](context-window.md), connected devices show compaction progress and, once compaction finishes, where the conversation was compacted. When you run `/clear`, the conversation resets on connected devices.
With [cross-session messaging](cross-session-messaging.md), the Remote Control connection also carries messages between your own Claude Code sessions on different machines, and messages arriving from your [Claude Code on the web](claude-code-on-the-web.md) sessions. These messages travel through Anthropic servers like the rest of Remote Control traffic. [Message sessions on other machines](cross-session-messaging.md) covers the cross-machine delivery rules and the `isolatePeerMachines` approval requirement, and [Control inbound messages](cross-session-messaging.md) covers the inbound controls. Cross-session messaging requires Claude Code v2.1.224 or later.
...
- **Forwarded dialogs expire**: Claude Code keeps permission prompts and `AskUserQuestion` questions open until you answer them. When Claude Code forwards another kind of dialog to the remote session, such as the model-choice prompt shown after a safety refusal, it waits five minutes by default, then closes the dialog and continues with the dialog’s no-action default. Set [`dialogExpiry`](settings.md) to adjust or disable the deadline. Requires Claude Code v2.1.224 or later. The approval dialog for a held cross-session message uses the same deadline. [The held-message expiry rules](cross-session-messaging.md) cover the cases where Claude Code keeps the dialog open past it.
...
### [​](#”remote-control-is-not-yet-enabled-for-your-account”) ”Remote Control is not yet enabled for your account”
...
### [​](#remote-control-could-not-resume-the-previous-session-under-the-current-login) “Remote Control could not resume the previous session under the current login”
...
The automatic reconnection is part of disconnect recovery. When a [Remote Control disconnect message](errors.md) says to run `/login` to restore Remote Control, Claude Code watches for a new sign-in and reconnects to the previous remote session on its own. Any other disconnect listed in that entry needs a manual `/remote-control`. Both messages were added in v2.1.225.
