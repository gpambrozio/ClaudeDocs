...
- **Work from both surfaces at once**: the conversation and the progress of [subagents](sub-agents.md) and [dynamic workflows](workflows.md) stay in sync across all connected devices, so you can send messages from your terminal, browser, and phone interchangeably. Before v2.1.207, sessions hosted by the [Desktop app](desktop.md) didn’t send subagent or workflow progress to connected devices.
...
With [cross-session messaging](cross-session-messaging.md), the Remote Control connection also carries messages between your own Claude Code sessions on different machines, and messages arriving from your [Claude Code on the web](claude-code-on-the-web.md) sessions, traveling through Anthropic servers like the rest of Remote Control traffic. [Message sessions on other machines](cross-session-messaging.md) covers the cross-machine delivery rules and the `isolatePeerMachines` approval requirement. [Control inbound messages](cross-session-messaging.md) covers the inbound controls. Cross-session messaging requires Claude Code v2.1.224 or later.
