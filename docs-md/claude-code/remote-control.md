...
- **`false`**: turn auto-connect off, though a `true` from [managed settings](settings.md) outranks it, because Claude Code saves the choice to your user settings. A `false` in project or local settings (`.claude/settings.json`, `.claude/settings.local.json`) turns auto-connect off even over a managed `true`.
...
- **The error mentions `disableRemoteControl`**: your IT administrator has disabled Remote Control on this device through [managed settings](settings.md), independent of the organization-wide toggle and of how you’re signed in.
