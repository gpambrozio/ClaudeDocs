...
### [​](#make-ctrl-w-delete-back-to-whitespace) Make editing keys follow readline conventions

Set [`keybindingFlavor`](settings-reference.md) to `"readline"` to make the prompt’s editing keys follow GNU readline conventions, as in Bash. The default value is `"classic"`. Requires Claude Code v2.1.238 or later. Under `"readline"`:
...
This setting is separate from the [keybindings configuration file](keybindings.md): the word-editing commands aren’t actions there, so you can’t remap them in `keybindings.json`.
...
- Background tasks are automatically cleaned up when Claude Code exits. If you background the session instead of exiting it, Claude Code hands them to the background session, where they keep running. See [background a running session](agent-view.md)
...
- **Anthropic API with [feature-flag fetching](env-vars.md) on**: about every 90 seconds while you’re active in the session, and less often while you’re idle, while your terminal is unfocused, or while Claude Code keeps finding no pull request for the branch
