...
Keyboard shortcuts may vary by platform and terminal. In [fullscreen rendering](fullscreen.md), press `?` in the transcript viewer to see available shortcuts there.**macOS users**: Option/Alt key shortcuts (`Alt+B`, `Alt+F`, `Alt+D`, `Alt+Y`, `Alt+P`) require configuring Option as Meta in your terminal. See [Enable Option key shortcuts on macOS](terminal-config.md) for the setting in each terminal.
...
| `Ctrl+L` | Redraw or clear the screen | Forces a full terminal redraw, keeping input and conversation history. Use this to recover if the display becomes garbled or partially blank. In [fullscreen rendering](fullscreen.md), it also clears the screen, and you can scroll up to see the earlier messages |
| `Ctrl+O` | Toggle transcript viewer | Shows detailed tool usage and execution, with a timestamp and the model used on each assistant message. Also expands lines that collapse by default, such as MCP calls, shown as a single `Called slack 3 times` line, and [messages from your other sessions](cross-session-messaging.md), shown as a one-line `Message from @<sender>` preview |
...
### [​](#make-ctrl-w-delete-back-to-whitespace) Word boundaries in editing shortcuts
...
These readline conventions apply in Claude Code v2.1.261 and later. The [`keybindingFlavor`](settings-reference.md) setting that turned them on in earlier versions is deprecated and has no effect.
...
| Option key | `Option+Enter` | After enabling [Option as Meta](terminal-config.md) on macOS |
| Shift+Enter | `Shift+Enter` | Native in iTerm2, WezTerm, Ghostty, Kitty, Warp, Apple Terminal, Windows Terminal. For other terminals, see [Enter multiline prompts](terminal-config.md) |
...
| `@` | File path mention | Trigger file path autocomplete. In sessions with [cross-session messaging](cross-session-messaging.md), when you type at least one letter after the `@`, Claude Code also suggests your other live sessions on this machine, so you can tell Claude to message the one you pick. Requires Claude Code v2.1.232 or later |
...
### [​](#remap-insert-mode-key-sequences) Remap INSERT-mode key sequences

The [`vimInsertModeRemaps`](settings-reference.md) setting maps a two-key INSERT-mode sequence to Escape, so a mapping like `jj` returns you to NORMAL mode. Requires Claude Code v2.1.208 or later.
...
- When you recall a prompt that included pasted text, Claude Code sends the full pasted content again when you resubmit. If the content has since been [cleaned up](claude-directory.md), Claude Code doesn’t send the literal `[Pasted text #N]` string; see [Paste large content](terminal-config.md) for what happens to the prompt
...
- If you background the session instead of exiting it, your background tasks keep running in the background session. See [background a running session](agent-view.md)
- Background tasks are automatically terminated if output exceeds 5GB, with a note in stderr explaining why
- On macOS and Linux, Claude Code terminates running background tasks when the operating system signals memory pressure, provided the session has been idle for at least 30 minutes and no turn or subagent is running. Set [`CLAUDE_CODE_DISABLE_BG_SHELL_PRESSURE_REAP`](env-vars.md) to `1` to turn this off. Requires Claude Code v2.1.193 or later. Background commands owned by a [subagent](sub-agents.md) are instead terminated after 60 minutes, configurable in milliseconds with [`CLAUDE_SUBAGENT_BG_SHELL_MAX_MS`](env-vars.md). A command owned by a subagent running in the foreground also ends when that subagent gives its final response; see [Background commands](tools-reference.md) in the tools reference. Before v2.1.218, neither the memory-pressure reap nor the 60-minute limit covered commands moved to the background with `Ctrl+B`
...
### [​](#shell-mode-with-prefix) Shell mode with `!` prefix
...
In a regular interactive session, commands you type in shell mode run outside the [sandbox](sandboxing.md) even when you’ve enabled sandboxing, because the sandbox applies to the commands Claude runs. See [strict sandbox mode](sandboxing.md) for the sessions where shell-mode commands run sandboxed too, such as background sessions with strict sandbox mode on.
Claude responds to the command output automatically once it lands in the transcript, so you can run `! npm test` and get an explanation of the failures without a second prompt. The response costs the same as sending a normal prompt. To restore the earlier behavior where the output is added to context without a response, set [`respondToBashCommands`](settings-reference.md) to `false` in `settings.json`. Before v2.1.186, shell mode always added output to context without a response.

## [​](#queue-messages-while-claude-works) Queue messages while Claude works
...
### [​](#when-claude-code-sends-what-you-queued) When Claude Code sends what you queued
...
- [`/model`](model-config.md): once you confirm the [cache warning](prompt-caching.md), if Claude Code shows one, Claude Code applies your change to the next request it makes in that turn
- [`/effort`](model-config.md): once you confirm the [cache warning](prompt-caching.md), if Claude Code shows one, Claude Code applies your change to the next request it makes in that turn
- [`/fast`](fast-mode.md): Claude Code keeps the fast mode setting that was active when the turn started, so your speed change applies from your next turn. If your current model doesn’t support fast mode, turning it on also [switches your model](prompt-caching.md), and Claude Code uses the new model from its next request in that turn

### [​](#take-back-what-you-queued) Take back what you queued
...
### [​](#when-claude-code-skips-suggestions) When Claude Code skips suggestions

In interactive mode, Claude Code leaves prompt suggestions off by default and hides the **Prompt suggestions** toggle in `/config` in a [session that doesn’t fetch feature flags](env-vars.md), such as one on a third-party provider or through a Claude apps gateway, and in a [first session after an install or upgrade](env-vars.md) whose flags haven’t arrived yet.
...
### [​](#turn-prompt-suggestions-off) Turn prompt suggestions off

To disable prompt suggestions entirely, use any of the following:

- Turn off **Prompt suggestions** in `/config`
- Set [`promptSuggestionEnabled`](settings-reference.md) to `false` in your settings file
...
To turn prompt suggestions off across an organization, set `promptSuggestionEnabled` to `false` in [managed settings](managed-settings.md). Also set `CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION` to `false` under the managed [`env`](settings-reference.md) key so that users can’t re-enable them with their own environment variable.
...
To turn the feature off, set [`emojiCompletionEnabled`](settings-reference.md) to `false` in `settings.json`. This disables both the suggestion popup and the inline replacement.
...
### [​](#turn-spell-checking-on-or-off) Turn spell checking on or off

Claude Code reads the [`spellcheck`](settings-reference.md) setting from three places, and ignores it in a project’s `.claude/settings.json` and `.claude/settings.local.json`. Turn it on from whichever one you use:

- User settings
- Command line
- Managed settings

Add `spellcheck` to `~/.claude/settings.json`. It applies in every project you open, like the rest of your [user settings](settings.md):
...
Add `spellcheck` to one of your organization’s [managed settings sources](permissions.md). It applies to every user who receives those settings, and they can’t turn it off:
...
### [​](#what-claude-code-underlines) What Claude Code underlines
...
### [​](#when-claude-code-underlines-nothing) When Claude Code underlines nothing
...
To bind keys to these actions, see [Diff panel actions](keybindings.md).
...
To rebind these keys, see [Diff actions](keybindings.md).
...
- **Single response**: there are no follow-up turns in the overlay. To continue the thread, ask another `/btw` question. To continue with full tool access in a local session, press `f` to fork this question and answer into a [background subagent](sub-agents.md).
...
On [Opus 4.8, Sonnet 5, Fable 5, Mythos 5, and later versions of those families](tools-reference.md), Claude keeps track of multi-step work without a written checklist, and Claude Code doesn’t provide the tools that fill this list, so it stays empty. If you’d like the task list on those models anyway, opt in with `CLAUDE_CODE_ENABLE_TODO_TOOLS=1` or one of the other ways under [Task tool availability](tools-reference.md). On earlier models such as Opus 4.7, and after you opt in, the task list works as follows:
...
## [​](#wait-for-a-usage-limit-to-reset) Wait for a usage limit to reset

When a claude.ai [usage limit](errors.md) stops Claude mid-task, Claude Code waits in the open session and continues the task on its own after the limit resets. Automatic continue is on by default in interactive sessions signed in with a claude.ai subscription. Requires Claude Code v2.1.234 or later.
...
- **Early**: when you finish adding [usage credits](costs.md) with `/usage-credits`, sign back in after `/upgrade`, or switch models with `/model` during the wait, Claude Code checks whether usage is available again and continues right away if it is. It doesn’t check after an upgrade or purchase you make in a browser on your own. Under [`opusplan`](model-config.md) and other model settings that run plan mode on a different model, Claude Code waits for the reset instead.
...
### [​](#cancel-the-wait) Cancel the wait

Press `Esc` at an empty prompt, or `Ctrl+C`, while the line shows, or run [`/rate-limit-options`](commands.md) and pick **Don’t continue automatically**. Claude Code confirms with a line that starts `Automatic continue cancelled`.
...
- **The continuation is blocked**: a [`UserPromptSubmit` hook](hooks.md) that blocks the continuation prompt, or a failure before it reaches the model, ends the wait. Claude Code tells you the continuation didn’t run. Send a prompt to continue.

### [​](#start-a-wait-yourself) Start a wait yourself
...
- **An Opus or Sonnet limit while you run a model outside that family**: your next turn may not hit that limit. [`opusplan`](model-config.md) and other model settings that run plan mode on the limited family don’t get this exception.
...
- **An [LLM gateway](llm-gateway.md) without a saved claude.ai login**: Claude Code offers the wait only while a saved claude.ai login is the active credential.

### [​](#turn-automatic-continue-off) Turn automatic continue off

In `/config`, turn off **Continue automatically at usage limit**, or set [`autoContinueAtUsageLimit`](settings-reference.md) to `false` in your user settings. `/config autoContinueAtUsageLimit=false` also works, including with `-p`, but the `key=value` form can’t turn it back on, because the setting grants unattended execution. Which settings files Claude Code reads for this key is in the [settings reference](settings-reference.md).
