...
`claude plugin eval` runs your [plugin](plugins.md) against a suite of test cases and scores the results. Each case is a realistic prompt plus one or more graders. A grader is a pass/fail check on what Claude produced, such as a regex over the reply, whether a particular tool was called, or a rubric that a second model judges the reply against.
...
* A plugin directory with a `plugin.json` or `.claude-plugin/plugin.json` manifest, or a [skills-directory plugin](plugins-reference.md#skills-directory-plugins).
...
| `name@skills-dir`                                         | The same, for a [skills-directory plugin](plugins-reference.md#skills-directory-plugins)                                                                                                         |
...
* [Create plugins](plugins.md): build the plugin you're testing, and load it with `--plugin-dir` during development
* [Plugins reference](plugins-reference.md#plugin-eval): the `plugin eval` and `plugin eval init` command entries and the manifest's `experimental.evals` key
...
* [Create and distribute a plugin marketplace](plugin-marketplaces.md): publish the plugin once its suite passes
