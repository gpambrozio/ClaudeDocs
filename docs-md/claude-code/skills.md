...
This guide shows you how to create, use, and manage Agent Skills in Claude Code. For background on how Skills work across Claude products, see [What are Skills?](agents-and-tools/agent-skills/overview.md).
...
| Plugin | Bundled with [plugins](plugins.md) | Anyone with the plugin installed |
...
| **[Slash commands](slash-commands.md)** | Create reusable prompts (e.g., `/deploy staging`) | You type `/command` to run it |
| **[CLAUDE.md](memory.md)** | Set project-wide instructions (e.g., “use TypeScript strict mode”) | Loaded into every conversation |
| **[Subagents](sub-agents.md)** | Delegate tasks to a separate context with its own tools | Claude delegates, or you invoke explicitly |
| **[Hooks](hooks.md)** | Run scripts on events (e.g., lint on file save) | Fires on specific tool events |
| **[MCP servers](mcp.md)** | Connect Claude to external tools and data sources | Claude calls MCP tools as needed |
...
| `agent` | No | Specify which [agent type](sub-agents.md) to use when `context: fork` is set (e.g., `Explore`, `Plan`, `general-purpose`, or a custom agent name from `.claude/agents/`). Defaults to `general-purpose` if not specified. Only applicable when combined with `context: fork`. |
| `hooks` | No | Define hooks scoped to this Skill’s lifecycle. Supports `PreToolUse`, `PostToolUse`, and `Stop` events. |
| `user-invocable` | No | Controls whether the Skill appears in the slash command menu. Does not affect the [`Skill` tool](slash-commands.md) or automatic discovery. Defaults to `true`. See [Control Skill visibility](#control-skill-visibility). |
...
#### [​](#example:-multi-file-skill-structure) Example: multi-file Skill structure
...
2. **Programmatic invocation**: Claude calls it via the [`Skill` tool](slash-commands.md)
...
#### [​](#example:-model-only-skill) Example: model-only Skill
...
#### [​](#give-a-subagent-access-to-skills) Give a subagent access to Skills

[Subagents](sub-agents.md) do not automatically inherit Skills from the main conversation. To give a custom subagent access to specific Skills, list them in the subagent’s `skills` field:
...
#### [​](#run-a-skill-in-a-subagent-context) Run a Skill in a subagent context
...
- **Plugins**: To share Skills across multiple repositories, create a `skills/` directory in your [plugin](plugins.md) with Skill folders containing `SKILL.md` files. Distribute through a [plugin marketplace](plugin-marketplaces.md).
- **Managed**: Administrators can deploy Skills organization-wide through [managed settings](iam.md). See [Where Skills live](#where-skills-live) for managed Skill paths.
...
### [​](#view-and-test-skills) View and test Skills
...
### [​](#skill-doesn’t-load) Skill doesn’t load
