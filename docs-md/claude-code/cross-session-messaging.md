...
A message is a piece of text one Claude writes to another, never conversation history or files. To move a whole conversation or its context, [resume the session](sessions.md) instead.
...
## [​](#when-to-use-cross-session-messaging) When to use cross-session messaging
...
- **Coordinate parallel worktrees**: when sessions work the same repository in separate [worktrees](worktrees.md), Claude can tell the other sessions what landed.
...
- To push external events, such as CI results or chat messages, into a session, use [channels](channels.md)

## [​](#message-another-session) Message another session
...
Once delivered, the message counts toward [usage](costs.md) like a prompt you type, and the receiving Claude can reply to the sender the same way, except in the [one-way cross-machine case](#message-sessions-on-other-machines).
...
### [​](#get-a-notice-when-another-session-goes-idle) Get a notice when another session goes idle
...
#### [​](#ask-for-a-notice) Ask for a notice
...
#### [​](#what-each-session-shows) What each session shows
...
### [​](#see-which-sessions-claude-can-reach) See which sessions Claude can reach
...
- **Your other local sessions**: Claude Code sessions running on the same machine, including [background sessions](agent-view.md). A session appears only when it binds an [inbox socket](#the-sessions-inbox-socket). The worker process that the [supervisor process](agent-view.md) keeps ready for your next background session appears once you dispatch work to it.
...
| On another of your machines | Through Anthropic servers, arriving over that machine’s [Remote Control](remote-control.md) connection |
| On [Claude Code on the web](claude-code-on-the-web.md) | Through Anthropic servers, straight to the cloud session |
...
## [​](#how-a-session-treats-an-incoming-message) How a session treats an incoming message
...
### [​](#control-inbound-messages) Control inbound messages

Set [`crossSessionInbound`](settings.md) to choose what a session does with messages arriving from your other sessions:
...
### [​](#non-interactive-sessions) Non-interactive sessions
...
A `-p` session can’t show the approval dialog. When the [inbound default](#control-inbound-messages) holds a message there, Claude Code keeps it for the same [`dialogExpiry`](settings.md) deadline the dialog uses, five minutes by default:
...
### [​](#the-sessions-inbox-socket) The session’s inbox socket
...
- **Own-child messages**: when no `crossSessionInbound` value applies, Claude Code delivers a message it verifies came from the session’s own child processes, such as a hook or Bash command posting back to its own session’s socket.
  ...
  - Where that process evidence is missing, on macOS after the posting process has exited and in containers where Claude Code runs as process ID 1, Claude Code verifies a child that sent the session’s exported [`CLAUDE_CODE_MESSAGING_TOKEN`](env-vars.md) as its first-line auth frame.
...
- **Sandboxed sessions**: control whether a Bash command can reach the socket from inside the [sandbox](sandboxing.md) with the sandbox’s Unix-socket settings, [`sandbox.network.allowAllUnixSockets` and `sandbox.network.allowUnixSockets`](settings.md).

## [​](#restrict-cross-session-messaging) Restrict cross-session messaging
...
### [​](#require-approval-for-cross-machine-messages) Require approval for cross-machine messages

Set [`isolatePeerMachines`](settings.md) to `true` to require your explicit approval before any `SendMessage` reaches a session beyond this machine:
...
### [​](#turn-off-cross-session-messaging) Turn off cross-session messaging
...
Administrators can turn both sides off for an organization in [managed settings](permissions.md), combining the deny rules with the `refuse`:
...
- **Feature-flag evaluation**: when any of [`CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC`, `DISABLE_TELEMETRY`, `DO_NOT_TRACK`, or `DISABLE_GROWTHBOOK`](env-vars.md) turns off the feature-flag evaluation the feature depends on, cross-session messaging stays off. Each variable’s row says which values do that. Unset whichever applies. These variables can come from your shell, from a settings file’s `env` map, or from managed settings.
...
- **`/list-agents` works but a send didn’t arrive**: messaging is on, and something narrower applies:
  ...
  - **Cloud session missing**: a cloud session appears only while this session is connected to [Remote Control](remote-control.md).
  - **Other-machine session missing**: a session on another of your machines appears only when it runs with [Remote Control](remote-control.md) and this session is connected as well.
...
- [Settings](settings.md): `crossSessionInbound`, `isolatePeerMachines`, and `dialogExpiry`
