...
- **Sessions beyond this machine**: shown while [Remote Control](remote-control.md) is connected and labeled `Remote Control`. These are your sessions on other machines and your [Claude Code on the web](claude-code-on-the-web.md) sessions. Claude can’t send a message to start a conversation with one of these sessions. It can only reply to a message that arrived from one of them. See [Message sessions on other machines](#message-sessions-on-other-machines).
...
| On another of your machines | Through Anthropic servers, arriving over that machine’s [Remote Control](remote-control.md) connection | Replies only |
| On [Claude Code on the web](claude-code-on-the-web.md) | Through Anthropic servers, straight to the cloud session | Replies only |
...
- Left unanswered past the [`dialogExpiry`](settings.md) deadline, the dialog closes and Claude Code drops the message. The deadline defaults to five minutes.
...
- Claude Code exports it to [hooks](hooks.md) and Bash commands as the [`CLAUDE_CODE_MESSAGING_SOCKET`](env-vars.md) environment variable. The export happens before any hook runs, including `SessionStart`. Each session exports its own socket, never one inherited from a parent session.
...
- [Remote Control](remote-control.md): connect the sessions that cross-machine messaging travels through
