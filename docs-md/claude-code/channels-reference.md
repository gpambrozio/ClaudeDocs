...
To use an existing channel instead of building one, see [Channels](channels.md). Telegram, Discord, and fakechat are included in the research preview.
...
A channel published to your own marketplace still needs `--dangerously-load-development-channels` to run, since it isn’t on the [approved allowlist](channels.md). To get it added, [submit it to the official marketplace](plugins.md). Channel plugins go through security review before being approved.

## [​](#see-also) See also

- [Channels](channels.md) to install and use Telegram, Discord, or the fakechat demo, and to enable channels for a Team or Enterprise org
