...
- Automatically cleaned up along with sessions after 30 days, configurable via [`cleanupPeriodDays`](settings.md)
...
Summarize keeps you in the same session and compresses context. If you want to branch off and try a different approach while preserving the original session intact, use [`/branch`](sessions.md) or `claude --continue --fork-session` instead.
