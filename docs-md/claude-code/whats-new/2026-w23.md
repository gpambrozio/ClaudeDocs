...
Releases [v2.1.158 → v2.1.165](changelog.md)4 features · June 1–5
...
[Enable auto mode on third-party providers](permission-modes.md)
...
[Protected paths](permission-modes.md)
...
[Plugin commands](plugins-reference.md)
...
[Decide what to enforce](admin-setup.md)

Other wins

The trigger keyword for [dynamic workflows](workflows.md) changed from `workflow` to `ultracode`; asking for a workflow in your own words still works, and the keyword is highlighted in violet in the prompt

[Stop and SubagentStop hooks](hooks.md) can return `hookSpecificOutput.additionalContext` to give Claude feedback and keep the turn going instead of being treated as an error
