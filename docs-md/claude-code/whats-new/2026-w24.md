...
Releases [v2.1.166 → v2.1.176](changelog.md)3 features · June 8–12
...
[Spawn nested subagents](sub-agents.md)
...
[Test against a clean configuration](debug-your-config.md)

Other wins

[`fallbackModel`](model-config.md) configures up to three fallback models tried in order when the primary is overloaded or unavailable, and `--fallback-model` now applies to interactive sessions too
