...
Releases [v2.1.143 → v2.1.149](/en/changelog#2-1-143)1 feature · May 18–22
...
[Auto mode](/en/permission-modes#eliminate-prompts-with-auto-mode)

Other wins

[`/usage`](/en/costs#track-your-costs) now shows a per-category breakdown of what’s driving your plan limits, attributing recent usage to skills, subagents, plugins, and individual MCP servers

”Extra usage” is renamed to “usage credits” across the CLI, and `/extra-usage` is now `/usage-credits`. The old name still works.

New [`/code-review`](/en/code-review) command reports correctness bugs at a chosen effort level such as `/code-review high`, and `—comment` posts findings as inline GitHub PR comments. `/simplify` remains as a separate cleanup-only review.
