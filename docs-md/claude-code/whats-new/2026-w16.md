...
Releases [v2.1.105 → v2.1.113](changelog.md)5 features · April 13–17
...
[Model config: effort levels](model-config.md)
...
[Auto mode](permission-modes.md) is now available for Max subscribers on Opus 4.7, and the `—enable-auto-mode` flag is no longer required

[Session recap](interactive-mode.md) shows a one-line summary of what happened while you were away; run `/recap` on demand or turn it off from `/config`
...
Push notification tool: with [Remote Control](remote-control.md) connected and “Push when Claude decides” enabled, Claude can ping your phone when it needs you
