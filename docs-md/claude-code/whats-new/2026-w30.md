...
Releases [v2.1.214 → v2.1.219](changelog.md)3 features · July 20–24

Claude Opus 5new model

Claude Opus 5 is the new default Opus model in Claude Code. It’s the default on Max, Team Premium, Enterprise pay-as-you-go, and the Anthropic API, and on Claude Platform on AWS, Amazon Bedrock, and Google Cloud’s Agent Platform. On the Anthropic API and on Max, Team, and Enterprise plans, Opus 5 runs with a [1M-token context window](model-config.md); on Amazon Bedrock and Google Cloud’s Agent Platform, select the 1M model variant. Fast mode moves to Opus 5 at $10/$50 per MTok. Requires v2.1.219 or later.

[](https://mintcdn.com/claude-code/N3yEaTYPXMXFrF6k/images/whats-new/opus-5.mp4?fit=max&auto=format&n=N3yEaTYPXMXFrF6k&q=85&s=8536b1cb3180e539008f39930403e47b)
...
[Scan and fix your codebase](claude-security.md)

Other wins

[`/code-review`](code-review.md) now runs as a background subagent with its own context window, so review work stays out of your conversation and the findings arrive when it completes

`/verify`, `/code-review`, and `/deep-research` run only when you invoke them; Claude no longer launches them on its own

[Emoji shortcodes](interactive-mode.md) autocomplete in the prompt input: type `:heart:` to insert an emoji, or two or more characters after `:` for suggestions; turn it off with `emojiCompletionEnabled`

Skills with `context: fork` [run in the background](skills.md) by default, and `background: false` in the skill’s frontmatter waits for the result in the same turn

A session runs up to 20 subagents concurrently by default; change the [limit](sub-agents.md) with `CLAUDE_CODE_MAX_CONCURRENT_SUBAGENTS`
...
New [`sandbox.filesystem.disabled`](sandboxing.md) setting skips filesystem isolation while keeping network egress control
...
[Fast mode](fast-mode.md) no longer supports Opus 4.7: `/fast` now applies to Opus 5 and Opus 4.8
