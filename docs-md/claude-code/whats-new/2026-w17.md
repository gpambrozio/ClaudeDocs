...
Releases [v2.1.114 → v2.1.119](changelog.md)4 features · April 20–24
...
[Ultrareview guide](ultrareview.md)
...
[Interactive mode: session recap](interactive-mode.md)
...
[Terminal config: create a custom theme](terminal-config.md)
...
[Vim visual mode](interactive-mode.md): press `v` for character selection or `V` for line selection in the prompt input, with operators and visual feedback

Hooks can now call MCP tools directly via [`type: “mcp_tool”`](hooks.md), so a hook can hit an already-connected server without spawning a process

`/cost` and `/stats` are merged into [`/usage`](commands.md); the old names still work as typing shortcuts that open the relevant tab

`/config` changes (theme, editor mode, verbose, and similar) now persist to `~/.claude/settings.json` and follow the same project/local/policy precedence as other [settings](settings.md)

[Forked subagents](sub-agents.md) can be enabled on external builds with `CLAUDE_CODE_FORK_SUBAGENT=1`: a fork inherits your full conversation context instead of starting fresh

Default [effort level](model-config.md) for Pro and Max subscribers on Opus 4.6 and Sonnet 4.6 is now `high` (was `medium`)
...
New [`claude plugin tag`](plugin-dependencies.md) command creates release git tags for plugins with version validation
