...
If Claude hits a permission prompt while you’re away from the terminal, the session pauses until you approve locally. For unattended use, [`--dangerously-skip-permissions`](permissions.md) bypasses prompts, but only use it in environments you trust.
...
Channels are controlled by the `channelsEnabled` setting in [managed settings](settings.md).
...
- [Build your own channel](channels-reference.md) for systems that don’t have plugins yet
- [Remote Control](remote-control.md) to drive a local session from your phone instead of forwarding events into it
- [Scheduled tasks](scheduled-tasks.md) to poll on a timer instead of reacting to pushed events
