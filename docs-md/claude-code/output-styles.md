...
When you switch styles mid-session, Claude uses the new style starting with your next message. The style is part of the system prompt, so that first message rebuilds the [prompt cache](prompt-caching.md#changing-output-style) once. Before v2.1.251, the new style applied only after you ran `/clear` or started a new session.
