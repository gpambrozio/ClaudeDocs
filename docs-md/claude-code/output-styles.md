...
* **`/output-style` command**: run `/output-style <style>` to switch, for example `/output-style concise`. With no argument, the command lists the styles you can pick and marks the current one. Claude Code saves your selection to `.claude/settings.local.json` at the [local project level](settings.md).
...
| `--append-system-prompt` | Appends to the system prompt without removing anything       | You want a one-off addition passed as a [CLI flag](cli-reference.md#system-prompt-flags) at launch |
