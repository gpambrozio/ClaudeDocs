...
To write your first one, start from the [quickstart](claude-apps-gateway.md), which builds a minimal working config and runs it. Once you have a config you’re happy with, the [deployment guide](claude-apps-gateway-deploy.md) covers containerizing and hosting it on Kubernetes, Cloud Run, or your own platform.
...
## [​](#secret-expansion) Secret expansion
...
OpenID Connect (OIDC) is the SSO protocol the gateway uses with your identity provider; see [Identity provider setup](claude-apps-gateway-deploy.md) for what to register on the IdP side.

| Field | Required | Description |
| --- | --- | --- |
| `issuer` | Yes | OIDC discovery base. Must serve discovery at `/.well-known/openid-configuration`. Use HTTPS in production; the gateway accepts an `http://` issuer. A loopback issuer such as `http://localhost:8081` is rejected by the [SSRF guard](claude-apps-gateway-deploy.md) unless `CLAUDE_GATEWAY_ALLOW_LOOPBACK=1` is set in the gateway’s environment. |
...
#### [​](#idp-requests-through-a-forward-proxy) IdP requests through a forward proxy
...
#### [​](#upstream-error-messages) Upstream error messages
...
For example, the gateway replaces Amazon Bedrock’s `Input is too long for requested model.` with `capability_rejected: prompt_too_long`. Claude Code [compacts automatically](errors.md) on that token, as it does on `prompt is too long`.
...
Instead of a static key or bearer, you can use Workload Identity Federation. Create a federation rule by following the [Workload Identity Federation guide](manage-claude/workload-identity-federation.md), then mount your workload’s OIDC JWT as a file, such as a Kubernetes projected service-account token or a CI platform’s id-token. The gateway exchanges the JWT for a short-lived bearer and refreshes it automatically. The token file is re-read on every exchange, so rotated projected tokens are picked up without a restart.
...
| Model access | Amazon Bedrock enables model access by default in commercial regions. The remaining account-level gate is Anthropic’s one-time use case form: if no one in your AWS account has submitted it, open the Amazon Bedrock console, select an Anthropic model from the Model catalog, and complete the form. See [Submit use case details](amazon-bedrock.md) for the AWS Organizations form and the permissions the submitter needs. |
...
#### [​](#google-cloud-agent-platform) Google Cloud Agent Platform
...
| `read_keys` | No | Array of `{id, key}`. Read-only: every `GET` endpoint, including listing caps, fetching one by ID, and reading [`/effective`](claude-apps-gateway-spend-limits.md) and [`/audit`](claude-apps-gateway-spend-limits.md). |
...
| `blocked_message` | No | Appended verbatim to the `429 billing_error` a blocked developer sees. Write the whole instruction, such as a URL or a Slack channel. When unset, the gateway sends only the default message. See [How enforcement works](claude-apps-gateway-spend-limits.md). |
...
The `pricing` block tells the spend meter what to charge instead of USD list price, so caps and [`/effective`](claude-apps-gateway-spend-limits.md) reflect your contracted rates. Amounts stay in USD and remain an estimate, not an invoice. Two prerequisites:
...
- A row replaces list price for requests that `upstream`, an [`upstreams[].name`](#upstreams), serves for `model`. That includes the higher [fast mode](fast-mode.md) rate, so fast and standard requests meter at the same four rates.
...
#### [​](#matcher-values-that-stop-the-gateway-at-boot) Matcher values that stop the gateway at boot
...
#### [​](#what-goes-in-cli) What goes in `cli`

Each `cli` value is a complete Claude Code `managed-settings.json` document, the same schema you would deploy via MDM or `/etc/claude-code/managed-settings.json`, expressed here as YAML. The CLI applies the delivered document at the managed tier, above user and project settings, in place of server-managed settings. It therefore ignores the settings [restricted to OS-level policy sources](server-managed-settings.md), such as `policyHelper` and `wslInheritsWindowsSettings`.
...
The full key reference is in [Claude Code settings](settings-reference.md). The keys most operators reach for first:
...
#### [​](#claude-desktop-overlay) Claude Desktop overlay
...
#### [​](#precedence-with-other-managed-sources) Precedence with other managed sources

If a device also has an MDM-delivered policy or a local `managed-settings.json`, gateway-delivered settings rank first. [Precedence within the managed tier](managed-settings.md) on the managed settings page says when the local sources apply, and has the [keys Claude Code reads from every admin source](managed-settings.md) regardless of which source it selected, such as the sandbox lock keys, `forceRemoteSettingsRefresh`, and the per-variable `env` merge. A [`policyHelper`](settings-reference.md) configured in an MDM profile or the managed settings file runs only when the gateway delivers no settings; the entry says what its output replaces.
Embedding hosts such as [Claude Desktop](desktop.md) can supply policy through the SDK `managedSettings` option. [Parent settings from embedding hosts](managed-settings.md) says when Claude Code applies it, and [Restrict parent settings](claude-apps-gateway.md) lists which allow-direction settings still apply without the `allowManaged*Only` locks.
...
- `http://localhost:<port>` passes config validation, but the [SSRF guard](claude-apps-gateway-deploy.md) blocks every export with `ECONNREFUSED_SSRF` unless you set `CLAUDE_GATEWAY_ALLOW_LOOPBACK=1` in the gateway’s environment
...
[Traces](monitoring-usage.md) additionally require `CLAUDE_CODE_ENHANCED_TELEMETRY_BETA=1` on each client. The gateway doesn’t push that variable, so set it through a managed policy’s `env` block. It isn’t among the variables Claude Code applies without the developer’s approval, so delivering it through a policy is covered by the same [security approval dialog](#managed) that the pushed OTLP endpoint already triggers.
...
This full reference config exercises every core section; the [HTTP tuning blocks](#http-tuning) keep their defaults. Copy it, delete what you don’t need, and fill in your values. The config in the [Quickstart](claude-apps-gateway.md) is a minimal version of this.
...
## [​](#client-side-managed-settings) Client-side managed settings
...
`parentSettingsBehavior: "merge"` keeps Claude Desktop’s delivery of the egress allowlist to its embedded Claude Code sessions working; [Deliver policy to Claude Desktop sessions](claude-apps-gateway.md) explains the mechanism and where the opt-in must sit.
