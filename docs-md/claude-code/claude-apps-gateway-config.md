...
A Claude apps gateway deployment is configured by one YAML file, conventionally `gateway.yaml`. The file defines everything the gateway does: where it listens, how developers sign in, where inference goes, and which policies and telemetry apply. This page is the reference for every option in that file. To write your first one, start from the [quickstart](claude-apps-gateway.md), which builds a minimal working config and runs it; once you have a config you’re happy with, the [deployment guide](claude-apps-gateway-deploy.md) covers containerizing and hosting it on Kubernetes, Cloud Run, or your own platform.
...
For the client-side Bedrock deployment that the gateway replaces or fronts, see [Claude Code on Amazon Bedrock](amazon-bedrock.md). The gateway-side upstream:
...
For the client-side Foundry deployment, see [Claude Code on Microsoft Foundry](microsoft-foundry.md). The gateway-side upstream:
