...
| `permissions.disableBypassPermissionsMode` | CLI | Set to `disable` to block [`bypassPermissions`](permission-modes.md), the mode that auto-approves every tool call, and the `--dangerously-skip-permissions` flag |
...
Embedding hosts can supply policy through the SDK `managedSettings` option. It is ignored by default and applies only when a managed source opts in with [`parentSettingsBehavior: "merge"`](settings.md), filtered so it can tighten policy but not loosen it.
...
Every other key, including `allowManagedPermissionRulesOnly` and `disableBypassPermissionsMode`, comes from the highest-priority source only. See [Settings precedence](settings.md) for the same rule on the settings page.
...
Everything above configures the gateway server. Pointing developer machines at it is configured separately, on each device, through Claude Code’s [managed settings](settings.md). The gateway can’t push these keys itself, because they’re what tell the client where the gateway is.
