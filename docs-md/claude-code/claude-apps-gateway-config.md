...
Every other key, including `disableBypassPermissionsMode`, comes from the highest-priority source only. Two [parent-settings](claude-apps-gateway.md) checks read every admin source:
...
Everything above configures the gateway server. Pointing developer machines at it is configured separately, on each device, through Claude Code’s [managed settings](settings.md). The gateway can’t push the login keys itself, because they’re what tell the client where the gateway is.
