...
To write your first one, start from the [quickstart](claude-apps-gateway.md#quickstart), which builds a minimal working config and runs it. Once you have a config you're happy with, the [deployment guide](claude-apps-gateway-deploy.md) covers containerizing and hosting it on Kubernetes, Cloud Run, or your own platform.
...
OpenID Connect (OIDC) is the SSO protocol the gateway uses with your identity provider; see [Identity provider setup](claude-apps-gateway-deploy.md#identity-provider-setup) for what to register on the IdP side.

| Field                           | Required | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------------------- | -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `issuer`                        | Yes      | OIDC discovery base. Must serve discovery at `/.well-known/openid-configuration`. Use HTTPS in production; the gateway accepts an `http://` issuer. A loopback issuer such as `http://localhost:8081` is rejected by the [SSRF guard](claude-apps-gateway-deploy.md#threat-model-summary) unless `CLAUDE_GATEWAY_ALLOW_LOOPBACK=1` is set in the gateway's environment.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `scopes`                        | No       | Full override of the OIDC scopes the gateway requests. Default `[openid, profile, email, offline_access]`. Set when your IdP rejects scopes it doesn't recognize, or requires a custom scope to emit groups or email. Must include `openid`. Dropping `offline_access` disables refresh tokens, so developers re-run the browser login every `session.ttl_hours`. See [Identity provider setup](claude-apps-gateway-deploy.md#identity-provider-setup) for per-IdP scope recipes such as Google's refresh-token flow.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| Field                     | Required | Description                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ------------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `postgres_url`            | Yes      | `postgres://` or `postgresql://` URL. Required: the device-grant rendezvous, where the browser callback writes and the polling CLI reads, needs cross-replica state. The gateway runs its own schema migrations at boot and on upgrade, so the role needs rights to create and alter tables on the target schema. See [Upgrades](claude-apps-gateway-deploy.md#upgrades) and [Postgres](claude-apps-gateway-deploy.md#postgres). |
...
The Amazon Bedrock, Claude Platform on AWS, Google Cloud's Agent Platform, and Microsoft Foundry upstreams can name your account IDs, role ARNs, and project IDs in their error text. The gateway records that full text in the [operational log](claude-apps-gateway-deploy.md#logs). What the developer sees from those upstreams depends on the rejection:
...
For example, the gateway replaces Amazon Bedrock's `Input is too long for requested model.` with `capability_rejected: prompt_too_long`. Claude Code [compacts automatically](errors.md#prompt-is-too-long) on that token, as it does on `prompt is too long`.
...
Instead of a static key or bearer, you can use Workload Identity Federation. Create a federation rule by following the [Workload Identity Federation guide](../api/manage-claude/workload-identity-federation.md), then mount your workload's OIDC JWT as a file, such as a Kubernetes projected service-account token or a CI platform's id-token. The gateway exchanges the JWT for a short-lived bearer and refreshes it automatically. The token file is re-read on every exchange, so rotated projected tokens are picked up without a restart.
...
For the client-side Amazon Bedrock deployment that the gateway replaces or fronts, see [Claude Code on Amazon Bedrock](amazon-bedrock.md). The gateway-side upstream:
...
| Model access    | Amazon Bedrock enables model access by default in commercial regions. The remaining account-level gate is Anthropic's one-time use case form: if no one in your AWS account has submitted it, open the Amazon Bedrock console, select an Anthropic model from the Model catalog, and complete the form. See [Submit use case details](amazon-bedrock.md#1-submit-use-case-details) for the AWS Organizations form and the permissions the submitter needs.                                                                                                                                                                                |
...
For the client-side deployment of the same platform, see [Claude Code on Claude Platform on AWS](claude-platform-on-aws.md). The gateway-side upstream:
...
For the equivalent client-side setup, see [Claude Code on Google Cloud](google-vertex-ai.md). The gateway-side upstream:
...
For the client-side Microsoft Foundry deployment, see [Claude Code on Microsoft Foundry](microsoft-foundry.md). The gateway-side upstream:
...
Optional. Enables `/v1/organizations/spend_limits`, which mirrors Anthropic's public Admin API, and per-developer spend enforcement on `/v1/messages`. See [Spend limits](claude-apps-gateway-spend-limits.md) for how caps are set and enforced; this section covers the `gateway.yaml` keys that turn the feature on and tune it.
...
| `read_keys`               | No       | Array of `{id, key}`. Read-only: every `GET` endpoint, including listing caps, fetching one by ID, and reading [`/effective`](claude-apps-gateway-spend-limits.md#%2Feffective) and [`/audit`](claude-apps-gateway-spend-limits.md#%2Faudit).                                                                                                              |
...
| `blocked_message`         | No       | Appended verbatim to the `429 billing_error` a blocked developer sees. Write the whole instruction, such as a URL or a Slack channel. When unset, the gateway sends only the default message. See [How enforcement works](claude-apps-gateway-spend-limits.md#how-enforcement-works).                                                                       |
...
The `pricing` block tells the spend meter what to charge instead of USD list price, so caps and [`/effective`](claude-apps-gateway-spend-limits.md#%2Feffective) reflect your contracted rates. Amounts stay in USD and remain an estimate, not an invoice. Two prerequisites:
...
* A row replaces list price for requests that `upstream`, an [`upstreams[].name`](#upstreams), serves for `model`. That includes the higher [fast mode](fast-mode.md#understand-the-cost-tradeoff) rate, so fast and standard requests meter at the same four rates.
...
Each `cli` value is a complete Claude Code `managed-settings.json` document, the same schema you would deploy via MDM or `/etc/claude-code/managed-settings.json`, expressed here as YAML. The CLI applies the delivered document at the managed tier, above user and project settings, in place of server-managed settings. It therefore ignores the settings [restricted to OS-level policy sources](server-managed-settings.md#current-limitations), such as `policyHelper` and `wslInheritsWindowsSettings`.
...
The full key reference is in [Claude Code settings](settings-reference.md#all-settings). The keys most operators reach for first:
...
| `permissions.disableBypassPermissionsMode` | CLI           | Set to `disable` to block [`bypassPermissions`](permission-modes.md#skip-all-checks-with-bypasspermissions-mode), the mode that skips permission prompts, and the `--dangerously-skip-permissions` flag                                                                                                                                                    |
| `allowManagedPermissionRulesOnly`          | CLI           | When `true`, managed settings become the only settings source of permission rules. The [`allowManagedPermissionRulesOnly`](settings-reference.md#allowmanagedpermissionrulesonly) entry lists every source Claude Code then ignores.                                                                                                                       |
...
* Sandbox settings that intercept traffic, inject credentials, or weaken isolation, such as `sandbox.network.tlsTerminate` and the proxy port settings. [Security approval dialogs](server-managed-settings.md#security-approval-dialogs) lists them all.

[Approval memory](server-managed-settings.md#approval-memory) covers how long an approval lasts and when the dialog appears again.
...
To provide MCP servers to the Claude Code clients a policy matches, set [`managedMcpServers`](managed-mcp.md#provide-servers-through-managed-settings) in that policy's `cli` block. You need Claude Code v2.1.259 or later on the gateway server and on clients.

The gateway checks each entry at boot with [the same rules Claude Code applies on the client](managed-mcp.md#what-an-entry-can-contain), and if an entry fails a check, the gateway refuses to start and names the entry.

If you write a `${VAR}` reference in `gateway.yaml`, the gateway resolves it from its environment at boot through [secret expansion](#secret-expansion) before it runs the entry checks, so every matching client receives the literal value and can read it. The [header guidance for provided servers](managed-mcp.md#provide-servers-through-managed-settings) applies to the expanded value.
...
If a device also has an MDM-delivered policy or a local `managed-settings.json`, gateway-delivered settings rank first. [Precedence within the managed tier](managed-settings.md#precedence-within-the-managed-tier) on the managed settings page says when the local sources apply, and has the [keys Claude Code reads from every admin source](managed-settings.md#keys-read-from-every-admin-source) regardless of which source it selected, such as the sandbox lock keys, `forceRemoteSettingsRefresh`, and the per-variable `env` merge. A [`policyHelper`](settings-reference.md#policyhelper) configured in an MDM profile or the managed settings file runs only when the gateway delivers no settings; the entry says what its output replaces.

Embedding hosts such as [Claude Desktop](desktop.md) can supply policy through the SDK `managedSettings` option. [Parent settings from embedding hosts](managed-settings.md#parent-settings-from-embedding-hosts) says when Claude Code applies it, and [Restrict parent settings](claude-apps-gateway.md#restrict-parent-settings) lists which allow-direction settings still apply without the `allowManaged*Only` locks.
...
* `http://localhost:<port>` passes config validation, but the [SSRF guard](claude-apps-gateway-deploy.md#threat-model-summary) blocks every export with `ECONNREFUSED_SSRF` unless you set `CLAUDE_GATEWAY_ALLOW_LOOPBACK=1` in the gateway's environment
...
[Traces](monitoring-usage.md#traces-beta) also require `CLAUDE_CODE_ENHANCED_TELEMETRY_BETA=1` on each client. Set it in a managed policy's `env` block, since the gateway doesn't push it. Developers approve it in the same [security approval dialog](#managed) that the pushed endpoint already triggers.
...
Claude Code also copies each label onto every metric data point, so you can filter metrics by it in a backend that doesn't index resource attributes. To turn that copy off, see [Metrics cardinality control](monitoring-usage.md#metrics-cardinality-control).
...
* Neither you nor the developer has configured [`otelHeadersHelper`](settings-reference.md#otelheadershelper) in any settings source. With a helper configured, every signal stays on the relay.
...
| `rate_limits`    | `device_authorization.max` / `.window_seconds` | 30 / 600 | Per-IP rate limit on the unauthenticated device-authorization endpoint. Raise for a large org behind a shared egress IP or NAT. [Large rollouts](claude-apps-gateway-deploy.md#large-rollouts) shows how to size it. These limits apply only to the device-grant sign-in flow, not to `/v1/messages` inference. See [User-code brute-force resistance](claude-apps-gateway-deploy.md#user-code-brute-force-resistance).                                                                                                                                                                                                                                                                                    |
| `rate_limits`    | `device_verify.max` / `.window_seconds`        | 10 / 600 | Per-IP rate limit on `user_code` submissions at `/device`. It is what stops someone from guessing another developer's code. [Large rollouts](claude-apps-gateway-deploy.md#large-rollouts) shows how far to raise it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
Never turn this on for a gateway that developers use. Every request gets the canned reply and no model is called. The gateway logs a `load_test_mode is on` warning at boot and marks each `inference` [audit event](claude-apps-gateway-deploy.md#logs) with `load_test: true` while the mode is on.

## Complete example

This full reference config exercises every core section; the [HTTP tuning blocks](#http-tuning) keep their defaults. Copy it, delete what you don't need, and fill in your values. The config in the [Quickstart](claude-apps-gateway.md#quickstart) is a minimal version of this.
...
Everything above configures the gateway server. You point developer machines at the gateway separately, on each device, through Claude Code's [managed settings](managed-settings.md). The gateway can't push the login keys itself, because they're what tell the client where the gateway is.
...
`parentSettingsBehavior: "merge"` keeps Claude Desktop's delivery of the egress allowlist to its embedded Claude Code sessions working; [Deliver policy to Claude Desktop sessions](claude-apps-gateway.md#deliver-policy-to-claude-desktop-sessions) explains the mechanism and where the opt-in must sit.

Deploy the `managed-settings.json` file to each device, typically via your MDM platform. The file path differs by platform. See [where each mechanism stores the policy](managed-settings.md#where-each-mechanism-stores-the-policy).
...
Claude Code honors [`forceLoginGatewayUrl`](settings-reference.md#forcelogingatewayurl), [`gatewayInternalNetworks`](settings-reference.md#gatewayinternalnetworks), and the `"gateway"` value of [`forceLoginMethod`](settings-reference.md#forceloginmethod) only from a managed source on the machine: `managed-settings.json`, the macOS plist or Windows HKLM registry, or a policy helper. A developer setting them in their own `~/.claude/settings.json` has no effect, and neither does setting them in the gateway payload.

## Related

* [Claude apps gateway overview](claude-apps-gateway.md): quickstart and developer connection
* [Deployment guide](claude-apps-gateway-deploy.md): IdP setup, container image, Kubernetes and Cloud Run, and operations
* [Spend limits](claude-apps-gateway-spend-limits.md): per-developer caps and the Admin API
