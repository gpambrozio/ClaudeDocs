...
| --- | --- | --- |
| claude | Inherits | When a task doesn’t fit a more specialized agent. A catch-all with every tool [available to subagents](#available-tools). Also the [default agent](agent-view.md) for a dispatched [background session](agent-view.md), which runs with your settings’ permission mode rather than a parent conversation’s |
