...
Claude Code ignores the [OpenTelemetry exporter variables](settings-reference.md#variables-claude-code-ignores-in-env) in a repository's `.claude/settings.json` and `.claude/settings.local.json`, so a repository can't use them to turn telemetry on, choose where it goes, or capture content. Set them in managed settings, or have each developer set them in their shell or `~/.claude/settings.json`. A repository can still turn a signal off by setting its exporter selector, such as `OTEL_LOGS_EXPORTER`, to `none`, unless managed settings, a `--settings` file, or the environment you start Claude Code from sets that variable.
...
| `effort`                         | [Effort level](model-config.md#adjust-effort-level) applied to the request: `low`, `medium`, `high`, `xhigh`, or `max`. Absent when Claude Code sends no effort level, for example on a model that doesn't support effort. Requires Claude Code v2.1.274 or later        |                                |
...
* A WebFetch or WebSearch call that Claude Code moved to the background because you interrupted the turn to [send your queued messages right away](interactive-mode.md#when-claude-code-sends-what-you-queued) while the call ran. Claude receives that result later, after the tool span has ended
...
* A warning notification in interactive sessions, [`otelHeadersHelper failed; telemetry is not being exported`](errors.md#otelheadershelper-failed), shown once per session when the helper first fails
* `/status` output
...
* `effort`: [Effort level](model-config.md#adjust-effort-level) applied to the request: `"low"`, `"medium"`, `"high"`, `"xhigh"`, or `"max"`. Absent when Claude Code sends no effort level, for example on a model that doesn't support effort.
...
| `request_id`        | Server-assigned ID of the API request, read from the `request-id` response header, such as `req_011...`. On a response with no `request-id` header, as on [Amazon Bedrock](amazon-bedrock.md), the value comes from the `x-amzn-requestid` header instead. Present on `api_request`, `api_error`, `api_refusal`, `assistant_response`, and `api_response_body` when the response carries either header. Matches the same attribute on the `llm_request` trace span. The `x-amzn-requestid` source requires Claude Code v2.1.282 or later |
...
* `speed`: `"fast"` or `"normal"`, indicating whether fast mode was active
* `query_source`: Subsystem that issued the request, such as `"repl_main_thread"`, `"compact"`, or a subagent name
* `effort`: [Effort level](model-config.md#adjust-effort-level) applied to the request. Absent when Claude Code sends no effort level, for example on a model that doesn't support effort.
* `agent.name`, `skill.name`, `plugin.name`, `marketplace.name`, `mcp_server.name`, `mcp_tool.name`: Skill, plugin, agent, and MCP attribution for the request. See [Cost counter](#cost-counter) for definitions and redaction behavior.
...
* `num_outputs_persisted`: Number of hook outputs over the [10,000-character cap](hooks.md#json-output) that Claude Code saved to a file. Requires Claude Code v2.1.280 or later
* `managed_only`: `"true"` when only managed-policy hooks are permitted
* `hook_source`: `"policySettings"` or `"merged"`
* `safe_mode`: `"true"` when the session was started with [`--safe-mode`](cli-reference.md), `"false"` otherwise. Requires Claude Code v2.1.169 or later
* `hook_definitions`: JSON-serialized hook configuration. Included only when both detailed beta tracing and `OTEL_LOG_TOOL_DETAILS=1` are enabled
...
Logged with the [managed settings](managed-settings.md) a session resolved: once at session start, again when either the managed settings or the [policy helper](managed-settings.md#compute-the-policy-with-a-helper-program)'s state changes during the session, and when Claude Code refuses to start or ends the session for one of the reasons the `error.type` attribute lists.
...
* Server-managed settings can set it without showing the [security approval dialog](server-managed-settings.md#security-approval-dialogs), because the variable only adds your organization's own redacted policy to an event your organization already receives.

In an interactive session in a folder you haven't [trusted](permissions.md#what-runs-before-you-trust-a-folder), Claude Code doesn't export the refusal event.
...
* `error.type`: why Claude Code stopped the session. Present only on `refused` events:
  * `"helper_failed"`: a [policy helper run failed](settings-reference.md#helper-failures)
  ...
  * `"consent_rejected"`: the user rejected the [security approval dialog](server-managed-settings.md#security-approval-dialogs) for server-managed settings
  * `"force_refresh_failed"`: the settings fetch that [`forceRemoteSettingsRefresh`](settings-reference.md#forceremotesettingsrefresh) requires failed
  * `"gateway_rejected"`: a [Claude apps gateway](claude-apps-gateway.md) answered the managed settings load with HTTP 403
  * `"version_below_minimum"`: this version of Claude Code is below [`requiredMinimumVersion`](settings-reference.md#requiredminimumversion) or above [`requiredMaximumVersion`](settings-reference.md#requiredmaximumversion)
  * `"_OTHER"`: the Claude apps gateway managed settings load failed for another reason
* `managed_settings.sources`: every managed source that delivers at least one [policy key](managed-settings.md#how-claude-code-combines-managed-sources), highest priority first, including sources whose keys don't take effect under `first-wins`. Values are `"remote"`, `"plist"` or `"hklm"` for the MDM or OS-level policy, `"file"` for managed settings files and drop-ins, `"parent"` when an [embedding host](managed-settings.md#let-an-embedding-host-add-policy) supplies settings, and `"hkcu"` for the [Windows HKCU registry value](managed-settings.md#where-each-mechanism-stores-the-policy) when Claude Code [reads it](managed-settings.md#how-claude-code-combines-managed-sources). A source that carries only control keys, or that Claude Code couldn't read, isn't listed. Emitted as an array of strings, empty when no managed source delivers a policy key
* `managed_settings.source_behavior`: the [`managedSourcesBehavior`](settings-reference.md#managedsourcesbehavior) value Claude Code read, `"first-wins"` or `"merge"`. `"first-wins"` when no source sets the key
* `managed_settings.helper.state`: state of the policy helper that the selected MDM or file source configures:
  ...
  * `"bad_path"`, `"not_a_file"`, `"exit_nonzero"`, `"timed_out"`, `"oversize"`, `"parse_failed"`, `"envelope_invalid"`, or `"schema_rejected"`: the helper's last run failed. [Helper failures](settings-reference.md#helper-failures) describes the cases
...
* `managed_settings.helper.entry`: `"policyHelper"` when Claude Code selected a [`policyHelper`](settings-reference.md#policyhelper). Absent when it selected no helper
* `managed_settings.helper.path`: the helper's configured [`path`](settings-reference.md#policyhelper-path). Present whenever Claude Code selected a helper, whether or not `OTEL_LOG_MANAGED_SETTINGS` is set
