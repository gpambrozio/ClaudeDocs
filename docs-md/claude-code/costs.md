...
Use `/cost` to check your current token usage, or [configure your status line](statusline.md) to display it continuously.
...
- **Use plan mode for complex tasks**: Press Shift+Tab to enter [plan mode](common-workflows.md) before implementation. Claude explores the codebase and proposes an approach for your approval, preventing expensive re-work when the initial direction is wrong.
