...
The **Diff tool** entry appears in `/config` only when Claude Code is connected to the IDE, so run `claude` from the JetBrains terminal or run [`/ide`](commands.md) first from an external terminal. See [`diffTool`](settings.md) for the underlying setting.
