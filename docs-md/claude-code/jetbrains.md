...
The **Diff tool** entry appears in `/config` only when Claude Code is connected to the IDE, so run `claude` from the JetBrains terminal or run [`/ide`](commands.md) first from an external terminal. See [`diffTool`](settings-reference.md#difftool) for the underlying setting.
...
* Using Manual mode for edits, because `acceptEdits` and auto mode both approve edits inside your working directory without asking, except in [protected paths](permission-modes.md#protected-paths)
...
The server is named `ide` and is hidden from `/mcp` because there's nothing to configure. If your organization uses a [`PreToolUse` hook](hooks.md#pretooluse) to allowlist MCP tools, though, you'll need to know it exists.
