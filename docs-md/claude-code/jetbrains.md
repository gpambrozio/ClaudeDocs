...
- **Selection context**: the current selection or tab in the IDE is automatically shared with Claude Code. [`Read` deny rules](permissions.md) block this sharing for matching files
...
Claude Code works with any paid Claude subscription (Pro, Max, Team, or Enterprise) or a Claude Console account, and no API key is required. You’ll be prompted to [log in](authentication.md) the first time you run `claude`.
...
The **Diff tool** entry appears in `/config` only when Claude Code is connected to the IDE, so run `claude` from the JetBrains terminal or run [`/ide`](commands.md) first from an external terminal. See [`diffTool`](settings-reference.md) for the underlying setting.
...
When Claude Code runs in a JetBrains IDE in [`acceptEdits` permission mode](permission-modes.md), it may be able to modify IDE configuration files that can be automatically executed by your IDE. This may increase the risk of running Claude Code in `acceptEdits` mode and allow bypassing Claude Code’s permission prompts for bash execution.
When running in JetBrains IDEs, consider:

- Using Manual mode for edits, because `acceptEdits` and auto mode both approve edits inside your working directory without asking, except in [protected paths](permission-modes.md)
...
The server is named `ide` and is hidden from `/mcp` because there’s nothing to configure. If your organization uses a [`PreToolUse` hook](hooks.md) to allowlist MCP tools, though, you’ll need to know it exists.
