...
| `/config [key=value ...]` | Open the [Settings](settings.md) interface to adjust theme, model, [output style](output-styles.md), and other preferences. From v2.1.181, pass one or more `key=value` pairs to set a setting directly without opening the interface, for example `/config thinking=false`. The `key=value` form also works in non-interactive mode (`-p`) and from [Remote Control](remote-control.md). Run `/config help` to list the keys you can set. Alias: `/settings` |
