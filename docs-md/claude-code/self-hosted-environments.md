...
- **Plans**: public beta for Team and Enterprise organizations. Self-hosted environments are off by default; an [Owner or admin](cloud-environments.md) turns on **Allow self-hosted environments** on the [**Cloud environments** admin page](https://claude.ai/admin-settings/cloud-environments), which requires [Claude Code on the web](claude-code-on-the-web.md) to be enabled for the organization.
