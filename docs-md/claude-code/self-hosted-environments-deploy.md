...
* **Enforce the repo-settings guard**: choose the guard mode with [`--confine-repo-settings`](self-hosted-environments-reference.md). The default `warn` logs a violation and still spawns the session, `enforce` refuses the session, and `off` disables the scan. The runner scans each repository's committed settings for:
  ...
  * An operator-posture override such as `sandbox.enabled: false`
  ...
  The guard runs regardless of [`--trust-workspace`](self-hosted-environments-reference.md), and doesn't cover repository hooks, `.mcp.json`, or Bash rules; see [Permissions and tool approval](self-hosted-environments-configuration.md) for where those grants belong.
