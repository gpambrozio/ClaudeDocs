...
Claude Code deletes old session transcripts after the [`cleanupPeriodDays`](settings.md) retention period, but excludes the files in the memory directory from that [retention sweep](claude-directory.md). `MEMORY.md` and topic files stay until you or Claude edits or deletes them.
