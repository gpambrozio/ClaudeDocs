...
Client telemetry is off by default; configuring `telemetry.forward_to` is what turns it on for connected developers, and each interactive client shows a security approval dialog for the pushed settings, as described in the [configuration reference](claude-apps-gateway-config.md). On AWS, each signal maps to a destination as follows.
