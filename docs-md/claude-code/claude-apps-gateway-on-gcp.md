...
The `upstreams` block points at Google Cloud’s Agent Platform with `auth: {}`, so the gateway authenticates via Application Default Credentials from the runtime service account. See the [configuration reference](claude-apps-gateway-config.md) for every field.Two `listen` fields describe what fronts the gateway:
