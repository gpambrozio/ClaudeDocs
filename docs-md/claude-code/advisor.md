...
Claude Code doesn’t offer Fable 5 as the advisor. For organizations with [Fable 5 access](model-config.md), the `/advisor` picker lists it as a dimmed, unselectable row labeled `Fable 5 (temporarily unavailable)`, and Claude Code rejects `/advisor fable` and `--advisor fable`. Fable 5 as the main model isn’t affected.A remotely configured rollout controls when Fable 5 returns as an advisor option.
