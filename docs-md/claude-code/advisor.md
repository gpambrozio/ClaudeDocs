...
Claude Code doesn't invoke a saved advisor that your organization's [`availableModels`](model-config.md#restrict-model-selection) allowlist excludes. To use the advisor, pick an allowed model with `/advisor`.

Claude Code still saves an advisor that your current main model doesn't support. That advisor activates after you switch to a [compatible main model](#choose-an-advisor-model) with [`/model`](model-config.md#setting-your-model). If the API already refused the saved advisor in the current conversation, it stays off until `/clear` or `/compact`, even after you switch models.
...
Fable 5.1 requires Claude Code v2.1.257 or later. Both Fable models require [Fable access](model-config.md#work-with-fable).
