...
On some plans, Fable as the advisor also needs your one-time [consent to bill Fable 5 usage to usage credits](model-config.md). For what happens before you have given that consent, see [Fable advisor and usage credits](#fable-advisor-and-usage-credits).
...
On some plans, Fable as the advisor also needs your one-time [consent to bill Fable 5 usage to usage credits](model-config.md). For what `/advisor fable` does before you have given that consent, see [Fable advisor and usage credits](#fable-advisor-and-usage-credits).
...
Fable 5 requires Claude Code v2.1.170 or later and [Fable 5 access](model-config.md), whether it acts as the main model or the advisor.
...
On some plans, Fable 5 usage bills to usage credits, and Claude Code asks for your [one-time consent to bill Fable 5 usage to usage credits](model-config.md) when you select Fable 5 with `/model`. Fable as the advisor bills the same way, so on those plans Claude Code doesn’t apply Fable as the advisor until you have accepted that consent.
...
To accept the consent, run `/model fable` and choose to continue on Fable 5. Claude Code records the consent and [saves Fable 5 as your selected model](model-config.md). Then select Fable as the advisor.
...
Enabling or disabling the advisor mid-session does not invalidate your main model’s [prompt cache](prompt-caching.md). Unlike [changing model or effort level](prompt-caching.md), toggling `/advisor` keeps the cached prefix intact, and the advisor’s returned guidance is cached as part of the transcript on later turns.
