...
Agent teams are experimental and disabled by default. Enable them by adding `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` to your [settings.json](settings.md) or environment. Agent teams have [known limitations](#limitations) around session resumption, task coordination, and shutdown behavior.
...
From there, Claude creates a team with a [shared task list](interactive-mode.md), spawns teammates for each perspective, has them explore the problem, synthesizes findings, and attempts to [clean up the team](#clean-up-the-team) when finished.
...
When spawning a teammate, you can reference a [subagent](sub-agents.md) type from any [subagent scope](sub-agents.md): project, user, plugin, or CLI-defined. The teammate inherits that subagent’s system prompt, tools, and model. This lets you define a role once, such as a security-reviewer or test-runner, and reuse it both as a delegated subagent and as an agent team teammate.
...
- **Manual parallel sessions**: [Git worktrees](common-workflows.md) let you run multiple Claude Code sessions yourself without automated team coordination
