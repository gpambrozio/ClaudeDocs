...
Claude Code checks each teammate’s model, whether requested in your prompt or set through **Default teammate model**, against your organization’s [`availableModels`](model-config.md) allowlist. When the allowlist blocks a value, Claude Code substitutes another model:
