...
Each teammate has its own context window. When spawned, a teammate loads the same project context as a regular session: CLAUDE.md, MCP servers, and skills. If you start the lead with [`--setting-sources`](cli-reference.md#cli-flags), teammates load from the same restricted list of sources. Before v2.1.281, [split-pane](#choose-a-display-mode) teammates loaded every settings source.
...
* **Messaging between your own sessions**: [cross-session messaging](cross-session-messaging.md) lets Claude pass findings between the sessions you run yourself
