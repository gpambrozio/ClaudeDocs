...
| `claude gateway` | Start the self-hosted [Claude apps gateway](claude-apps-gateway.md) server, for administrators deploying SSO and policy in front of Claude Code on Bedrock, Vertex AI, or Foundry. Requires `--config` pointing at a [`gateway.yaml`](claude-apps-gateway-config.md). Available in Claude Code v2.1.195 and later. | `claude gateway --config gateway.yaml` |
...
| `--model` | Sets the model for the current session with an alias for the latest model (`sonnet`, `opus`, `haiku`, or `fable`) or a model’s full name. Overrides the [`model`](settings.md) setting and [`ANTHROPIC_MODEL`](model-config.md) | `claude --model claude-sonnet-4-6` |
