...
| `--advisor <model>` | Enable the server-side [advisor tool](advisor.md) for this session with a model alias: `opus`, `sonnet`, or `fable` (v2.1.170+), or a full model ID. Takes precedence over the `advisorModel` setting for the session. Requires Claude Code v2.1.98 or later | `claude --advisor opus` |
...
| `--permission-prompt-tool` | Specify an MCP tool to handle permission prompts in non-interactive mode. As of v2.1.199, the prompt tool can’t approve an MCP tool marked as [requiring user interaction](mcp.md): an `allow` result for one is converted to a deny | `claude -p --permission-prompt-tool mcp_auth_tool "query"` |
