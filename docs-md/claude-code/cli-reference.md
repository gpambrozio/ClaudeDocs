...
| --- | --- | --- |
| `--add-dir` | Add additional working directories for Claude to read and edit files. Grants file access; most `.claude/` configuration is [not discovered](permissions.md) from these directories. Validates each path exists as a directory | `claude --add-dir ../apps ../lib` |
...
| `--effort` | Set the [effort level](model-config.md) for the current session. Options: `low`, `medium`, `high`, `xhigh`, `max`; available levels depend on the model. Session-scoped and does not persist to settings | `claude --effort high` |
...
| `--teammate-mode` | Set how [agent team](agent-teams.md) teammates display: `auto` (default), `in-process`, or `tmux`. See [Choose a display mode](agent-teams.md) | `claude --teammate-mode in-process` |
