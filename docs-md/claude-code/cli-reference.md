...
| `--effort` | Set the [effort level](model-config.md) for the current session. Options: `low`, `medium`, `high`, `xhigh`, `max`; available levels depend on the model. Overrides the [`effortLevel`](settings.md) setting for this session and does not persist | `claude --effort high` |
