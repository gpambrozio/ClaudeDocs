...
| `--agent-teams` | Enable experimental [agent teams](agent-teams.md). Makes the `SendMessage`, `TeamCreate`, and `TeamDelete` tools available. Equivalent to `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` | `claude --agent-teams` |
...
| `--enable-auto-mode` | Unlock [auto mode](permission-modes.md) in the `Shift+Tab` cycle. Requires a Team, Enterprise, or API plan and Claude Sonnet 4.6 or Opus 4.6 | `claude --enable-auto-mode` |
