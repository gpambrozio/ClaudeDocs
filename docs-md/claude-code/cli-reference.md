...
| `--teammate-mode` | Set how [agent team](agent-teams.md) teammates display: `in-process` (default), `auto`, or `tmux`. The default changed from `auto` in v2.1.179. Overrides the [`teammateMode`](settings.md) setting for this session. See [Choose a display mode](agent-teams.md) | `claude --teammate-mode auto` |
