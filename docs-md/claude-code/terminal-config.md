...
#### [​](#set-up-shift+enter-with-/terminal-setup) Set up Shift+Enter with /terminal-setup
...
#### [​](#set-up-shift+enter-in-tmux) Set up Shift+Enter in tmux
...
#### [​](#set-up-option+enter-on-macos) Set up Option+Enter on macOS
...
### [​](#reduce-flicker-and-memory-usage) Reduce flicker and memory usage

If you see flicker during long sessions, or your terminal scroll position jumps to the top while Claude is working, try [fullscreen rendering](fullscreen.md). It uses an alternate rendering path that keeps memory flat and adds mouse support. Run `/tui fullscreen` to switch in your current conversation.
