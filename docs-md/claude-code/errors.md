...
* One of a plugin's [default component folders](plugins-reference.md#file-locations-reference), such as `skills/` or `commands/`: the plugin's other components still load
...
When Claude Code can't summarize the commits, the message reads `worktree has commits that are not pushed anywhere` instead. In [agent view](agent-view.md), the session's row shows `not deleted` with the same reason.
