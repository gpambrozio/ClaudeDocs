...
The session reached the API client without any credential. This appears in [background sessions](agent-view.md), cloud sessions, and Agent SDK contexts where the interactive login check doesn’t run before the first request.
...
### [​](#the-remote-sent-a-reply-this-version-cant-display) The remote sent a reply this version can’t display

`/context` shows this line in a terminal [attached to a cloud session](claude-code-on-the-web.md):
...
The **Auto-compact** toggle in `/config` writes `autoCompactEnabled` to user settings. The hint appears only when a `/config` change would take effect. For example, it doesn’t appear when [`DISABLE_AUTO_COMPACT`](env-vars.md) or [`DISABLE_COMPACT`](env-vars.md) turned auto-compact off. It also doesn’t appear when a higher-precedence scope, such as project or managed settings, set `autoCompactEnabled` to `false`. Nor does it appear in a terminal attached to a cloud session, where the cloud session owns auto-compact. Before v2.1.235, the line carried no auto-compact hint.
