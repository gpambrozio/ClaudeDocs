...
Claude Code runs a live [Remote Control](remote-control.md) connection on short-lived credentials that it obtains and renews using your saved claude.ai login. When Claude Code can’t produce a login token that claude.ai accepts, whether while connecting or mid-session, it stops Remote Control and shows the reason in a warning and a transcript line that starts with `Remote Control disconnected`. Your local session keeps running without Remote Control.
...
- In a session started without those restrictions, run `/tui fullscreen`, or `/tui default` to switch back. Claude Code saves the [`tui` setting](settings.md) there and uses it for every later session
