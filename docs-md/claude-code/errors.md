...
- If `/status` shows an `API key` row, an approved [`ANTHROPIC_API_KEY`](authentication.md) is the active credential and takes precedence over your login, so `/login` doesn’t replace it. Rotate the key in the Claude Console, or run `unset ANTHROPIC_API_KEY` to fall back to your subscription.
