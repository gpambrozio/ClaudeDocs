...
This is an entitlement check, not a quota exhaustion. It fires even when your session and weekly allowances have capacity remaining. See [Extended context](model-config.md) for which plans include 1M context directly and which require usage credits.
...
- For a tool that [background subagents drop](sub-agents.md), such as `LSP` or `TaskCreate`, remove the entry or ask Claude to run the subagent in the foreground
