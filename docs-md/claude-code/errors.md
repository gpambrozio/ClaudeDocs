...
This page lists runtime errors Claude Code displays and how to recover from each one, plus what to check when responses seem off without an error. For installation errors such as `command not found` or TLS failures during setup, see [Troubleshooting](troubleshooting.md).
...
If you are prompted to log in repeatedly, see [Not logged in or token expired](troubleshooting.md) for system clock and macOS Keychain fixes.
...
- For repeated prompts to log in across launches, see the system clock and macOS Keychain checks in [Troubleshooting](troubleshooting.md)
- For other failures including `403 Forbidden` and OAuth browser issues, see [Permissions and authentication](troubleshooting.md)
...
- Trim large `CLAUDE.md` memory files or split them into [imports](memory.md)
...
- See [Model not found](troubleshooting.md) to locate where a stale ID is set across `--model`, `ANTHROPIC_MODEL`, and settings files
...
- **Effort level**: run `/effort` to check the current reasoning level and raise it for hard debugging or design work. Defaults vary by model and plan, so check before assuming you are below the maximum. See [Adjust effort level](model-config.md) for per-model defaults and the `ultrathink` shortcut.
...
- Permission denied or filesystem errors during install: [Troubleshooting](troubleshooting.md)
