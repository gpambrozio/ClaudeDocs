...
* **Click a collapsed tool result** to expand it and see the full output. Click again to collapse. The tool call and its result expand together. Only messages that have more to show are clickable.
  ...
  * Clicking also expands a dim `Message from @<sender>` line when the sender is a [teammate](agent-teams.md) or another agent running in your session. The line for a message from [one of your other sessions](cross-session-messaging.md#what-a-message-looks-like) also shows the message's first line and isn't clickable, so press `Ctrl+o` to read that one.
