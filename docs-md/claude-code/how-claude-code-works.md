...
![The agentic loop: Your prompt leads to Claude gathering context, taking action, verifying results, and repeating until task complete. You can interrupt at any point.](https://mintcdn.com/claude-code/ELkJZG54dIaeldDC/images/agentic-loop.svg?fit=max&auto=format&n=ELkJZG54dIaeldDC&q=85&s=e30acfc80d6ff01ec877dd19c7af58b2)
...
These are the primary capabilities. Claude also has tools for spawning subagents, asking you questions, and other orchestration tasks. See [Tools available to Claude](settings.md) for the complete list.
...
**Sessions are ephemeral.** Unlike claude.ai, Claude Code has no persistent memory between sessions. Each new session starts fresh. Claude doesn’t “learn” your preferences over time or remember what you worked on last week. If you want Claude to know something across sessions, put it in your [CLAUDE.md](memory.md).
...
Since sessions are tied to directories, you can run parallel Claude sessions by using [git worktrees](common-workflows.md), which create separate directories for individual branches.
...
- **Delegate mode**: Claude coordinates work through [agent teammates](agent-teams.md) only, with no direct implementation. Only available when an agent team is active.
...
Add Skills, MCP connections, and custom commands](features-overview.md)[## Common workflows
