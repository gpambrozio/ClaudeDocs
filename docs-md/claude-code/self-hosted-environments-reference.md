...
Self-hosted environments are in public beta on Team and Enterprise plans; an [Owner](cloud-environments.md) enables them by turning on **Allow self-hosted environments** on the [**Cloud environments** admin page](https://claude.ai/admin-settings/cloud-environments). This page is the flag and metric reference; see the [quickstart](self-hosted-environments-quickstart.md) for setup and [Deploy to production](self-hosted-environments-deploy.md) for the fleet recipes.

This page is the reference for the two processes you run in a [self-hosted environment](self-hosted-environments.md): the runner, which executes Claude Code [cloud sessions](claude-code-on-the-web.md) on your hosts, and the optional autoscaling orchestrator, which starts runners as sessions queue. Each has its own flag table. Both run on Linux or macOS hosts, which the defaults such as `/workspace` and `~/.claude` assume. Run `claude self-hosted-runner --help` for the authoritative list on your installed version.
...
## [​](#runner-cli-flags) Runner CLI flags
...
| `--base-dir <path>` | `SELF_HOSTED_RUNNER_BASE_DIR` | `/workspace`; none on Windows | Directory for repository checkouts and per-session working directories. The runner needs write access to this path or its parent. The runner creates the directory at startup and exits with `cannot create or write to base directory` when it can’t create or write to it. Before v2.1.225, the runner created the directory when the first session started, so an unusable path failed sessions rather than startup. On Windows, which isn’t a supported runner host, there is no default: the runner exits at startup unless you pass the flag or set the variable. Use the same value on every runner in an environment. See [Keep the base directory and capacity identical across runners](self-hosted-environments-deploy.md). |
| `--capacity <n>` | none | `1` | Maximum concurrent sessions this runner handles. All sessions belong to the same locked [owner](self-hosted-environments.md). Use the same value on every runner in an environment; see [Keep the base directory and capacity identical across runners](self-hosted-environments-deploy.md). |
...
| `--configure-git` | `SELF_HOSTED_RUNNER_CONFIGURE_GIT=1` | off | Write global git identity and enable Anthropic commit signing at startup. See [Configure git](self-hosted-environments-deploy.md). |
| `--confine-repo-settings <mode>` | `SELF_HOSTED_RUNNER_CONFINE_REPO_SETTINGS` | `warn` | Sets the mode of the guard that flags a session when a repository’s committed settings try to grant write or read access outside that session’s own workspace, set environment variables, or override the operator’s sandbox or hooks posture, such as `sandbox.enabled: false` or `disableAllHooks`. The default `warn` logs the violation and still starts the session, `enforce` refuses the session, and `off` disables the scan. See [Harden your deployment](self-hosted-environments-deploy.md). |
...
| `--post-session-hook-timeout-sec <n>` | `SELF_HOSTED_RUNNER_POST_SESSION_HOOK_TIMEOUT_MS` | `60` | Budget for the [`post-session` hook](self-hosted-environments-configuration.md) on every session end, including runner shutdown |
...
| `--startup-timeout-min <n>` | `SELF_HOSTED_RUNNER_STARTUP_TIMEOUT_MS` | `15` | Release a session slot if the child hasn’t signaled that it initialized within N minutes of spawn. Cleared by the child’s init signal on the [activity channel](self-hosted-environments-configuration.md), not by ordinary output, after which `--release-idle-session-min` takes over. `0` disables. |
| `--trust-workspace [bool]` | `SELF_HOSTED_RUNNER_TRUST_WORKSPACE` | on | Seed persisted trust for each session’s repository paths so repo-committed `permissions.allow` and `additionalDirectories` are honored. Set `false` to drop repo-committed permission grants and configure allow rules in the host config’s `settings.json` instead; repository-committed `sandbox.*` settings still apply either way, which is why the [repo-settings guard](self-hosted-environments-deploy.md) scans them regardless of this flag. |
...
### [​](#scm-connector-flags) SCM connector flags
...
The following Prometheus Operator `PodMonitor` covers both processes. It selects pods by the `app.kubernetes.io/part-of: claude-code-self-hosted-runner` label and the named `health` port that the [Kubernetes recipe](self-hosted-environments-deploy.md) sets; adjust the namespaces to match your deployment:
...
### [​](#session-lifecycle-counter-semantics) Session lifecycle counter semantics
...
The `orchestrator_*` rows exist only on environments running the [on-demand orchestrator](self-hosted-environments-configuration.md). On a fixed fleet whose runners outlive their sessions, with `--drain-grace-sec` above `0`, use `sum(rate(claude_code_self_hosted_runner_sessions_started_total[5m]))` for throughput; on a one-shot fleet that series has the same scrape-window problem as the terminal counters, so rely on the queued-sessions count instead. Check backlog on the environment’s **Activity** tab, on the [**Cloud environments** admin page](https://claude.ai/admin-settings/cloud-environments): the runners don’t export a queue-depth series.
For per-session outcome reporting, use the [`post-session` hook](self-hosted-environments-configuration.md) instead: it fires at every session end where a child process was spawned, apart from abrupt runner termination such as a VM preemption, per the [hook’s own contract](self-hosted-environments-configuration.md).

## [​](#what’s-next) What’s next

- [Self-hosted environments](self-hosted-environments.md): the environment, runner, and session model; the [quickstart](self-hosted-environments-quickstart.md) and [Deploy to production](self-hosted-environments-deploy.md) hold setup and operations
- [Customize sessions](self-hosted-environments-configuration.md): wrapper scripts, lifecycle hooks, and on-demand runners
- [Verify session identity](self-hosted-environments-identity.md): the session token, its claims, and how to verify it
