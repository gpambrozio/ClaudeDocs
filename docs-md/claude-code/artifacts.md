...
- You’re not in your first session since you installed Claude Code or upgraded from a version before v2.1.221. In that [first session after an install or upgrade](env-vars.md), Claude can’t read comments yet; start a new session and ask again.
...
- **Press `Ctrl+X Ctrl+K` twice within 3 seconds**: the chord that [stops every running background subagent](interactive-mode.md) also stops Claude from replying on every artifact for the rest of the session.
