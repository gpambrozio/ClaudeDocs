...
If a session's heap memory passes 2.5GB, a critical memory usage warning appears. To free the memory, restart Claude Code and run [`claude --continue`](cli-reference.md#cli-flags) to resume the conversation in a fresh process.

Outside [fullscreen rendering](fullscreen.md), running `/compact` frees memory too. The warning disappears once memory use drops back below 2.5GB.
...
When [sandboxing](sandboxing.md) is on, clipboard utilities such as `pbcopy`, `xclip`, and `wl-copy` can fail to reach the system clipboard from inside a sandboxed Bash command, leaving your clipboard unchanged after Claude pipes text to them.
...
* Hold your terminal's native-selection key while you drag, then copy with your terminal's usual shortcut, such as `Cmd+C`. The key is `Fn` in Terminal.app and `Option` in iTerm2. [Keep native text selection](fullscreen.md#keep-native-text-selection) lists it for other terminals.
* Set [`CLAUDE_CODE_DISABLE_MOUSE=1`](env-vars.md) on the remote machine so your terminal handles selection for the whole session.
