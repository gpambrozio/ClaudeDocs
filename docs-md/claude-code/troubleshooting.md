...
## [​](#debug-installation-problems) Debug installation problems
...
### [​](#failed-to-fetch-version-from-storage-googleapis-com) `Failed to fetch version from storage.googleapis.com`
...
### [​](#windows-wrong-install-command) Windows: wrong install command
...
### [​](#windows-claude-desktop-overrides-claude-cli-command) Windows: Claude Desktop overrides `claude` CLI command
...
### [​](#linux-wrong-binary-variant-installed-musl/glibc-mismatch) Linux: wrong binary variant installed (musl/glibc mismatch)
...
### [​](#illegal-instruction-on-linux) `Illegal instruction` on Linux
...
Sandboxed commands cannot launch Windows binaries such as `cmd.exe`, `powershell.exe`, or executables under `/mnt/c/`. WSL hands these off to the Windows host over a Unix socket, which the sandbox blocks. If a command needs to invoke a Windows binary, add it to [`excludedCommands`](settings.md) so it runs outside the sandbox.
...
- **Unsupported platform.** Prebuilt binaries are published for `darwin-arm64`, `darwin-x64`, `linux-x64`, `linux-arm64`, `linux-x64-musl`, `linux-arm64-musl`, `win32-x64`, and `win32-arm64`. Claude Code does not ship a binary for other platforms; see the [system requirements](setup.md).
...
- **Behind a proxy**: corporate proxies can interfere with API requests. See [network configuration](network-config.md) for proxy setup.

### [​](#model-not-found-or-not-accessible) Model not found or not accessible
...
Check where the model is set, in [priority order](model-config.md):
...
To browse models available to your account, start `claude` interactively and run `/model` to open the picker. For Vertex AI deployments, see [the Vertex AI troubleshooting section](google-vertex-ai.md).
...
| Managed settings | [Managed settings](settings.md) (server-managed, MDM/OS-level policies, or file-based) |
...
For additional JetBrains configuration tips, see the [JetBrains IDE guide](jetbrains.md).

### [​](#report-windows-ide-integration-issues) Report Windows IDE integration issues
...
### [​](#escape-key-not-working-in-jetbrains-ide-terminals) Escape key not working in JetBrains IDE terminals
...
2. **Use post-processing hooks**: set up automatic formatting hooks to detect and add missing language tags. See [Auto-format code after edits](hooks-guide.md) for an example of a PostToolUse formatting hook.
...
### [​](#reduce-markdown-formatting-issues) Reduce markdown formatting issues
...
1. See the [Error reference](errors.md) for `API Error: 5xx`, `529 Overloaded`, `429`, and request validation errors that appear during a session
