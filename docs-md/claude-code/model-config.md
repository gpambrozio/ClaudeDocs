...
- **Subagent and teammate models**: the `model` field in [subagent](sub-agents.md) frontmatter, the Agent tool’s `model` parameter, [agent team](agent-teams.md) teammate models including the `teammateDefaultModel` setting, `CLAUDE_CODE_SUBAGENT_MODEL`, and, on v2.1.197 and earlier, the model picker in the `/agents` wizard
