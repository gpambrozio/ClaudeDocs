...
Subagent IDs are generated fresh for each spawn. Teammate agents, the named members of an [agent team](agent-teams.md), reuse a stable name-based ID across reconnections. In both cases the ID identifies an agent, not a person or a device, so don't treat the agent ID header as a user identifier.
...
* When the upstream rejects a [thinking signature](../api/build-with-claude/extended-thinking.md), Claude Code retries the request without the conversation's earlier thinking blocks and keeps them out of every later request. New responses still include thinking
