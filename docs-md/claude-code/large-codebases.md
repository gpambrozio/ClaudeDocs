...
For the full worktree settings reference, see [Worktree settings](settings.md).
...
Once conventions live in plugins, a teammate starting Claude in an unfamiliar part of the tree has no signal about which plugin that area’s owners maintain. A [`SessionStart` hook](hooks.md) can close that gap, since anything the hook prints to stdout is added to Claude’s context before the first prompt.
For example, you can write a script that reads the launch directory from the [hook input](hooks.md), looks it up in a path-to-plugin map committed to the repository, and prints the recommendation for Claude to relay in its first reply. See [Automate workflows with hooks](hooks-guide.md) to write and register the hook.
...
- **Save the plan to a file before editing**: [plan first](best-practices.md) and ask Claude to write the plan to a markdown file in the repository. A long cross-package session [compacts its context](context-window.md) along the way, and the saved plan survives where conversation history may not
