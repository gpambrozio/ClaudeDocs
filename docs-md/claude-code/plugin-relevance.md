...
* **You want to install plugins**: see [Install and manage plugins](plugins/install.md)
...
Users see no suggestions from a marketplace until an administrator allowlists it in [managed settings](plugins/org.md), even when its `marketplace.json` declares `relevance`.

To allowlist a marketplace, edit your managed settings as follows:

* Add the marketplace name to `pluginSuggestionMarketplaces`.
* For any marketplace other than the official Anthropic marketplace, also declare the marketplace source, either as that name's entry in [`extraKnownMarketplaces`](plugins/org.md#require-a-marketplace-and-its-plugins) or as an entry in [`strictKnownMarketplaces`](plugins/org.md#allowlist-with-strictknownmarketplaces).
...
* [Host a marketplace](plugins/host-marketplace.md): run the marketplace that hosts your plugins
* [Marketplace reference](plugins/marketplace-reference.md#plugin-entries): every field a plugin entry accepts
* [Recommend your plugin from your CLI](plugins/cli-hints.md): prompt users from your own CLI instead of from Claude Code's session signals
* [Manage plugins for your organization](plugins/org.md): `extraKnownMarketplaces`, `strictKnownMarketplaces`, and the rest of the plugin policy keys
