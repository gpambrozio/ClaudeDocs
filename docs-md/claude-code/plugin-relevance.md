...
Declaring `relevance` in `marketplace.json` is not enough on its own. An administrator must allowlist the marketplace in [managed settings](managed-settings.md) before its suggestions appear to users.
