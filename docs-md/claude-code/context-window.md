...
export const ContextWindow = () => {
  ...
  return <>
    ...
    color: '#D97757'
     ...
     setHasInteracted(true)} style={isFullscreen ? {
    height: '100vh',
    borderRadius: 0,
    display: 'flex',
    ...
    return  setHovIdx(b.visIdx)} onMouseLeave={() => setHovIdx(null)} onClick={() => setSelIdx(selIdx === b.visIdx ? null : b.visIdx)} style={{
      width: w + '%',
      height: '100%',
      background: b.color,
    ...
    return 
                ...
                {leavingSubagent && 
                 ...
                 setHovIdx(i)} onMouseLeave={() => setHovIdx(null)} onClick={() => setSelIdx(selIdx === i ? null : i)} style={{
      display: 'flex',
      ...
      opacity: evt.vis === 'full' ? 1 : 0.5
          ...
          {activeGate && (activeGate.kind === 'prompt' || activeGate.kind === 'bang' || activeGate.kind === 'slash') && 
    ...
    flexShrink: 0
          ...
          {activeGate && activeGate.kind === 'compact' && 
              
                
                  Context is at {fmt(totalTokens)} tokens.
                  Run <code style={{
    fontFamily: mono,
    background: 'var(--cw-track)',
    padding: '1px 4px',
...
| [Background commands](interactive-mode.md#background-bash-commands) and background [subagents](sub-agents.md#run-subagents-in-foreground-or-background) | Keep running. Claude Code reminds Claude which ones are still running so it doesn't start a duplicate |
