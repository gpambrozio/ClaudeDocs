...
What you see depends on the [permission mode](permission-modes.md) shown at the bottom of the prompt box:
...
- **Permission modes**: click the mode indicator at the bottom of the prompt box to switch permission modes. On Pro, Max, and Team plans, Auto is the built-in starting permission mode. See [how the extension chooses the starting permission mode](permission-modes.md) for what changes that, and every permission mode the indicator offers.
  - **Auto**: a classifier reviews most actions instead of asking you. See [auto mode](permission-modes.md) for what it reviews and blocks.
...
- **Command menu**: click `/` or type `/` to open the command menu. Options include attaching files, switching models, toggling extended thinking, viewing plan usage (`/usage`), and starting a [Remote Control](remote-control.md) session (`/remote-control`). The Customize section provides access to MCP servers, hooks, memory, permissions, and plugins. Items with a terminal icon open in the integrated terminal.
  - The Settings section includes **Enable Remote Control for all sessions**, which sets [`remoteControlAtStartup`](settings.md) to control whether [new interactive sessions connect to Remote Control automatically](remote-control.md). Requires Claude Code v2.1.203 or later.
...
- **Claude Code settings** in `~/.claude/settings.json`: shared between the extension and CLI. Use it for allowed commands, environment variables, hooks, and MCP servers. On Pro, Max, and Team plans, it’s also one input to the permission mode conversations start in. [Switch permission modes](permission-modes.md) lists the order. See [Settings](settings.md) for details.
...
| `initialPermissionMode` | - | Controls approval prompts for new conversations: `default`, `plan`, `acceptEdits`, or `bypassPermissions`. `manual` is an alias for `default` and selects the mode labeled **Manual** in the mode indicator. When you leave it unset, the extension chooses the starting permission mode as described in [Switch permission modes](permission-modes.md). |
...
| `claudeProcessWrapper` | - | Executable used to launch the Claude process. The bundled binary path is passed as an argument when present. Set this to a separately installed `claude` binary if the extension build doesn’t include one for your platform. In a wrapped setup, conversations start in Manual mode unless you set `initialPermissionMode` or picked Manual, Edit automatically, or Auto in an earlier conversation, because the extension skips the settings and built-in-default steps there; see [Switch permission modes](permission-modes.md). An “Unsupported platform” error at activation means no binary is bundled for your platform; see [which platforms have prebuilt binaries](troubleshoot-install.md). |
...
- **Errors and model prompts**: the extension announces errors in the conversation, and announces when the [usage-credits consent prompt](model-config.md) or the [flagged-request prompt](model-config.md) appears.
...
Each worktree maintains independent file state while sharing git history. This prevents Claude instances from interfering with each other when working on different tasks. For more details, see [Run parallel sessions with Git worktrees](worktrees.md).
...
### [​](#cmd+esc-does-nothing-on-macos) Cmd+Esc does nothing on macOS
...
If you run `claude` in a VS Code integrated terminal, Claude Code reinstalls the extension automatically. To keep it uninstalled, turn off **Auto-install IDE extension** in `/config`, or set [`autoInstallIdeExtension`](settings.md) to `false`. You can also set the [`CLAUDE_CODE_IDE_SKIP_AUTO_INSTALL`](env-vars.md) environment variable to `1`.
