...
- **Extension settings**: Open with `Cmd+,` (Mac) or `Ctrl+,` (Windows/Linux), then go to Extensions → Claude Code.
  ...
  | Environment Variables | Set environment variables for the Claude process. **Not recommended**—use [Claude Code settings](settings.md) instead so configuration is shared between extension and CLI. |
...
- **Claude Code settings** (`~/.claude/settings.json`): These settings are shared between the VS Code extension and the CLI. Use this file for allowed commands and directories, environment variables, hooks, and MCP servers. See the [settings documentation](settings.md) for details.
...
| --- | --- | --- |
| Slash commands | [Full set](slash-commands.md) | Subset (type `/` to see available) |
