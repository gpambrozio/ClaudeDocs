...
Onboarding sets up the **Default** environment for you, whether you connect through [the web](web-quickstart.md) or a CLI flow such as `/web-setup`; if web onboarding shows an environment form instead of creating the environment, keep the form’s defaults to get the same **Default** environment. **Default** carries no configuration of its own:
...
Create, edit, and archive environments from the environment selector at [claude.ai/code](https://claude.ai/code), which you reach after [web onboarding](web-quickstart.md). Environments you create are personal to your account; [shared environments](#organization-shared-environments) created by your admins appear in the same selector. See [Installed tools](#installed-tools) for what’s available without any configuration.
...
Claude Code on the web also sets some variables itself when it starts a session. For [`CLAUDE_AUTOCOMPACT_PCT_OVERRIDE`](claude-code-on-the-web.md), the value Claude Code on the web sets overrides one you add here, so adding that key here has no effect.
...
### [​](#select-an-environment-from-the-cli) Select an environment from the CLI

Run `/remote-env` in your terminal to choose the default environment for cloud sessions you create from the CLI, such as [`claude --cloud`](claude-code-on-the-web.md). The command opens a picker of your existing environments and saves your choice to the `remote.defaultEnvironmentId` key in your [user settings](settings.md), so it applies in every project on your machine until you change it, unless the same key is set at a higher-precedence [settings layer](settings.md), such as a repo’s project settings.
A [self-hosted environment](self-hosted-environments.md) ID, which has the form `ccpool_...`, follows a stricter source rule. See [`remote.defaultEnvironmentId`](settings.md) for the settings layers Claude Code honors it from.
...
### [​](#archive-an-environment) Archive an environment
...
- No new session can start in an archived environment, on any surface. If the environment was your saved [CLI default](#select-an-environment-from-the-cli), CLI cloud sessions fall back to your first available cloud environment. Anything configured with the environment explicitly, such as a [routine](routines.md), can’t start new sessions in it; point it at another environment.

### [​](#organization-shared-environments) Organization-shared environments

Owners and admins on Team and Enterprise plans can create cloud environments that are shared with every member of the organization; the same roles manage everything else on the **Cloud environments** admin page, including [self-hosted environments](self-hosted-environments.md). Shared environments appear in each member’s environment selector alongside their personal ones, so a team can standardize on one configuration instead of each member recreating it.
...
GitHub operations use a [separate proxy](#github-proxy) that is independent of this setting, and Claude Code’s connection to the Anthropic API still works at **None**, as noted under [Security and isolation](claude-code-on-the-web.md).
...
If sessions in the environment work with [artifacts](artifacts.md), include `*.frame.claudeusercontent.com` in your list. Claude Code fetches artifact content from that host. If you leave it out, Claude can’t read artifacts in sessions that run in the environment.
...
## [​](#what’s-available-in-cloud-sessions) What’s available in cloud sessions
...
### [​](#what-carries-over-from-your-setup) What carries over from your setup
...
| MCP servers you added with `claude mcp add` at the default local scope or the user scope | No | Those write to `~/.claude.json` on your machine, not the repo. Add the server with `claude mcp add --scope project`, which writes the repo’s [`.mcp.json`](mcp.md), and commit that file |
| Transport variables in your repo’s `.claude/settings.json` `env` block, such as `NODE_EXTRA_CA_CERTS` and the [mTLS client certificate variables](network-config.md) | No | The hosting environment manages the session’s API connection, so Claude Code ignores these keys and notes each ignored key in the session’s debug log |
...
### [​](#link-output-back-to-the-session) Link output back to the session
...
### [​](#environment-caching) Environment caching
...
Use a setup script to provision the VM itself: toolchains and CLI tools that aren’t [pre-installed](#installed-tools). Use a [SessionStart hook](hooks.md) for project setup that should run everywhere, cloud and local, like `npm install`.
...
| --- | --- | --- |
| **Where you configure them** | The environment dialog at [claude.ai/code](https://claude.ai/code) | A [settings file](settings.md) such as your repo’s `.claude/settings.json`; see [What carries over from your setup](#what-carries-over-from-your-setup) for which files reach a cloud session |
...
If you have SessionStart hooks in your user-level `~/.claude/settings.json`, don’t expect them in the cloud: user-level settings stay on your machine. In a cloud session, Claude Code runs hooks from the repository and from your organization’s [server-managed settings](server-managed-settings.md); sessions in a [self-hosted environment](self-hosted-environments-configuration.md) also run hooks the operator seeded from the runner host’s `~/.claude/`.
...
#### [​](#limitations-in-cloud-sessions) Limitations in cloud sessions
...
- [SessionStart hooks](hooks.md): repo-committed setup that runs in local and cloud sessions
