...
- **Web onboarding on Team and Enterprise**: shows a **Create your first cloud environment** form unless an Owner has turned on [Quick web setup](claude-code-on-the-web.md); keep the form’s defaults and click **Create & finish** to get the same **Default** environment
...
#### [​](#which-requests-get-the-credential) Which requests get the credential
...
#### [​](#requests-that-never-get-the-credential) Requests that never get the credential
...
### [​](#set-the-environment-a-claude-tag-channel-uses) Set the environment a Claude Tag channel uses
...
- The Anthropic API, for Claude Code’s own requests, even at **None**, as noted under [Security and isolation](claude-code-on-the-web.md)
...
- **You’re configuring the local CLI or a self-hosted runner**: keep the host in that allowlist. See [network access requirements](network-config.md) and the self-hosted [network requirements](self-hosted-environments-deploy.md).
...
| --- | --- | --- |
| **Where you configure them** | The environment dialog at [claude.ai/code](https://claude.ai/code), plus the **Cloud environments** admin page for [shared environments](#organization-shared-environments) | A [settings file](settings.md) such as your repo’s `.claude/settings.json`; see [What carries over from your setup](#what-carries-over-from-your-setup) for which files reach a cloud session |
