...
### [​](#add-files-and-context-to-prompts) Add files and context to prompts
...
| **Bypass permissions** | `bypassPermissions` | Claude runs without permission prompts, except for the [actions no mode auto-approves](permission-modes.md), safety classifiers when Claude [acts on external sites](#browse-external-sites), or desktop actions where Claude always asks first, such as [archiving a session](#work-across-sessions). Equivalent to `--dangerously-skip-permissions` in the CLI. On Pro and Max plans, enable it in your Settings → Claude Code under “Allow bypass permissions mode”; on Team and Enterprise plans there is no Settings toggle, and organization policy controls it instead. Only use this in sandboxed containers or VMs. |
...
In Enterprise deployments that route Desktop to Google Cloud’s Agent Platform, auto mode is also available by default; see [Auto mode on Bedrock, Agent Platform, or Foundry](permission-modes.md) for the supported models.
...
### [​](#browse-external-sites) Browse external sites
...
#### [​](#approve-claude’s-actions-on-a-site) Approve Claude’s actions on a site
...
#### [​](#choose-between-the-browser-and-the-chrome-extension) Choose between the Browser and the Chrome extension
...
#### [​](#restrict-external-browsing-for-your-organization) Restrict external browsing for your organization
...
### [​](#monitor-pull-request-status) Monitor pull request status
...
### [​](#open-files-in-other-apps) Open files in other apps
...
To include gitignored files like `.env` in new worktrees, create a [`.worktreeinclude` file](worktrees.md) in your project root.
...
### [​](#watch-background-tasks) Watch background tasks
...
### [​](#work-across-sessions) Work across sessions
...
### [​](#continue-in-another-surface) Continue in another surface
...
### [​](#sessions-from-dispatch) Sessions from Dispatch
...
Connectors are [MCP servers](mcp.md) with a graphical setup flow. Use them for quick integration with supported services. For integrations not listed in Connectors, add MCP servers manually via [settings files](mcp.md). You can also [create custom connectors](https://support.claude.com/en/articles/11175166-getting-started-with-custom-connectors-using-remote-mcp).
...
The plugin browser is not available in cloud sessions, and plugins you install from the desktop app aren’t available for cloud sessions. To use a plugin in a cloud session, either declare it in the repository’s `.claude/settings.json` under [`enabledPlugins`](settings-reference.md) so Claude Code [installs it at session start](cloud-environments.md), or enable it for your claude.ai account so Claude Code loads it as a [synced plugin](plugins-reference.md). Plugins aren’t available in WSL sessions. For the full plugin reference including creating your own plugins, see [plugins](plugins.md).
...
#### [​](#auto-verify-changes) Auto-verify changes
...
On models with [adaptive reasoning](model-config.md), `MAX_THINKING_TOKENS` values other than `0` are ignored because adaptive reasoning controls thinking depth instead. On Opus 4.6 and Sonnet 4.6, set `CLAUDE_CODE_DISABLE_ADAPTIVE_THINKING` to `1` to use a fixed thinking budget; Fable models, Sonnet 5, and Opus 4.7 and later always use adaptive reasoning and have no fixed-budget mode.

#### [​](#local-sessions-on-managed-devices) Local sessions on managed devices

Your administrator can turn off local sessions with the [`disableDesktopLocalSessions` managed setting](#managed-settings). When they have, **Local** stays in the environment dropdown but is grayed out and can’t be selected, with a tooltip saying your organization turned it off, and on Windows the [WSL](desktop-wsl.md) entry, whose availability on managed devices is [governed separately](admin-setup.md), is grayed out the same way. New sessions default to the first SSH connection if one is configured, and Desktop shows a message that local sessions aren’t available on this device if you try to continue an existing one. Choose an [SSH](#ssh-sessions) or [cloud](#cloud-sessions) environment instead, or contact your IT team.
...
- **Models**: the same [models](model-config.md) are available in both. In Desktop, select the model from the dropdown next to the send button. You can change the model mid-session from the same dropdown.

#### [​](#mcp-servers-from-the-claude-desktop-chat-app) MCP servers from the Claude Desktop chat app
...
The app also re-delivers stdio servers from `~/.claude.json` to the embedded CLI in local sessions. When the top level of `~/.claude.json` (user scope) and `.mcp.json` define the same stdio server name, the Code tab uses the `~/.claude.json` definition, departing from the CLI [scope hierarchy](mcp.md).

The standalone CLI does not read `claude_desktop_config.json`. On macOS and WSL, run `claude mcp add-from-claude-desktop` to copy those servers into `~/.claude.json`. See [Import MCP servers from Claude Desktop](mcp.md) for the import flow and scope options.
...
| [Third-party providers](third-party-integrations.md) | Amazon Bedrock, Google Cloud’s Agent Platform, Microsoft Foundry | Anthropic’s API by default. For gateway routing, see [connect the desktop app to a gateway](llm-gateway-connect.md). To run the Code tab on Amazon Bedrock, Google Cloud’s Agent Platform, Microsoft Foundry, or a self-hosted LLM gateway, see [Claude Desktop on 3P](https://claude.com/docs/third-party/claude-desktop/overview). |
...
| iOS simulator | Drive the simulator via [computer use](computer-use.md) | [iOS Simulator pane](desktop-ios-simulator.md) opens automatically |
...
### [​](#403-or-authentication-errors-in-the-code-tab) 403 or authentication errors in the Code tab
...
### [​](#blank-or-stuck-screen-on-launch) Blank or stuck screen on launch
...
### [​](#git-and-git-lfs-errors) Git and Git LFS errors
...
### [​](#mcp-servers-not-working-on-windows) MCP servers not working on Windows
