...
- Claude Code checks each message from this surface against the receiving session’s [inbound controls](cross-session-messaging.md). If you set [`crossSessionInbound`](settings.md) to `refuse` in the receiving session, Claude Code drops messages from this surface. The check runs even when the receiving session doesn’t have [cross-session messaging](cross-session-messaging.md) itself. Before v2.1.234, Claude Code dropped every message from this surface to a receiving session without cross-session messaging.
