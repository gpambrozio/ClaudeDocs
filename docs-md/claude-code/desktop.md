...
- **[Cloud sessions](#cloud-sessions)**: receive [server-managed settings](server-managed-settings.md); device-deployed files don’t reach them, because they run on Anthropic-managed VMs. Sessions routed to a [self-hosted environment](self-hosted-environments.md) fall back to the managed settings file in the runner image when server-managed settings deliver no keys, per [settings precedence](server-managed-settings.md), apart from the [keys Claude Code reads from every admin source](managed-settings.md).
