...
New worktrees branch from the repository’s default branch, so most sessions don’t need this setting. Set `worktree.baseRef` in [settings](settings.md) to branch from your current work instead. The setting accepts two values:
