...
Read more about [configuring permissions](settings.md) and [enabling sandboxing](sandboxing.md).
...
Claude automatically checkpoints before changes. Double-tap `Escape` or run `/rewind` to open the rewind menu. You can restore conversation only, restore code only, restore both, or summarize from a selected message. See [Checkpointing](checkpointing.md) for details.
...
Understand the agentic loop, tools, and context management](how-claude-code-works.md)[## Extend Claude Code

Choose between skills, hooks, MCP, subagents, and plugins](features-overview.md)[## Common workflows

Step-by-step recipes for debugging, testing, PRs, and more](common-workflows.md)[## CLAUDE.md

Store project conventions and persistent context](memory.md)
