...
For a marketplace-installed plugin, a `displayName` on the [marketplace entry](plugins/marketplace-reference.md#plugin-entries) takes precedence over this value.
...
`${CLAUDE_PLUGIN_ROOT}` changes when the plugin updates, so don't write state there. For where the root moves and when the old directory is cleaned up, see the [loading page](plugins/loading.md).

When you uninstall the plugin from the last place it's installed, the `${CLAUDE_PLUGIN_DATA}` directory is deleted unless you pass [`--keep-data`](plugins/cli-reference.md).
...
| Executables   | `bin/`                       | Files here are on the Bash tool's `PATH` while the plugin is enabled, so Claude runs them as bare commands. claude.ai and Cowork don't install a plugin that has this directory, including one you [distribute through claude.ai organization settings](plugins/host-marketplace.md#distribute-through-organization-settings) |
...
To click through this layout and read what each file does, open the [plugin explorer](plugins/components.md#explore-the-plugin-directory).
...
A [marketplace entry](plugins/marketplace-reference.md) accepts every field on this page alongside [its own fields](plugins/marketplace-reference.md#plugin-entries), including `strict`.
