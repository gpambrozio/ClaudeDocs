...
| `ListAgents` | Lists the agents Claude can message with `SendMessage`, apart from [agent team](agent-teams.md) teammates, which Claude reaches through the team’s roster: subagents in the session, your other local Claude Code sessions, your [Claude Code on the web](claude-code-on-the-web.md) sessions when this session has cloud access, and your Remote Control sessions on other machines while this session is connected to [Remote Control](remote-control.md). Backs the `/list-agents` command. See [cross-session messaging](cross-session-messaging.md). Requires Claude Code v2.1.224 or later, and appears only in sessions where [cross-session messaging is enabled](cross-session-messaging.md) | No |
