...
| `RemoteTrigger` | Creates, updates, runs, and lists [Routines](routines.md) on claude.ai. Backs the `/schedule` command. The [`RemoteTrigger` input reference](agent-sdk/typescript.md) documents every action and the organization policies that remove the tool. Routines live on claude.ai and require a Pro, Max, Team, or Enterprise plan, so this tool is not accessible from Amazon Bedrock, Claude Platform on AWS, Google Cloud’s Agent Platform, or Microsoft Foundry | No |
...
| `SendMessage` | Sends a message to another agent: an [agent team](agent-teams.md) teammate, a [subagent it resumes](sub-agents.md) by agent ID or name, or one of your other Claude Code sessions, on this machine or beyond it. Messaging other sessions requires Claude Code v2.1.224 or later. [Cross-session messaging](cross-session-messaging.md) covers which sessions Claude can reach, [what a message looks like when it arrives](cross-session-messaging.md), and [how Claude gets a notice when another session goes idle](cross-session-messaging.md). Claude can include an optional `summary` input, typically 5-10 words, that Claude Code shows as a one-line preview. When Claude omits it on a [plain-text message](cross-session-messaging.md), Claude Code uses the first line of the message as the summary. Claude Code truncates a summary longer than 200 characters with an ellipsis | No |
...
- in [`permissions.allow`](settings-reference.md) and [`permissions.deny`](settings-reference.md) in settings, and the `/permissions` interface
- in the `--allowedTools` and `--disallowedTools` [CLI flags](cli-reference.md)
- in the Agent SDK’s [`allowedTools` and `disallowedTools`](agent-sdk/permissions.md) options
- in a [subagent’s `tools` or `disallowedTools`](sub-agents.md) frontmatter
- in a [skill’s `allowed-tools`](skills.md) frontmatter
- in a hook’s [`if` condition](hooks-guide.md)
...
| --- | --- | --- |
| `Bash(npm run *)` | Bash, Monitor | [Command pattern matching](permissions.md) |
| `PowerShell(Get-ChildItem *)` | PowerShell | [Command pattern matching](permissions.md) |
| `Read(~/secrets/**)` | Read, Grep, Glob, LSP | [Path pattern matching](permissions.md) |
| `Edit(/src/**)` | Edit, Write, NotebookEdit | [Path pattern matching](permissions.md) |
| `Skill(deploy *)` | Skill | [Skill name matching](skills.md) |
| `Agent(Explore)` | Agent | [Subagent type matching](permissions.md) |
| `WebFetch(domain:example.com)` | WebFetch | [Domain matching](permissions.md) |
...
Where you see a subagent’s permission prompts depends on whether it runs in the foreground or the background. Claude Code runs subagents in the background by default, apart from the [cases that run in the foreground](sub-agents.md).
...
To [limit what a subagent can reach](sub-agents.md) in the first place, narrow its `tools` field, for example by leaving Bash off the list, or set deny rules in your settings.
...
### [​](#what-persists-between-commands) What persists between commands
...
### [​](#timeout-and-output-limits) Timeout and output limits
...
To change how much of a valid result Claude receives inline, set the [`bashOutputMaxChars`](settings-reference.md) setting instead, up to 128,000 characters. It sizes the inline ceiling and the read-back window together, and Claude Code then ignores `BASH_MAX_OUTPUT_LENGTH`. Requires Claude Code v2.1.261 or later.
...
Claude Code can also count other kinds of processes it starts against the same limit. Set [`CLAUDE_CODE_TOOL_MEMORY_CGROUP_EXCLUDE`](env-vars.md) to a comma-separated list of the kinds to exempt from the cap; Claude Code applies the cap to every kind not on your list. Set it to `none` to cap every kind, or to `all-new` to cap only Bash, PowerShell, and Monitor tool commands. Requires Claude Code v2.1.246 or later. The kinds you can name:
...
Three checks must pass for an edit to apply. Before any of them, a path matched by a [`Read` deny rule](permissions.md) is refused, including creating a new file there. The refusal requires Claude Code v2.1.208 or later.
...
Viewing a file with Bash affects edit eligibility only, not permissions. See [Read and Edit permission rules](permissions.md) for which Bash commands your `Read` and `Edit` deny rules cover.
...
Claude Code keeps the tool inactive until you install a [code intelligence plugin](discover-plugins.md) for your language. In [cloud sessions](claude-code-on-the-web.md), Claude Code doesn’t start plugin language servers, so the LSP tool stays inactive there. Claude Code takes the language server’s configuration from the plugin, and you install the server binary yourself.
...
When Monitor runs a command, it uses the same [permission rules as Bash](permissions.md), so `allow` and `deny` patterns you have set for Bash apply here too. While [auto mode](permission-modes.md) is active, Claude Code sets aside allow rules that name `Monitor` itself, along with the other [broad allow rules it drops](permission-modes.md), so the classifier reviews Monitor commands the same way it reviews Bash commands.
...
Opening a WebSocket prompts for approval; in [auto mode](permission-modes.md) the classifier decides instead. The prompt doesn’t offer an option to skip future prompts for the same host.
Claude Code denies URLs that point at a private, link-local, or cloud-metadata address, including hostnames that resolve to one. It also denies hosts in `sandbox.network.deniedDomains`, and when [`allowManagedDomainsOnly`](settings-reference.md) is set in managed settings, any host outside the managed allowlist.
...
Your [PreToolUse hooks](hooks.md) receive the tool’s command string in `tool_input.command`, with the same fields as the Bash tool.
Match `Bash|PowerShell` in hooks that inspect shell commands; the [PowerShell hook input section](hooks.md) explains why matching `Bash` alone is not enough.
...
- `"defaultShell": "powershell"` in [`settings.json`](settings-reference.md): routes interactive `!` commands through PowerShell. Requires the PowerShell tool to be enabled.
...
- Name one of the tools in [`--allowedTools`](cli-reference.md), for example `claude --allowedTools TaskCreate`
- List the tools in [`--tools`](cli-reference.md), which restricts the session’s built-in tools to the ones it names. Include the tools you want alongside the other built-in tools you use
- In the Agent SDK, the [`allowedTools` and `tools` options](agent-sdk/todo-tracking.md) work the same way as the two flags
...
Claude Code gives a subagent the tools only when your session has them, even when the subagent runs a different model. An in-process [agent team](agent-teams.md) teammate follows your session the same way, while a teammate in its own [split pane](agent-teams.md) runs as a separate Claude Code process, so its own model decides. Without the Task tools, an agent coordinates with its team through messages instead of the [shared task list](agent-teams.md).
...
- WebFetch caches each response for 15 minutes by default, so repeated fetches of the same URL return quickly. On Claude Code v2.1.233 or later, set [`CLAUDE_CODE_WEBFETCH_CACHE_TTL_MS`](env-vars.md) to change how long WebFetch keeps each response.
...
In Manual and `acceptEdits` [permission modes](permission-modes.md), WebFetch prompts before fetching, except for domains your [permission rules](permissions.md) already allow or deny and a built-in set of preapproved documentation domains that fetch without a prompt. Whatever your rules allow, a fetch also passes the [WebFetch domain safety check](data-usage.md) first; that section covers what the check sends and the setting that skips it. The prompt offers three options:

- **Yes**: approves this fetch only. The next WebFetch call prompts again, even for the same domain.
- **Yes, and don’t ask again for `<domain>`**: approves the fetch and saves a `WebFetch(domain:...)` allow rule for that domain to `.claude/settings.local.json` for that repository. See [how saved approvals persist](permissions.md). When your organization sets [`allowManagedPermissionRulesOnly`](permissions.md), Claude Code hides this option.
- **No, and tell Claude what to do differently**: rejects the fetch.

To allow a domain in advance without a prompt, add an allow rule like `WebFetch(domain:example.com)`; `WebFetch(domain:*)` allows every domain. The `auto` and `bypassPermissions` [permission modes](permissions.md) skip the prompt, except for a domain an explicit `ask` rule matches.
...
Sandboxed commands don’t inherit WebFetch’s built-in set of preapproved documentation domains. To let a sandboxed command reach a domain without a prompt, add the domain to [`allowedDomains`](settings-reference.md) or allow it with a `WebFetch(domain:...)` rule, which the [sandbox also honors](sandboxing.md). WebFetch never reads the sandbox allowlist in return, so adding a domain to a sandbox or organization network allowlist doesn’t stop WebFetch from prompting for it.
