...
| `ListAgents`           | Lists the agents Claude can message with `SendMessage`: subagents in the session, [agent team](agent-teams.md) teammates, your other local Claude Code sessions, and, while this session is connected to [Remote Control](remote-control.md), your [Claude Code on the web](claude-code-on-the-web.md) sessions and your Remote Control sessions on other machines. Backs the `/list-agents` command. See [cross-session messaging](cross-session-messaging.md). Requires Claude Code v2.1.224 or later, and appears only in sessions where [cross-session messaging is enabled](cross-session-messaging.md#availability). Teammate rows and the first line showing this session's own name require v2.1.239 or later                                                                                                                                                                                                                                                                              | No                  |
...
In every case, the resolved set is limited to the [tools available to subagents](sub-agents.md#available-tools): a tool that isn't available to subagents is never granted, even when listed in `tools`.
...
In [background sessions](agent-view.md) and in [Claude Code on the web](claude-code-on-the-web.md), Claude Code provides the same tools on every model, listed or not.
