...
On Linux and WSL, set [`CLAUDE_CODE_TOOL_MEMORY_LIMIT`](env-vars.md) to a size such as `4G` to cap the memory that Bash and PowerShell tool commands can use, so one runaway build can’t take the memory the rest of the session needs. Requires Claude Code v2.1.233 or later.
