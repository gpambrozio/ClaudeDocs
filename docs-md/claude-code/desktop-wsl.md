...
## [​](#start-a-wsl-session) Start a WSL session
...
## [​](#what-works-in-a-wsl-session) What works in a WSL session
...
On devices managed by an organization, WSL sessions may be unavailable. If session start fails with a message that the device is managed, that’s controlled by your administrator. Administrators: see [how settings reach devices](admin-setup.md) in the deployment guide.
