...
Claude Tag is available on Team and Enterprise plans, and is distinct from the earlier [Claude Code in Slack](slack.md), which runs each session under an individual user’s account. On Pro and Max plans, where Claude Tag isn’t available, Claude Code in Slack remains the setup path.
