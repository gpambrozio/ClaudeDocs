...
`strictKnownMarketplaces` restricts what users can add, but doesn’t register marketplaces on its own. To make allowed marketplaces available automatically without users running `/plugin marketplace add`, pair it with [`extraKnownMarketplaces`](settings.md) in the same `managed-settings.json`. See [Using both together](settings.md).
