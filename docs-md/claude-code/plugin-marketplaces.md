...
- Deploy separate [endpoint-managed settings](managed-settings.md), such as a managed settings file or an MDM profile, to each group’s devices. On each device, Claude Code applies only the [highest-ranked managed source](managed-settings.md) that delivers any keys, so this route works only when the per-group file or profile is that source on the group’s devices.
