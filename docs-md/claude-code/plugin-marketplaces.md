...
Administrators can block command sources across an organization with the managed setting [`disableCommandPluginSources`](settings.md). If an organization sets [`allowManagedHooksOnly`](settings.md), Claude Code blocks command sources by default.
...
For full configuration options, see [Plugin settings](settings.md).
...
For organizations requiring strict control over plugin sources, administrators can restrict which plugin marketplaces users are allowed to add using the [`strictKnownMarketplaces`](settings.md) setting in managed settings. To also reject the CLI flags that sideload plugins, agents, and MCP servers for a single run, pair it with [`disableSideloadFlags`](settings.md). To allowlist which marketplaces’ plugins can appear as contextual install suggestions, set [`pluginSuggestionMarketplaces`](settings.md).
`strictKnownMarketplaces` matches the marketplace a plugin comes from, not the entries inside it, so users can still install a plugin with a [`command` source](#command-sources) from an allowed marketplace. To block command sources as well, set [`disableCommandPluginSources`](settings.md).
...
On these machines, add the marketplace to [`extraKnownMarketplaces`](settings.md) in the same `managed-settings.json` so Claude Code registers it automatically, or run `claude plugin marketplace add anthropics/claude-plugins-official`.
...
Allow every marketplace repository under a GitHub organization with an [owner-wildcard](settings.md) entry. Owner wildcards require Claude Code v2.1.223 or later.
...
`strictKnownMarketplaces` restricts what users can add, but doesn’t register marketplaces on its own. To register an allowed marketplace for users automatically, add it to [`extraKnownMarketplaces`](settings.md) in the same `managed-settings.json`.The official Anthropic marketplace is the only one Claude Code registers on its own, and only when the allowlist allows it. Automatic registration also misses some machines, such as non-interactive environments and machines where an earlier policy blocked it. To cover those machines, add the official marketplace to `extraKnownMarketplaces` as well. For the two settings side by side, see the [`strictKnownMarketplaces` reference](settings.md).
...
To block every marketplace repository under a GitHub owner, use the owner-wildcard form in a `blockedMarketplaces` entry: `{ "source": "github", "repo": "untrusted-org/*" }`. Requires Claude Code v2.1.223 or later. For the matching rules, which differ between the blocklist and the allowlist, see [Owner wildcards](settings.md).
...
Because `strictKnownMarketplaces` is set in [managed settings](settings.md), individual users and project configurations can’t override these restrictions.
