...
The same connectivity message appears on an open network when the check reaches `api.anthropic.com` but presents a credential Anthropic rejects. A session whose resolved key is a gateway-issued credential, held in [`ANTHROPIC_API_KEY`](llm-gateway-connect.md) or produced by an [`apiKeyHelper`](settings.md), sends the check with that key, and the rejected request is reported as a connectivity failure.
