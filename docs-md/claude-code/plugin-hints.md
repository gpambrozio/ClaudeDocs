...
Claude Code sets the [`CLAUDECODE`](env-vars.md) environment variable to `1` for every command it runs through the Bash and PowerShell tools. When your CLI sees that variable, it writes a self-closing `<claude-code-hint />` tag to stderr.
