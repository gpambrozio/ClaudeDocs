...
`managed-mcp.json` is a standalone file, so it cannot be delivered through [server-managed settings](server-managed-settings.md). Any process that can write to a system path with administrator privileges can deploy it. At scale, that’s usually through device management tooling, such as Jamf or a configuration profile on macOS, Group Policy or Intune on Windows, or your fleet management of choice on Linux. Claude Code looks for the file at one of these paths:
...
By default, deploying `managed-mcp.json` suppresses the [claude.ai connectors](mcp.md) Claude Code fetches itself, including connectors an administrator configured for the organization in the claude.ai admin console. To load those connectors alongside the servers in `managed-mcp.json`, set `"allowAllClaudeAiMcps": true` in a [managed settings source](admin-setup.md). Requires Claude Code v2.1.149 or later.
...
Before loading a server, including one from `managed-mcp.json`, Claude Code runs the three checks below in order. In-process `type: "sdk"` servers, which the [app that started the session registers](mcp.md), skip all three.
