...
`/goal` works in [non-interactive mode](headless.md) and through [Remote Control](remote-control.md). Setting a goal with `-p` runs the loop to completion in a single invocation:
