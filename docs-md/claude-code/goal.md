...
[Auto mode](auto-mode-config.md) on its own approves tool calls within a single turn but doesn't start a new one. Claude stops when it judges the work done. `/goal` adds a separate evaluator that checks your condition after every turn, so completion is decided by a fresh model rather than the one doing the work. The two are complementary: auto mode removes per-tool prompts, and `/goal` removes per-turn prompts.

The approaches above keep the current session running. You can also schedule work that runs independent of any open session, such as nightly tests or morning triage. See [scheduling options](scheduled-tasks.md#compare-scheduling-options) for cloud routines and desktop scheduled tasks.
...
`/goal` works in [non-interactive mode](headless.md), in the [desktop app](desktop.md), and through [Remote Control](remote-control.md). Setting a goal with `-p` runs the loop to completion in a single invocation:
...
If Claude keeps answering the evaluator without making progress (no tool use for several turns in a row), Claude Code stops the loop, prints a warning, and returns control to you with the goal still set. Evaluation resumes after your next prompt. The [hooks guide](hooks-guide.md#stop-hook-hits-the-block-cap) explains the underlying mechanism.
...
* An authentication failure, when Claude Code manages its own credentials. When a host manages them for you, such as the desktop app, the VS Code extension, or a [cloud session](claude-code-on-the-web.md), Claude Code leaves the goal active because the host restores access on its own.
* An exhausted credit balance
* A context overflow that [auto-compaction](model-config.md#set-the-auto-compact-window) couldn't clear
...
* **Pause**: after a failure that a retry would only repeat, such as an API rate limit, a claude.ai [usage limit](errors.md#youve-hit-your-session-limit), or a hook that ended the turn, a notice starting with `Goal paused` names the cause. If the session is [waiting to continue automatically when a usage limit resets](interactive-mode.md#wait-for-a-usage-limit-to-reset), Claude resumes work toward the goal then.

Send a message at any time to start the next turn immediately. To turn automatic retries off, set [`CLAUDE_CODE_GOAL_CHECKIN_MINUTES`](env-vars.md) to `0`, which also turns [check-ins](#background-work-defers-evaluation) off.
...
To change the first interval, set [`CLAUDE_CODE_GOAL_CHECKIN_MINUTES`](env-vars.md). Claude Code uses your value in place of the 30-minute interval and scales the later intervals with it. Set it to `0` to turn off check-ins and [automatic retries](#other-errors-retry-or-pause-the-goal).

Check-ins require Claude Code v2.1.234 or later.

### Evaluation model and cost

To evaluate on a different model, set [`ANTHROPIC_DEFAULT_HAIKU_MODEL`](model-config.md#environment-variables).

Claude Code reads `ANTHROPIC_DEFAULT_HAIKU_MODEL` everywhere it uses the small fast model, not only for `/goal` evaluation. When you set it, Claude Code also resolves the [`haiku` alias](model-config.md#model-aliases) to that model and runs [background functionality](costs.md#background-token-usage), such as conversation summarization, on it.
...
Claude Code makes `/goal` available under the same [workspace trust rule as hooks in settings files](permissions.md#what-runs-before-you-trust-a-folder), because the evaluator is part of the hooks system. `/goal` is also unavailable when [`disableAllHooks`](hooks.md#disable-or-remove-hooks) is `true` after settings precedence applies, or when [`allowManagedHooksOnly`](settings-reference.md#allowmanagedhooksonly) is set in managed settings. In each case, the command tells you why instead of silently doing nothing.

## See also

* [Run a prompt repeatedly with `/loop`](scheduled-tasks.md#run-a-prompt-repeatedly-with-%2Floop): re-run on a time interval instead of toward a condition
