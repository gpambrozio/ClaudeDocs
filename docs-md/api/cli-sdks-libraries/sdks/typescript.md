...
Note that requests that time out are [retried twice by default](typescript.md#retries).

## Long requests

Consider using the streaming [Messages API](typescript.md#streaming-responses) for longer running requests.

Avoid setting a large `max_tokens` value without using streaming. Some networks may drop idle connections after a certain period of time, which can cause the request to fail or [timeout](typescript.md#timeouts) without receiving a response from Anthropic.

This SDK also throws an error if a non-streaming request is expected to be above roughly 10 minutes long. Passing `stream: true` or [overriding](typescript.md#timeouts) the `timeout` option at the client or request level disables this error.

An expected request latency longer than the [timeout](typescript.md#timeouts) for a non-streaming request will result in the client terminating the connection and retrying without receiving a response.

When supported by the `fetch` implementation, the SDK sets a [TCP socket keep-alive](https://tldp.org/HOWTO/TCP-Keepalive-HOWTO/overview.html) option to reduce the impact of idle connection timeouts on some networks. This can be [overridden](typescript.md#configuring-proxies) by configuring a custom proxy.
