...
This page collects the configuration surfaces, validation constraints, and error mappings for [Workload Identity Federation](build-with-claude/workload-identity-federation.md). For setup walkthroughs, see the [provider guides](build-with-claude/workload-identity-federation.md).
...
| `token_lifetime_seconds` | Integer between `60` and `86400` (1 minute to 24 hours). Default `3600`. Values outside this range are rejected at request time. See [Token lifetime and refresh](build-with-claude/workload-identity-federation.md). |
