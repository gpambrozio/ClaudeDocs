...
- 504 - `timeout_error`: The request timed out while processing. Consider using [streaming](build-with-claude/streaming.md) for long-running requests.
...
When receiving a [streaming](build-with-claude/streaming.md) response via SSE, it's possible that an error can occur after returning a 200 response, in which case error handling wouldn't follow these standard mechanisms.
