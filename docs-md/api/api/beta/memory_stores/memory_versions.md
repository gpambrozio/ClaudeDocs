...
- `data: optional array of BetaManagedAgentsMemoryVersion`
  ...
  - `memory_id: string`
    ...
    ID of the memory this version snapshots (a `mem_...` value). Remains valid after the memory is deleted; pass it as `memory_id` to [List memory versions](memory_versions/list.md) to retrieve the memory's retained versions, including the `deleted` row while the lineage is retained.
  ...
  - `redacted_at: optional string or null`
    ...
    When this version was redacted, in RFC 3339 format, or `null` if it has not been redacted. When set, `content`, `path`, `content_size_bytes`, and `content_sha256` are all `null`. See [Redact a memory version](memory_versions/redact.md).
