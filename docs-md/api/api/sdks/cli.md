...
For endpoint-specific parameters and response schemas, see the [API reference](api/cli/messages/create.md). This page covers CLI-specific features and workflows that apply across all endpoints.
...
With the binary installed and `ANTHROPIC_API_KEY` set, call the [Messages API](api/cli/messages/create.md):
...
3. 3
   ...
   A session runs in an [environment](api/cli/beta/environments.md), which defines the container it executes in. Write the environment definition to `summarizer.environment.yaml`:
