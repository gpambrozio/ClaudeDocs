...
The API prioritizes requests in this tier over all other requests. This prioritization helps minimize ["server overloaded" errors](errors.md#http-errors), even during peak times.

For more information, see [Existing Priority Tier commitments](service-tiers.md#existing-priority-tier-commitments).
...
* For [US-only inference](../manage-claude/data-residency.md) (`inference_geo: "us"`) requests on Claude 4.6 and later models, input tokens are 1.1 tokens per token
* All other input tokens are 1 token per token

**Output tokens**

* For [US-only inference](../manage-claude/data-residency.md) (`inference_geo: "us"`) requests on Claude 4.6 and later models, output tokens are 1.1 tokens per token
...
Check the [Models overview](../models/overview.md) for more details on available models.
