...
This guide covers migrating [Messages API](build-with-claude/working-with-messages.md) code. If you use [Claude Managed Agents](managed-agents/overview.md), no changes beyond updating model name are required.
...
**Automate this migration with the Claude API skill.** In Claude Code, run `/claude-api migrate` to invoke the bundled [Claude API skill](agents-and-tools/agent-skills/claude-api-skill.md):
...
1. **Extended thinking removed:** `thinking: {type: "enabled", budget_tokens: N}` is no longer supported on Claude Opus 4.7 or later models and returns a 400 error. Switch to [adaptive thinking](build-with-claude/adaptive-thinking.md) (`thinking: {type: "adaptive"}`) and use the [effort parameter](build-with-claude/effort.md) to control thinking depth. Adaptive thinking is **off by default** on Claude Opus 4.7: requests with no `thinking` field run without thinking, matching Opus 4.6 behavior. Set `thinking: {type: "adaptive"}` explicitly to enable it.
   ...
   Adaptive thinking is steerable through prompting. For guidance on tuning when the model over- or under-thinks, see [Calibrating effort and thinking depth](build-with-claude/prompt-engineering/claude-prompting-best-practices.md).
...
4. **Updated token counting:** Claude Opus 4.7 uses a new tokenizer, contributing to its improved performance on a wide range of tasks. The new tokenizer may use roughly 1x to 1.35x as many tokens when processing text compared to previous models (up to ~35% more, varying by content).
   ...
   [`/v1/messages/count_tokens`](build-with-claude/token-counting.md) will return a different number of tokens for Claude Opus 4.7 than it did for Claude Opus 4.6. Token efficiency can vary by workload shape.
...
6. **Stricter effort calibration:** Meaningfully changing from Claude Opus 4.6, Claude Opus 4.7 respects [effort levels](build-with-claude/effort.md) strictly, especially at the low end. At `low` and `medium`, the model scopes its work to what was asked rather than going above and beyond.
   ...
   If you need to keep effort at `low` for latency, add targeted guidance: "This task involves multi-step reasoning. Think carefully through the problem before responding." See [Recommended effort levels for Claude Opus 4.7](build-with-claude/effort.md).
...
2. **Audit token-count expectations:** Any code path that estimates tokens client-side or assumes a fixed token-to-character ratio should be re-tested against Claude Opus 4.7. Use the [Token counting endpoint](build-with-claude/token-counting.md) to verify.
...
2. **Update tool versions**
   ...
   - **Text editor:** Use `text_editor_20250728` and `str_replace_based_edit_tool`. See [Text editor tool documentation](agents-and-tools/tool-use/text-editor-tool.md) for details.
   - **Code execution:** Upgrade to `code_execution_20250825`. See [Code execution tool documentation](agents-and-tools/tool-use/code-execution-tool.md) for migration instructions.
...
Sonnet 4.6 pricing is $3 per million input tokens, $15 per million output tokens. See [Claude pricing](about-claude/pricing.md) for details.
...
1. **Prefilling assistant messages is no longer supported**
   ...
   Prefilling assistant messages returns a `400` error on Sonnet 4.6. Use [structured outputs](build-with-claude/structured-outputs.md), system prompt instructions, or `output_config.format` instead.
...
Sonnet 4.5 pricing is $3 per million input tokens, $15 per million output tokens. See [Claude pricing](about-claude/pricing.md) for details.
...
Haiku 4.5 pricing is $1 per million input tokens, $5 per million output tokens. See [Claude pricing](about-claude/pricing.md) for details.
...
**Review new rate limits:** Haiku 4.5 has separate rate limits from Haiku 3.5 and Haiku 3. See [Rate limits documentation](api/rate-limits.md) for details.
