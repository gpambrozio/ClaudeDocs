...
Work with your support team to [define clear success criteria](test-and-evaluate/define-success.md) and write [detailed evaluations](test-and-evaluate/develop-tests.md) with measurable benchmarks and goals.
...
The [Claude Console](/dashboard) now features an Evaluation tool that allows you to test your prompts under various scenarios.
...
- [Keep Claude in character](test-and-evaluate/strengthen-guardrails/keep-claude-in-character.md): Prevent Claude from changing their style of context, even during long, complex interactions.
...
- Add additional [tools](build-with-claude/tool-use.md) to the prompt (which can include [prompt chains](build-with-claude/prompt-engineering/chain-prompts.md)) and see if you can achieve the functionality required.
