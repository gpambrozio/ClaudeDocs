...
* **Per-request cost:** per-token pricing is lower than Claude Sonnet 4.6's (see [Pricing](models/sonnet-5/whats-new-sonnet-5.md)), but because the same text produces more tokens, the cost of an equivalent request does not drop in direct proportion.

## API constraints inherited from Claude Sonnet 4.6

This constraint is unchanged from Claude Sonnet 4.6. Aside from the three [behavior changes](models/sonnet-5/whats-new-sonnet-5.md) (see [Migration guide](models/sonnet-5/whats-new-sonnet-5.md)), code that already runs on Claude Sonnet 4.6 needs no other changes.
...
Claude Sonnet 5 is priced at $2 per million input tokens and $10 per million output tokens, lower per-token pricing than Claude Sonnet 4.6's $3/$15. Because the [new tokenizer](models/sonnet-5/whats-new-sonnet-5.md) produces approximately 30% more tokens for the same text, the cost of an equivalent request does not drop in direct proportion to the per-token prices when comparing with Claude Sonnet 4.6. The exact difference depends on the content and workload shape.
...
1. **Token budgets and counts:** the [new tokenizer](models/sonnet-5/whats-new-sonnet-5.md) produces approximately 30% more tokens for the same text. The exact increase depends on the content and workload shape. Recount prompts with [token counting](build-with-claude/token-counting.md), and revisit `max_tokens` limits sized close to your expected output length.
