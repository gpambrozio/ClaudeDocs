...
* **Pricing:** the same as Claude Fable 5, except for a [lower cache read price](models/fable-5-1/whats-new-fable-5-1.md).
...
* **Answers from memory more often at `low` effort.** At the lowest effort level the model calls a search or retrieval tool less often. Raise effort for turns that need fresh information, including [mid-conversation](models/fable-5-1/whats-new-fable-5-1.md), or add the verification nudge from [Search triggering at low effort](build-with-claude/prompt-engineering/prompting-claude-fable-5-1.md).
...
2. Pass thinking blocks back unchanged and keep the history append-only. If your code builds the `messages` array itself, run the [history-editing check](models/fable-5-1/migration-guide.md): move per-turn reminders you currently inject and delete to [turn-scoped system messages](models/fable-5-1/whats-new-fable-5-1.md), move `system` and `tools` changes to mid-conversation system messages, trim context server-side or strip thinking blocks from turns you carry across a client-side summary, then pick a production [`prefix_mismatch_behavior`](build-with-claude/thinking.md) and monitor `input_transformations`.
3. Re-tune effort from the default (`high`), and consider [changing it mid-conversation](models/fable-5-1/whats-new-fable-5-1.md) instead of holding one level for the whole session.
...
5. Re-run your evals. Refusal handling, fallback, fallback credit, and token counts carry over unchanged. Cache reads cost less (see [Pricing](models/fable-5-1/whats-new-fable-5-1.md)), and default behavior differs in the ways listed under [Changed from Claude Fable 5](models/fable-5-1/whats-new-fable-5-1.md).
