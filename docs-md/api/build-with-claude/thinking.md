...
You don't always see this text, and what you see is never the raw chain of thought: the text in a thinking block is a [summary of Claude's reasoning](build-with-claude/thinking.md). The `display` field on the thinking configuration controls whether that summary is returned at all: `"summarized"` returns it, while `"omitted"`, the default on many models, returns thinking blocks with an empty `thinking` field. Either way the block is billed the same and passed back the same in multi-turn conversations. See [Controlling thinking display](build-with-claude/thinking.md) for per-model defaults and details.

If Claude uses tools, thinking can also appear between tool calls. See [Thinking with tool use](build-with-claude/thinking.md). For the full response format, see the [Messages API reference](api/messages/create.md).
...
Thinking tokens count toward `max_tokens`, so set it high enough to leave room for both the thinking and the response text. See [Cost control](build-with-claude/thinking-steering-and-cost.md) on the steering page and [Thinking and the context window](build-with-claude/thinking.md).
...
* `"summarized"`: thinking blocks contain [summarized thinking](build-with-claude/thinking.md) text, a readable summary of Claude's reasoning. This is the default on Claude Opus 4.6, Claude Sonnet 4.6, and earlier models.
* `"omitted"`: thinking blocks are returned with an empty `thinking` field. The `signature` field still carries the encrypted full thinking for multi-turn continuity (see [Thinking encryption](build-with-claude/thinking.md)). This is the default on Claude Fable 5.1, Claude Mythos 5.1, Claude Fable 5, Claude Mythos 5, Claude Opus 5, Claude Sonnet 5, Claude Opus 4.8, Claude Opus 4.7, and [Claude Mythos Preview](https://anthropic.com/glasswing).
* `"updates"` (beta): reasoning blocks are returned with an empty `thinking` field, as with `"omitted"`, and the short [progress updates](build-with-claude/thinking.md) some models write between tool calls come back as readable text. Requires the beta header `thinking-display-updates-2026-08-18`.
...
* When streaming with `display: "omitted"`, no `thinking_delta` events are emitted. With `display: "updates"`, only [progress-update blocks](build-with-claude/thinking.md) stream `thinking_delta` events. See [Streaming thinking](build-with-claude/thinking.md) for the event sequence.
...
With `display: "updates"` (beta), reasoning blocks stream as they do under `"omitted"`. Each [progress-update block](build-with-claude/thinking.md) streams its text as `thinking_delta` events before the `tool_use` block it introduces. A pause of several seconds before the progress-update block opens is normal:
...
1. **Tool choice limitation (manual mode):** tool use with manual extended thinking (`thinking: {type: "enabled"}`) only supports `tool_choice: {"type": "auto"}` (the default) or `tool_choice: {"type": "none"}`. Using `tool_choice: {"type": "any"}` or `tool_choice: {"type": "tool", "name": "..."}` results in an error because these options force tool use, which is incompatible with manual extended thinking. Adaptive thinking, including on models where thinking is on by default, supports forced tool use, except on Claude Fable 5.1 and Claude Mythos 5.1 (see [Response prefill and forced tool use](build-with-claude/thinking.md)).
2. **Preserving thinking blocks:** when you return tool results, you must pass the thinking blocks from the assistant message back to the API, complete and unmodified. See [Preserving thinking blocks](build-with-claude/thinking.md).
...
Toggling thinking modes also invalidates prompt caching. See [Thinking and prompt caching](build-with-claude/thinking.md).
...
* When [streaming](build-with-claude/thinking.md), expect a pause of several seconds before a progress-update block opens. See the `"updates"` trace in [Streaming thinking](build-with-claude/thinking.md).
...
**Degradation strips thinking from the cacheable history.** If thinking becomes disabled mid-turn and you pass thinking content in the current tool-use turn, the thinking content is stripped and thinking remains disabled for that request (see [graceful degradation](build-with-claude/thinking.md)). [Interleaved thinking](build-with-claude/thinking.md) amplifies cache invalidation effects, because thinking blocks can occur between multiple tool calls.
...
* **Prior-turn thinking** depends on the [preservation default](build-with-claude/thinking.md). On [models that keep all prior turns](build-with-claude/thinking.md), previous thinking blocks remain in context, count toward the window, and are billed as input tokens like the rest of the conversation history. On models that keep only the last turn, the API strips older thinking blocks automatically when you pass them back, so they don't consume window space or input tokens.
...
* It is only strictly necessary to send back thinking blocks when [using tools with thinking](build-with-claude/thinking.md). Otherwise you can omit thinking blocks from previous turns. If you do pass them back, whether the API keeps or strips them depends on the model (see [Thinking block preservation by model](build-with-claude/thinking.md)). Use [context editing](build-with-claude/context-editing.md) to configure this.
...
* When [streaming responses](build-with-claude/thinking.md), the signature arrives as a `signature_delta` inside a `content_block_delta` event just before the `content_block_stop` event.
...
The `data` field is opaque and encrypted. Like the `signature` field on regular thinking blocks, pass `redacted_thinking` blocks back to the API unchanged when continuing a multi-turn conversation with [tools](build-with-claude/thinking.md).
...
`redacted_thinking` blocks are a distinct content block type returned when thinking is safety-redacted. This is separate from the [`display: "omitted"`](build-with-claude/thinking.md) option, which returns regular `thinking` blocks with an empty `thinking` field.
