...
*2 Claude Opus 5 accepts `"disabled"` at [effort](effort.md) `high` or below; combining it with effort `xhigh` or `max` returns a 400 error. This restriction is enforced on each request.*
...
This happens because thinking is always on for all of these models (see the [per-model configuration table](thinking-troubleshooting.md#rejected-configurations)).
...
A block whose `thinking` field is empty is still complete: the `signature` holds the reasoning. Send it back with the turn like any other. See [Send assistant turns back exactly as returned](preserved-thinking.md#append-assistant-turns-exactly-as-returned).
