...
Background compaction, often called async compaction, changes two things in the [compaction loop](compaction-on-demand.md#compact-in-a-loop): the compaction request runs while the conversation continues on its full history, and the swap waits until the block arrives. [Continue from the summary](compaction-on-demand.md#continue-from-the-summary) and [Handle a missing summary or an error](compaction-on-demand.md#when-no-summary-comes-back) apply unchanged.
...
1. Send the [compaction request](compaction-on-demand.md#request-a-summary) with your history as it stands, and record how many messages it held.
...
If the response has any other `stop_reason`, no summary was produced, which counts as a failure in step 2. Keep the full history; [Handle a missing summary or an error](compaction-on-demand.md#when-no-summary-comes-back) lists the causes and what to do for each.
...
Turns that arrive while the summary is being written are kept turns. If you send thinking blocks back on a model with [preserved thinking](preserved-thinking.md), the thinking in those turns stays valid only while the [conditions for kept thinking](compaction-thinking-blocks.md#conditions-for-kept-thinking-to-stay-valid) hold.
