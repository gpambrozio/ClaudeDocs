...
See [Migrating to adaptive thinking](build-with-claude/extended-thinking.md) to move to adaptive thinking. If your model supports only extended thinking, this page describes the supported configuration; no change is needed until you move to a newer model.

If a request fails with a 400 error whose message starts with `"thinking.type.enabled" is not supported`, your model uses adaptive thinking instead. See [Troubleshooting thinking](build-with-claude/thinking-troubleshooting.md), or jump to [Migrating to adaptive thinking](build-with-claude/extended-thinking.md).
...
When you are ready to move off manual budgets, see [Migrating to adaptive thinking](build-with-claude/extended-thinking.md).
...
* `budget_tokens` can exceed `max_tokens` here; the [budget rules](build-with-claude/extended-thinking.md) explain this exception.
...
Switching modes is a thinking-configuration change, so the first request after the switch invalidates cache breakpoints, as described in [Prompt caching in manual mode](build-with-claude/extended-thinking.md).
