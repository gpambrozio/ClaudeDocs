...
Keep-tail compaction keeps the last few turns of a conversation word for word after the summary. It changes two things in the [compaction loop](compaction-on-demand.md#compact-in-a-loop): which messages go into the compaction request, and what you send after the block. Everything in [Continue from the summary](compaction-on-demand.md#continue-from-the-summary) applies unchanged.
...
Send the kept turns exactly as they are in your history, thinking blocks included. Both requests carry the beta header, as in [Request a summary](compaction-on-demand.md#request-a-summary).
...
The next request sends the returned block first, then the kept turn exactly as it was, then the new `user` message. [Continue from the summary](compaction-on-demand.md#continue-from-the-summary) shows a request that starts with a block.

The following program is the loop from [Compact in a loop](compaction-on-demand.md#compact-in-a-loop), changed to keep the last two turns. The highlighted lines show where it differs from the loop.
...
If you send thinking blocks back on a model with [preserved thinking](preserved-thinking.md), the thinking in the kept turns stays valid only while the [conditions for kept thinking](compaction-thinking-blocks.md#conditions-for-kept-thinking-to-stay-valid) hold, and one of them limits where the cut can fall.
