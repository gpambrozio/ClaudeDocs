...
- Rate limits apply to both Batches API HTTP requests and the number of requests within a batch waiting to be processed. See [Message Batches API rate limits](api/rate-limits.md). Additionally, we may slow down processing based on current demand and your request volume. In that case, you may see more requests expiring after 24 hours.
...
If your result has an error, its `result.error` will be set to our standard [error shape](api/errors.md).
