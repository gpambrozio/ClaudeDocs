...
The effort parameter is supported by [Claude Mythos Preview](https://anthropic.com/glasswing), Claude Opus 4.6, Claude Sonnet 4.6, and Claude Opus 4.5.
...
- **Claude Mythos Preview** uses [adaptive thinking](build-with-claude/adaptive-thinking.md) by default (no `thinking` configuration required). `thinking: {type: "disabled"}` is rejected. Effort controls thinking depth the same way as on Opus 4.6.
