...
| **Client-side** | SDK           | Compaction                                                                                            | Available in [TypeScript and Ruby SDKs](cli-sdks-libraries/overview.md) when using [`tool_runner`](agents-and-tools/tool-use/tool-runner.md). Generates a summary and replaces full conversation history. See [Client-side compaction](build-with-claude/context-editing.md). |
...
The simplest way to enable tool result clearing is to specify only the strategy type. All other [configuration options](build-with-claude/context-editing.md) use their default values:
...
Compaction is available in the [TypeScript and Ruby SDKs](cli-sdks-libraries/overview.md) when using the [`tool_runner` method](agents-and-tools/tool-use/tool-runner.md).
...
1. **Threshold check:** The SDK calculates total tokens as `input_tokens + cache_creation_input_tokens + cache_read_input_tokens + output_tokens` (see [Prompt caching](build-with-claude/prompt-caching.md) for the cache token fields).
...
Compaction runs client-side in the SDK `tool_runner` helpers, so it has no direct HTTP equivalent. Use [server-side compaction](build-with-claude/compaction.md) instead, which handles compaction on Anthropic's servers.
...
In v1.0 and later, the Python SDK's tool runner does not support client-side `compaction_control`. Use [server-side compaction](build-with-claude/compaction.md) instead: it works with the tool runner by passing the `compact_20260112` edit in the request's `context_management` parameter.
...
The C# SDK includes a tool runner, but it does not support client-side `compaction_control`. Use [server-side compaction](build-with-claude/compaction.md) instead: it works with the tool runner by passing the `compact_20260112` edit in the request's `context_management` parameter.

**Go**

The Go SDK includes a tool runner, but it does not support client-side `compaction_control`. Use [server-side compaction](build-with-claude/compaction.md) instead: it works with the tool runner by passing the `compact_20260112` edit in the request's `context_management` parameter.

**Java**

The Java SDK includes a tool runner, but it does not support client-side `compaction_control`. Use [server-side compaction](build-with-claude/compaction.md) instead: it works with the tool runner by passing the `compact_20260112` edit in the request's `context_management` parameter.

**PHP**

The PHP SDK includes a tool runner, but it does not support client-side `compaction_control`. Use [server-side compaction](build-with-claude/compaction.md) instead: it works with the tool runner by passing the `compact_20260112` edit in the request's `context_management` parameter.
...
| `summary_prompt`          | string  | No       | See [Default summary prompt](build-with-claude/context-editing.md) | Custom prompt for summary generation     |
...
In v1.0 and later, the Python SDK's tool runner does not support `compaction_control`. Use [server-side compaction](build-with-claude/compaction.md) instead.
...
The C# SDK's tool runner does not support `compaction_control`. Use [server-side compaction](build-with-claude/compaction.md) instead.

**Go**

The Go SDK's tool runner does not support `compaction_control`. Use [server-side compaction](build-with-claude/compaction.md) instead.

**Java**

The Java SDK's tool runner does not support `compaction_control`. Use [server-side compaction](build-with-claude/compaction.md) instead.

**PHP**

The PHP SDK's tool runner does not support `compaction_control`. Use [server-side compaction](build-with-claude/compaction.md) instead.
