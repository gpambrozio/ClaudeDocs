...
For most use cases, [server-side compaction](compaction.md) is the primary strategy for managing context in long-running conversations. The strategies on this page are useful for specific scenarios where you need more fine-grained control over what content is cleared.
...
| **Client-side** | SDK           | Compaction                                                                                            | Available in [TypeScript and Ruby SDKs](../cli-sdks-libraries/overview.md) when using [`tool_runner`](../agents-and-tools/tool-use/tool-runner.md). Generates a summary and replaces full conversation history. See [Client-side compaction](context-editing.md#client-side-compaction-sdk). |
...
An assistant conversation turn may include multiple content blocks (for example, when using tools) and multiple thinking blocks (for example, with [interleaved thinking](thinking.md#interleaved-thinking)).
...
The simplest way to enable tool result clearing is to specify only the strategy type. All other [configuration options](context-editing.md#configuration-options-for-tool-result-clearing) use their default values:
...
The [token counting](token-counting.md) endpoint supports context management, allowing you to preview how many tokens your prompt will use after context editing is applied.
...
Compaction runs client-side in the SDK `tool_runner` helpers, so it has no direct HTTP equivalent. Use [server-side compaction](compaction.md) instead, which handles compaction on Anthropic's servers.

**CLI**

The CLI does not include a `tool_runner` helper. Use [server-side compaction](compaction.md) instead, which handles compaction on Anthropic's servers without SDK-side integration.
...
| `summary_prompt`          | string  | No       | See [Default summary prompt](context-editing.md#default-summary-prompt) | Custom prompt for summary generation     |
...
* Use the [token counting](token-counting.md) endpoint to get accurate context length
...
In v1.0 and later, the Python SDK's tool runner does not support `compaction_control`. Use [server-side compaction](compaction.md) instead.
...
The C# SDK's tool runner does not support `compaction_control`. Use [server-side compaction](compaction.md) instead.

**Go**

The Go SDK's tool runner does not support `compaction_control`. Use [server-side compaction](compaction.md) instead.

**Java**

The Java SDK's tool runner does not support `compaction_control`. Use [server-side compaction](compaction.md) instead.

**PHP**

The PHP SDK's tool runner does not support `compaction_control`. Use [server-side compaction](compaction.md) instead.
