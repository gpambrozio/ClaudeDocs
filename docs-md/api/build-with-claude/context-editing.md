...
| **Client-side** | SDK | Compaction | Available in [Python and TypeScript SDKs](api/client-sdks.md) when using [`tool_runner`](agents-and-tools/tool-use/implement-tool-use.md). Generates a summary and replaces full conversation history. See [Compaction](#client-side-compaction-sdk) below. |
