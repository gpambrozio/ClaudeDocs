...
- **Authorization:** The OAuth `scope` granted on the minted token. At launch this is always `workspace:developer`, which grants the same access as an API key issued for that workspace (see [OAuth scopes](api/authentication/wif-reference.md)). The rule also sets `token_lifetime_seconds` (60 to 86400, default 3600).
...
3. 3
   ...
   | Authorization | OAuth scope (`workspace:developer` at launch; see [OAuth scopes](api/authentication/wif-reference.md)) and token lifetime in seconds. |
...
The token-exchange response follows [RFC 6749 §5.1](https://www.rfc-editor.org/rfc/rfc6749#section-5.1). See [Token exchange response](api/authentication/wif-reference.md) for the field reference.
...
For the full precedence table, the per-tier semantics, and the profile file schema, see [Credential precedence](api/authentication/wif-reference.md) in the WIF reference.
...
STS web identity tokens, or EKS IRSA projected tokens.](build-with-claude/wif-providers/aws.md)[Google Cloud

Google-signed identity tokens via the metadata server.](build-with-claude/wif-providers/gcp.md)[Microsoft Azure

Managed Identity (IMDS) and Entra Workload ID on AKS.](build-with-claude/wif-providers/azure.md)[GitHub Actions

Keyless CI authentication with the Actions OIDC token.](build-with-claude/wif-providers/github-actions.md)[Kubernetes

Self-managed and on-premises clusters using projected service-account tokens.](build-with-claude/wif-providers/kubernetes.md)[Okta

Okta service applications using client-credentials flow.](build-with-claude/wif-providers/okta.md)

## See also

- [WIF reference](api/authentication/wif-reference.md): environment variables, profile file schema, validation rules, and error codes
- [Authentication](api/authentication/overview.md): all authentication options across the Anthropic SDKs
