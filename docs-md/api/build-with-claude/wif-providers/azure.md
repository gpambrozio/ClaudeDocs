...
Follow the [setup walkthrough](build-with-claude/workload-identity-federation.md) to register a federation issuer, create an Anthropic service account, and create a federation rule in the Claude Console. In the Console, choose the **OIDC** provider option and supply the Entra-specific values that follow.
...
Alternatively, register your AKS cluster's OIDC issuer with Anthropic directly and skip the Entra hop. See [Kubernetes](build-with-claude/wif-providers/kubernetes.md) for that pattern.
...
- Review the full configuration model in [Workload Identity Federation](build-with-claude/workload-identity-federation.md).
- See the [provider guides](build-with-claude/workload-identity-federation.md) for AWS, Google Cloud, GitHub Actions, and Kubernetes.
- For environment variables, profile files, and credential precedence, see the [WIF reference](api/authentication/wif-reference.md).
