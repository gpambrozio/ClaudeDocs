...
Follow the [setup walkthrough](build-with-claude/workload-identity-federation.md) to register a federation issuer, create an Anthropic service account, and create a federation rule in the Claude Console. Use these Google Cloud-specific values.
...
If you prefer the file-based path, have your container entrypoint write the metadata-server response to a file and set `ANTHROPIC_IDENTITY_TOKEN_FILE`, `ANTHROPIC_FEDERATION_RULE_ID`, `ANTHROPIC_ORGANIZATION_ID`, and `ANTHROPIC_SERVICE_ACCOUNT_ID` as environment variables on the Cloud Run service. The SDK constructor with no arguments then picks the federation path up automatically.
...
- Read the [Workload Identity Federation](build-with-claude/workload-identity-federation.md) page for the full resource model and SDK credential precedence.
