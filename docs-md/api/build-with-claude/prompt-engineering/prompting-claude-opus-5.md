...
This guide covers the prompting patterns specific to Claude Opus 5. For the model's capabilities and API changes, see [What's new in Claude Opus 5](../../models/opus-5/whats-new-opus-5.md). For techniques that apply across all current Claude models, see [Prompting best practices](claude-prompting-best-practices.md).
...
For API changes when migrating from Claude Opus 4.8 (thinking on by default, and disabling thinking capped at `high` effort), see the [migration guide](../../models/opus-5/migration-guide.md#migrating-from-claude-opus-4-8-to-claude-opus-5).
...
* **Long-context work:** Claude Opus 5 has a [1M token context window](../context-windows.md) as both the default and the maximum, and its instruction following, tool calling, and reasoning stay consistent throughout the window.
...
* **Multi-agent coordination:** Claude Opus 5 coordinates teams of subagents well, with effective writer-verifier patterns and few cases of agents overwriting each other's work. For cost-sensitive workloads, cap delegation; see [Controlling subagent spawning](prompting-claude-opus-5.md#controlling-subagent-spawning).

## Response length and verbosity

Claude Opus 5's default user-facing responses run longer than prior Opus models'. The [effort parameter](../effort.md) controls how much the model [thinks](../thinking-steering-and-cost.md) rather than how much it says: lowering effort can reduce thinking volume without reliably shortening the visible response. To control response length, prompt for it explicitly.
