...
If Claude's [response is cut off due to hitting the `max_tokens` limit](build-with-claude/handling-stop-reasons.md), and the truncated response contains an incomplete tool use block, you'll need to retry the request with a higher `max_tokens` value to get the full tool use.
