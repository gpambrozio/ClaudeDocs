...
Direct `stop_reason` access on result messages is currently **TypeScript-only**. The Python SDK's `ResultMessage` does not include this field. For Python, see [Read stop\_reason in Python](#read-stop_reason-in-python) for a workaround using stream events.
...
- [Stream responses in real-time](agent-sdk/streaming-output.md): access raw API events including `message_delta` as they arrive
- [Structured outputs](agent-sdk/structured-outputs.md): get typed JSON responses from the agent
- [Tracking costs and usage](agent-sdk/cost-tracking.md): understand token usage and billing from result messages
