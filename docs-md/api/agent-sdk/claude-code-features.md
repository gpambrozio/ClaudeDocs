...
The setting sources option ([`setting_sources`](agent-sdk/python.md) in Python, [`settingSources`](agent-sdk/typescript.md) in TypeScript) controls which filesystem-based settings the SDK loads. Without it, your agent won't discover skills, `CLAUDE.md` files, or project-level hooks.
...
| Delegate an isolated subtask to a fresh context (research, review) | [Subagents](agent-sdk/subagents.md) | `agents` parameter + `allowedTools: ["Task"]` |
