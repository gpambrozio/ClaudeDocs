...
- **Authorization:** The OAuth `scope` granted on the minted token. The default is `workspace:developer`, which grants the same access as an API key issued for that workspace. Some products lock the scope when you create a rule from their flow; for example, the [MCP tunnels](agents-and-tools/mcp-tunnels/overview.md) create-tunnel modal creates rules scoped to `org:manage_tunnels`. See [OAuth scopes](manage-claude/wif-reference.md). The rule also sets `token_lifetime_seconds` (60 to 86400, default 3600).
...
3. 3
   ...
   | Authorization | OAuth scope (`workspace:developer` by default, or a product-specific scope such as `org:manage_tunnels`; see [OAuth scopes](manage-claude/wif-reference.md)) and token lifetime in seconds. |
...
runs (container env, CI secrets, shell profiles). The CLI's [`ant auth status`](api/sdks/cli.md)
...
Google-signed identity tokens from the metadata server.](manage-claude/wif-providers/gcp.md)[Microsoft Azure
