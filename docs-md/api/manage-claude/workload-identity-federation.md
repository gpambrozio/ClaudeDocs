...
- **Authorization:** The OAuth `scope` granted on the minted token. At launch this is always `workspace:developer`, which grants the same access as an API key issued for that workspace (see [OAuth scopes](manage-claude/wif-reference.md)). The rule also sets `token_lifetime_seconds` (60 to 86400, default 3600).
...
3. 3
   ...
   | Authorization | OAuth scope (`workspace:developer` at launch; see [OAuth scopes](manage-claude/wif-reference.md)) and token lifetime in seconds. |
