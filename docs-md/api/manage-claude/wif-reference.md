...
| `workspace:manage_tunnels` | The [MCP tunnels API](agents-and-tools/mcp-tunnels/reference.md): create, list, and get tunnels, register and archive CA certificates, reveal and rotate the tunnel token, and archive tunnels. The Console's create-tunnel modal locks this scope when you create a rule from it. |
