...
This page collects the configuration surfaces, validation constraints, and error mappings for [Workload Identity Federation](workload-identity-federation.md). For setup walkthroughs, see the [provider guides](workload-identity-federation.md#identity-providers).
...
| `ANTHROPIC_PROFILE`             | No                               | Name of a [configuration profile](wif-reference.md#profile-configuration-file) to load. Takes precedence over the federation environment variables in this table.                                                                                                                    | `staging-profile`                      |
...
| `workspace:manage_tunnels` | The [MCP tunnels API](../agents-and-tools/mcp-tunnels/reference.md#tunnels-api): create, list, and get tunnels, register and archive CA certificates, reveal and rotate the tunnel token, and archive tunnels. The Console's create-tunnel modal window locks this scope when you create a rule from it.                                                                                                                                                                                                                                      |
| `org:admin`                | Full access to the [Admin API](admin-api.md) (organization members, invites, workspaces, API keys, and the rest). An OAuth `org:admin` token can only create or modify rules scoped to `workspace:developer` or `workspace:inference`, and cannot update an issuer that backs a rule with any other scope; see the [constraints](wif-admin-api.md#permissions-and-constraints).                                                                                               |
...
| `org:admin`           | `admin`                             | Full Admin API access (organization members, invites, workspaces, API keys, and the rest), minus the OAuth-caller carve-outs; see [constraints](wif-admin-api.md#permissions-and-constraints). |
...
For complete parameter details and response schemas, see the [Service accounts API reference](../api/beta/organization/service_accounts.md), [Federation issuers API reference](../api/beta/organization/federation/issuers.md), and [Federation rules API reference](../api/beta/organization/federation/rules.md).
...
| `token_lifetime_seconds`                 | Integer between `60` and `86400` (1 minute to 24 hours). Default `3600`. Values outside this range are rejected at request time. See [Token lifetime and refresh](workload-identity-federation.md#token-lifetime-and-refresh).              |
...
| Token exchange succeeds but a Claude API request returns 403 | The minted token's scope does not grant access to that endpoint.                                                                                                            | Check the rule's `oauth_scope` against [OAuth scopes](wif-reference.md#oauth-scopes). |
...
If the issuer rotated its signing key and immediately started signing with it, exchanges can fail for up to a minute while Anthropic's JWKS cache refreshes. See [Key rotation and caching](wif-reference.md#key-rotation-and-caching).
