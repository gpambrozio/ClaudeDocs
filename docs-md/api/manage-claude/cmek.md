...
- Compliance API [local session transcripts](manage-claude/compliance-sessions.md) (Cowork and Claude Code on users' machines) currently return no message content. Session metadata is listed as usual, and the local session messages endpoint (`GET /v1/compliance/apps/sessions/local/{session_id}/messages`) returns each message with its content marked unavailable; see [Retrieve a local session transcript](manage-claude/compliance-sessions.md) for the response shape.
