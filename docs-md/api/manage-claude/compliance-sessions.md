...
The following table summarizes how [local sessions](#retrieve-local-sessions) and [remote sessions](#retrieve-remote-sessions) differ.
...
Project instruction files such as `CLAUDE.md` appear as ordinary user-role content. Skill content appears when the client sends it as message content and is not distinguished from other user text. For a coverage summary and a comparison with OpenTelemetry logging for Cowork and Claude Code, see the [Compliance API FAQ](manage-claude/compliance-faq.md).
...
The messages endpoint returns the session's transcript: user prompts, assistant responses, and tool calls and results. Thinking blocks and images are not included. For a coverage summary and a comparison with Cowork's OpenTelemetry logging, see the [Compliance API FAQ](manage-claude/compliance-faq.md).
