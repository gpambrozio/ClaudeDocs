...
The first save generates your webhook signing secret and reveals it once. Copy it and store it securely before clicking **Next**: the secret cannot be retrieved later, only [rotated](inference-hooks-configuration.md#rotate-your-signing-secret).

Your AI security server uses this secret to verify the signature on every request it receives, including the connection test in the next step. For the verification procedure, see [Verify the signature](inference-hooks-endpoint.md#verify-the-signature).

**Add request headers and test the connection**
