...
Inference hooks are in beta and available to Claude Enterprise organizations. Configuring them requires the `organization:manage` permission in claude.ai, which only the Owner and Primary owner roles hold; see [Configure Inference hooks](inference-hooks-configuration.md).
...
A verdict is a small JSON object: `{"action": "allow"}` lets the request proceed, and a deny carries the user-facing reason. For the full verdict schema, see [Return a verdict](inference-hooks-endpoint.md#return-a-verdict).
...
* **Claude Code.** The user runs `/rewind` and selects the prompt that first brought the content in. If asked, they select **Restore conversation**, then edit or clear the prompt that returns to the input field. `/clear` starts over. See [Checkpointing](../../claude-code/checkpointing.md).
...
* **Real-time transcript archival.** Archive each transcript as it arrives and always return `allow`, as a push-based alternative to polling the [Compliance API](compliance-api.md).
...
Use Inference hooks to stop a request before it reaches the model, and the [Compliance API](compliance-api.md) to audit what happened afterward.
