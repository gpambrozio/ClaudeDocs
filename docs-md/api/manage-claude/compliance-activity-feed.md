...
| *additional fields* | varies          | Type-specific fields, for example `claude_chat_id` on chat events or `claude_file_id` on file events. See [Query compliance activities](../api/compliance/activities/list.md) in the API reference for the per-type field list. |
