...
| `agent.tool_use`                 | Agent invokes a pre-built agent tool (bash, file operations, and so on). Carries `evaluated_permission` and, usually, `evaluation` (see [how each call was evaluated](permission-policies.md#see-how-each-call-was-evaluated)). |
| `agent.tool_result`              | Result of a pre-built agent tool execution.                                                                                                                                                                                                                                     |
| `agent.mcp_tool_use`             | Agent invokes an MCP server tool. Carries `evaluated_permission` and, usually, `evaluation` (see [how each call was evaluated](permission-policies.md#see-how-each-call-was-evaluated)).                                        |
