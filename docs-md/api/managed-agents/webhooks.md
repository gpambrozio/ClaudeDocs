...
| `agent.updated`  | A [new version of the agent](managed-agents/agent-setup.md) was published. Updates that do not create a new version do not trigger this event. |
...
| Event                      | Trigger                                                                                                                                                                                                                                                                                                                                                                   |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `deployment_run.started`   | A scheduled run started. Only scheduled runs emit `deployment_run` events; [manual runs](managed-agents/scheduled-deployments.md) do not.                                                                                                                                                                           |
| `deployment_run.succeeded` | A scheduled run created its session. The event carries the same `data.id` (the run ID) as the run's `deployment_run.started` event. To follow the session's work, subscribe to its session events (the Session events tab), or fetch the [deployment run](managed-agents/scheduled-deployments.md) for its `session_id`. |
| `deployment_run.failed`    | A scheduled run did not create a session. The event carries the same `data.id` as the run's `deployment_run.started` event. Fetch the [deployment run](managed-agents/scheduled-deployments.md) for the error details.                                                                                                   |
...
An environment's [work items](managed-agents/self-hosted-sandboxes.md) emit no webhook events.
...
Individual [memories](managed-agents/memory.md) and memory versions emit no webhook events.
...
Parse the body, switch on `data.type`, and fetch the resource by ID. Return any `2xx` to acknowledge. Any other response counts against the endpoint: a `3xx` disables it immediately (redirects are never followed), while other failures are retried; see [Delivery behavior](managed-agents/webhooks.md) for the retry and auto-disable rules.
