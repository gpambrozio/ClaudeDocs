...
Sessions are long-running interactions. While most real-time interactions happen through the [SSE event stream](events-and-streaming.md), webhooks notify you of major state changes.
...
Some of these events are named differently from the matching events on the session's [event stream](events-and-streaming.md). For example, the stream's `session.status_idle` and `session.status_running` correspond to the `session.status_idled` and `session.status_run_started` webhook events.
...
| `session.budget_reached`           | The session reached its [budget](budgets.md) and paused. Fires at most once for each budget value you set; changing the budget arms it again.                                                                                                                                                                                                                                                                                            |
...
| `session.thread_created`           | New [multiagent thread](multiagent-orchestration.md) opened: an additional agent called by the coordinator is starting work, or the session's [advisor](multiagent-orchestration.md#give-the-session-an-advisor) is being consulted.                                                                                                                                                     |
| `session.thread_idled`             | An agent in a [multiagent interaction](multiagent-orchestration.md) is waiting for input.                                                                                                                                                                                                                                                                                                                                                |
| `session.thread_terminated`        | A [multiagent thread](multiagent-orchestration.md) terminated, either because the thread was archived or because it exhausted its retries. A coordinator-spawned child that finishes its work goes `idle`, not `terminated` (an advisor thread terminates once its consultation completes). Fires for child threads only; the primary thread's end, including archiving the whole session, surfaces only as `session.status_terminated`. |
| `session.outcome_evaluation_ended` | [Outcome evaluation](define-outcomes.md) for a single iteration completed.                                                                                                                                                                                                                                                                                                                                                               |
...
| `agent.updated`  | A [new version of the agent](agent-setup.md#update-an-agent) was published. Updates that do not create a new version do not trigger this event. |
...
| Event                      | Trigger                                                                                                                                                                                                                                                                                                                                                                   |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `deployment_run.started`   | A scheduled run started. Only scheduled runs emit `deployment_run` events; [manual runs](scheduled-deployments.md#trigger-a-manual-run) do not.                                                                                                                                                                           |
| `deployment_run.succeeded` | A scheduled run created its session. The event carries the same `data.id` (the run ID) as the run's `deployment_run.started` event. To follow the session's work, subscribe to its session events (the Session events tab), or fetch the [deployment run](scheduled-deployments.md#deployment-runs) for its `session_id`. |
| `deployment_run.failed`    | A scheduled run did not create a session. The event carries the same `data.id` as the run's `deployment_run.started` event. Fetch the [deployment run](scheduled-deployments.md#deployment-runs) for the error details.                                                                                                   |
...
An environment's [work items](self-hosted-sandboxes.md) emit no webhook events.
...
Individual [memories](memory.md) and memory versions emit no webhook events.
...
Parse the body, switch on `data.type`, and fetch the resource by ID. Return any `2xx` to acknowledge. Any other response counts against the endpoint: a `3xx` disables it immediately (redirects are never followed), while other failures are retried; see [Delivery behavior](webhooks.md#delivery-behavior) for the retry and auto-disable rules.
