...
All agents share the same sandbox, filesystem, and [vault credentials](vaults.md), but each agent runs in its own **session thread**, a context-isolated event stream with its own conversation history. The coordinator reports activity in the **primary thread** (which is the same as the session-level [event stream](events-and-streaming.md)); additional threads are spawned at runtime when the coordinator delegates work.
...
Each agent uses its own configuration: model, system prompt, tools, MCP servers, and skills. Session-level [agent configuration overrides](sessions.md#override-agent-configuration-for-a-session) are the exception; they apply to the coordinator and its `self` copies. Tools, MCP servers, and context are not shared.
...
When [defining your agent](agent-setup.md), set `multiagent` to declare the roster of agents the coordinator can delegate to:
...
When agents pin an [inference geography](../manage-claude/data-residency.md) (`model.inference_geo` in the [agent definition](agent-setup.md)), the coordinator's pin and every roster member's pin must either all be set to the same value or all be unset. A mismatched roster is rejected with a 400 validation error, both when the agent is saved and when a [session-create override](sessions.md#override-agent-configuration-for-a-session) changes any of the pins.
...
The advisor model must meet a minimum capability bar, and the agent's own model must not be more capable than its advisor; models of equal capability can pair. An invalid pairing is rejected with a 400 validation error when the agent is saved. Valid pairings follow the advisor tool's [model compatibility](../agents-and-tools/tool-use/advisor-tool.md#model-compatibility) table.
...
Advisor threads are exempt from the concurrent-thread limit. They appear in the session's [thread list](multiagent-orchestration.md#threads) with `agent` set to the advisor form exactly as configured (`{"type": "advisor", "model": ...}`) and `parent_thread_id` set to the primary thread.
...
To remove the advisor, [update the agent](agent-setup.md#update-an-agent) with a roster that no longer includes the advisor entry. If the advisor is the roster's only entry, clear the roster entirely by setting `"multiagent": null`.
...
A [session budget](budgets.md) is a single shared cap across all of a session's threads. As the cap is reached, threads pause independently, and each thread's cost is priced at the thread's own served model.

A maximum of 25 concurrent threads is supported. The coordinator can call multiple copies of a single agent in the roster, creating multiple threads associated with one `agent`. [Advisor](multiagent-orchestration.md#give-the-session-an-advisor) consultation threads are exempt from this limit.
...
Advisor consultations emit these same thread events under the reserved name `anthropic.advisor` (as `agent_name` on the thread lifecycle events and `from_agent_name` on the advice delivery); see [Give the session an advisor](multiagent-orchestration.md#give-the-session-an-advisor) for the sequence.
