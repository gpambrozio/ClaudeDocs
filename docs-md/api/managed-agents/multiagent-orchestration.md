...
* `{"type": "advisor", "model": "<model id>"}` gives the session's primary thread an advisor it can consult mid-turn. At most one advisor entry per roster. See [Give the session an advisor](managed-agents/multiagent-orchestration.md).
...
Advisor threads are exempt from the concurrent-thread limit. They appear in the session's [thread list](managed-agents/multiagent-orchestration.md) with `agent` set to the advisor form exactly as configured (`{"type": "advisor", "model": ...}`) and `parent_thread_id` set to the primary thread.
...
A maximum of 25 concurrent threads is supported. The coordinator can call multiple copies of a single agent in the roster, creating multiple threads associated with one `agent`. [Advisor](managed-agents/multiagent-orchestration.md) consultation threads are exempt from this limit.
...
Advisor consultations emit these same thread events under the reserved name `anthropic.advisor` (as `agent_name` on the thread lifecycle events and `from_agent_name` on the advice delivery); see [Give the session an advisor](managed-agents/multiagent-orchestration.md) for the sequence.
