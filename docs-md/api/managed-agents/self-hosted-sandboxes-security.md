...
* **Per-session credentials.** Each work item your worker claims can carry a per-session `secret`, which the SDK worker uses in place of the environment service key. Access to [memory stores](memory.md) requires the `secret`: the memory store endpoints reject the environment key (see [Use memory stores](self-hosted-sandboxes.md#use-memory-stores)). Pass the `secret` only into the sandbox that serves that session, keep it out of images and shared volumes, and never log it.
