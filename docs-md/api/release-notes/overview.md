...
- We've launched the **`ant` CLI**, a command-line client for the Claude API that enables faster interaction with the Claude API, native integration with Claude Code, and versioning of API resources in YAML files. Learn more in the [CLI reference](api/sdks/cli.md).
...
- We've released the [Usage & Cost API](build-with-claude/usage-cost-api.md), allowing administrators to programmatically monitor their organization's usage and cost data.
...
- We've released the [Admin API](build-with-claude/administration-api.md), allowing users to programmatically manage their organization's resources.
...
- [Tool use](agents-and-tools/tool-use/overview.md) is now generally available across the Claude API, Amazon Bedrock, and Google Vertex AI.
